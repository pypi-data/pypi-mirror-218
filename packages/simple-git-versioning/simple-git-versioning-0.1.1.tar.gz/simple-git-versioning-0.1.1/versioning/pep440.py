...
class _Version(_Version, Protocol):
    ...
    def __str__(self) -> str:
        return (
            f"{f'{self.epoch}!' if self.epoch else ''}{'.'.join(map(str, self.release))}{f'{self.pre.label.value}{self.pre.revision}' if self.pre else ''}{f'.post{self.post}' if self.post is not None else ''}{f'.dev{self.dev}' if self.dev is not None else ''}{'+' + '.'.join(map(str, self.local)) if self.local else ''}"
        )
