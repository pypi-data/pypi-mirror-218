...
class Plate(OMEType):
    ...
    Parameters
    ----------
    description: A description for the plate.
    well:
    annotation_ref:
    plate_acquisition:
    id:
    name: The Name identifies the plate to the user. It is used much
        like the ID, and so must be unique within the document. If a
        plate name is not available when one is needed it will be
        ...
        ID.
    status: A textual annotation of the current state of the plate with
        respect to the experiment work-flow; e.g. 1. Seed cell: done; 2.
        Transfection: done;      3. Gel doc: todo.
    external_identifier: The ExternalIdentifier attribute may contain a
        reference to an external database.
    column_naming_convention: The ColumnNamingConvention
    row_naming_convention: The RowNamingConvention
    well_origin_x: This defines the X position to use for the origin of
        the fields (individual images) taken in a well. It is used with
        the X in the WellSample to display the fields in the correct
        position relative to each other. Each Well in the plate has the
        same well origin. Units are set by WellOriginXUnit. In the OMERO
        clients by convention we display the WellOrigin in the center of
        the view.
    well_origin_x_unit: The units of the well origin in X -
        default:reference frame.
    well_origin_y: This defines the Y position to use for the origin of
        the fields (individual images) taken in a well. It is used with
        the Y in the WellSample to display the fields in the correct
        position relative to each other. Each Well in the plate has the
        same well origin.  Units are set by WellOriginYUnit. In the
        OMERO clients by convention we display the WellOrigin in the
        center of the view.
    well_origin_y_unit: The units of the well origin in Y -
        default:reference frame.
    rows: The number of rows in the plate
    columns: The number of columns in the plate
    field_index: The index of the WellSample to display as the default
        Field
    """
