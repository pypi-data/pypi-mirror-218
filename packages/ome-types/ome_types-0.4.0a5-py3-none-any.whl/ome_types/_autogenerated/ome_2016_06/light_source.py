...
class LightSource(ManufacturerSpec):
    ...
    ----------
    annotation_ref:
    id: A LightSource ID must be specified for each light source, and
        the individual light sources can be referred to by their
        LightSource IDs (eg from Channel).
    power: The light-source power. Units are set by PowerUnit.
    power_unit: The units of the Power - default:milliwatts[mW].
    """
