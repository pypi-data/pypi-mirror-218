...
class JewishCalendar(JewishDate):
    ...
    def significant_day(self) -> Optional[str]:
        month_significant_day_function = getattr(self, f'{self.jewish_month_name()}_significant_day', None)
        if month_significant_day_function is not None:
            return month_significant_day_function()
        else:
            return None
