...
class Cycle:
    ...
    def __repr__(self):
        return "%s(start_date=%r, end_date=%r, iteration=%r)" % \
               (self.__module__ + "." + self.__class__.__qualname__,
                self.start_date, self.end_date, self.iteration)
    ...
    def from_perpetual_anchor(anchor: Anchor, cycle_end_calculation: Callable[[JewishDate, Optional[int]], JewishDate], date: JewishDate) -> 'Cycle':
        ...
        if start_date is None:
            raise ValueError("No start date found for perpetual cycle")
    ...
    def from_cycle_initiation(initial_cycle_date: JewishDate, cycle_end_calculation: Callable[[JewishDate, Optional[int]], JewishDate], date: JewishDate) -> Optional['Cycle']:
        ...
        while date > cycle.end_date:
            c = cycle.next(cycle_end_calculation)
            if c is None:
                raise ValueError("Could not find next cycle")
            cycle = c
        return cycle
