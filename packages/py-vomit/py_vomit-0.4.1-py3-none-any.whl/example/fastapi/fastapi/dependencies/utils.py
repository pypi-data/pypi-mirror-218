...
def get_flat_params(dependant: Dependant) -> List[ModelField]:
    ...
    return flat_dependant.path_params + flat_dependant.query_params + flat_dependant.header_params + flat_dependant.cookie_params
...
def analyze_param(*, param_name: str, annotation: Any, value: Any, is_path_param: bool) -> Tuple[Any, Optional[params.Depends], Optional[ModelField]]:
    ...
    if annotation is not inspect.Signature.empty and get_origin(annotation) is Annotated:
        ...
        assert len(fastapi_annotations) <= 1, f'Cannot specify multiple `Annotated` FastAPI arguments for {param_name!r}'
    ...
    if lenient_issubclass(type_annotation, (Request, WebSocket, HTTPConnection, Response, BackgroundTasks, SecurityScopes)):
    ...
    if field_info is not None:
        ...
        field = create_response_field(name=param_name, type_=use_annotation, default=field_info.default, alias=alias, required=field_info.default in (Required, Undefined), field_info=field_info)
...
def add_param_to_fields(*, field: ModelField, dependant: Dependant) -> None:
    ...
    if field_info.in_ == params.ParamTypes.path:
        ...
        assert field_info.in_ == params.ParamTypes.cookie, f'non-body parameters must be in path, query, header or cookie: {field.name}'
...
async def solve_dependencies(*, request: Union[Request, WebSocket], dependant: Dependant, body: Optional[Union[Dict[str, Any], FormData]]=None, background_tasks: Optional[BackgroundTasks]=None, response: Optional[Response]=None, dependency_overrides_provider: Optional[Any]=None, dependency_cache: Optional[Dict[Tuple[Callable[..., Any], Tuple[str]], Any]]=None) -> Tuple[Dict[str, Any], List[Any], Optional[BackgroundTasks], Response, Dict[Tuple[Callable[..., Any], Tuple[str]], Any]]:
    ...
    for sub_dependant in dependant.dependencies:
        ...
        if dependency_overrides_provider and dependency_overrides_provider.dependency_overrides:
            ...
            use_sub_dependant = get_dependant(path=use_path, call=call, name=sub_dependant.name, security_scopes=sub_dependant.security_scopes)
        solved_result = await solve_dependencies(request=request, dependant=use_sub_dependant, body=body, background_tasks=background_tasks, response=response, dependency_overrides_provider=dependency_overrides_provider, dependency_cache=dependency_cache)
        sub_values, sub_errors, background_tasks, _, sub_dependency_cache = solved_result
    ...
    if dependant.body_params:
        body_values, body_errors = await request_body_to_args(required_params=dependant.body_params, received_body=body)
...
async def request_body_to_args(required_params: List[ModelField], received_body: Optional[Union[Dict[str, Any], FormData]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    values = {}
            ...
            if received_body is not None:
                if is_sequence_field(field) and isinstance(received_body, FormData):
                    ...
                    try:
                        ...
                        continue
            if value is None or (isinstance(field_info, params.Form) and value == '') or (isinstance(field_info, params.Form) and is_sequence_field(field) and (len(value) == 0)):
                if field.required:
                    errors.append(get_missing_field_error(loc))
                else:
                    values[field.name] = deepcopy(field.default)
