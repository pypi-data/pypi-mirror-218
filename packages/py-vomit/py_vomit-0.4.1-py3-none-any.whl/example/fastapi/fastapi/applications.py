...
class FastAPI(Starlette):
    ...
    def build_middleware_stack(self) -> ASGIApp:
        ...
        middleware = [Middleware(ServerErrorMiddleware, handler=error_handler, debug=debug)] + self.user_middleware + [Middleware(ExceptionMiddleware, handlers=exception_handlers, debug=debug), Middleware(AsyncExitStackMiddleware)]
        app = self.router
    ...
    def setup(self) -> None:
        ...
        if self.openapi_url and self.docs_url:
            ...
            async def swagger_ui_html(req: Request) -> HTMLResponse:
                ...
                return get_swagger_ui_html(openapi_url=openapi_url, title=self.title + ' - Swagger UI', oauth2_redirect_url=oauth2_redirect_url, init_oauth=self.swagger_ui_init_oauth, swagger_ui_parameters=self.swagger_ui_parameters)
    ...
    def add_api_websocket_route(self, path: str, endpoint: Callable[..., Any], name: Optional[str]=None, *, dependencies: Optional[Sequence[Depends]]=None) -> None:
    ...
    def exception_handler(self, exc_class_or_status_code: Union[int, Type[Exception]]) -> Callable[[DecoratedCallable], DecoratedCallable]:
