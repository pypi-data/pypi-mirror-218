...
def test_query():
    ...
    assert response.json() == IsDict({'detail': [{'type': 'missing', 'loc': ['query', 'query'], 'msg': 'Field required', 'input': None, 'url': match_pydantic_error_url('missing')}]}) | IsDict({'detail': [{'loc': ['query', 'query'], 'msg': 'field required', 'type': 'value_error.missing'}]})
...
def test_query_int_query_42_5():
    ...
    assert response.status_code == 422
    assert response.json() == IsDict({'detail': [{'type': 'int_parsing', 'loc': ['query', 'query'], 'msg': 'Input should be a valid integer, unable to parse string as an integer', 'input': '42.5', 'url': match_pydantic_error_url('int_parsing')}]}) | IsDict({'detail': [{'loc': ['query', 'query'], 'msg': 'value is not a valid integer', 'type': 'type_error.integer'}]})

def test_query_int_query_baz():
    response = client.get('/query/int?query=baz')
    assert response.status_code == 422
    assert response.json() == IsDict({'detail': [{'type': 'int_parsing', 'loc': ['query', 'query'], 'msg': 'Input should be a valid integer, unable to parse string as an integer', 'input': 'baz', 'url': match_pydantic_error_url('int_parsing')}]}) | IsDict({'detail': [{'loc': ['query', 'query'], 'msg': 'value is not a valid integer', 'type': 'type_error.integer'}]})
...
def test_query_int_optional_query_foo():
    ...
    assert response.status_code == 422
    assert response.json() == IsDict({'detail': [{'type': 'int_parsing', 'loc': ['query', 'query'], 'msg': 'Input should be a valid integer, unable to parse string as an integer', 'input': 'foo', 'url': match_pydantic_error_url('int_parsing')}]}) | IsDict({'detail': [{'loc': ['query', 'query'], 'msg': 'value is not a valid integer', 'type': 'type_error.integer'}]})
