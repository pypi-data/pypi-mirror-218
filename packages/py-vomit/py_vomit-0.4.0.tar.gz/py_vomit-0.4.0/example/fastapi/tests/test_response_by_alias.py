...
class ModelNoAlias(BaseModel):
    name: str
    if PYDANTIC_V2:
        model_config = ConfigDict(json_schema_extra={'description': 'response_model_by_alias=False is basically a quick hack, to support proper OpenAPI use another model with the correct field names'})
    else:

        class Config:
            schema_extra = {'description': 'response_model_by_alias=False is basically a quick hack, to support proper OpenAPI use another model with the correct field names'}
