...
app = FastAPI()
state = {'/async': 'asyncgen not started', '/sync': 'generator not started', '/async_raise': 'asyncgen raise not started', '/sync_raise': 'generator raise not started', 'context_a': 'not started a', 'context_b': 'not started b', 'bg': 'not set', 'sync_bg': 'not set'}
errors = []
...
async def get_sync_context_b_bg(tasks: BackgroundTasks, state: dict=Depends(context_b)):

    async def bg(state: dict):
        state['sync_bg'] = f"sync_bg set - b: {state['context_b']} - a: {state['context_a']}"
    tasks.add_task(bg, state)
    return state
