...
from docs_src.security.tutorial005_an import app, create_access_token, fake_users_db, get_password_hash, verify_password
...
def test_token_no_sub():
    response = client.get('/users/me', headers={'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJkYXRhIjoiZm9vIn0.9ynBhuYb4e6aW3oJr_K_TBgwcMTDpRToQIE25L57rOE'})
    assert response.status_code == 401, response.text
    assert response.json() == {'detail': 'Could not validate credentials'}
    assert response.headers['WWW-Authenticate'] == 'Bearer scope="me"'

def test_token_no_username():
    response = client.get('/users/me', headers={'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJmb28ifQ.NnExK_dlNAYyzACrXtXDrcWOgGY2JuPbI4eDaHdfK5Y'})
    assert response.status_code == 401, response.text
    assert response.json() == {'detail': 'Could not validate credentials'}
    assert response.headers['WWW-Authenticate'] == 'Bearer scope="me"'
...
def test_token_inexistent_user():
    response = client.get('/users/me', headers={'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VybmFtZTpib2IifQ.HcfCW67Uda-0gz54ZWTqmtgJnZeNem0Q757eTa9EZuw'})
    assert response.status_code == 401, response.text
    assert response.json() == {'detail': 'Could not validate credentials'}
    assert response.headers['WWW-Authenticate'] == 'Bearer scope="me"'

def test_token_inactive_user():
    access_token = get_access_token(username='alice', password='secretalice', scope='me')
    response = client.get('/users/me', headers={'Authorization': f'Bearer {access_token}'})
    assert response.status_code == 400, response.text
...
def test_read_system_status():
    access_token = get_access_token()
    response = client.get('/status/', headers={'Authorization': f'Bearer {access_token}'})
    assert response.status_code == 200, response.text
