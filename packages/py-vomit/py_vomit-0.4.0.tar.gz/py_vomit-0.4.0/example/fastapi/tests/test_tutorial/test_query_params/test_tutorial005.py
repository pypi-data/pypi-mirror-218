...
def test_foo_no_needy():
    ...
    assert response.json() == IsDict({'detail': [{'type': 'missing', 'loc': ['query', 'needy'], 'msg': 'Field required', 'input': None, 'url': match_pydantic_error_url('missing')}]}) | IsDict({'detail': [{'loc': ['query', 'needy'], 'msg': 'field required', 'type': 'value_error.missing'}]})
