...
file_required = {'detail': [{'loc': ['body', 'file'], 'msg': 'field required', 'type': 'value_error.missing'}]}

def test_post_form_no_body():
    response = client.post('/files/')
    assert response.status_code == 422, response.text
    assert response.json() == IsDict({'detail': [{'type': 'missing', 'loc': ['body', 'file'], 'msg': 'Field required', 'input': None, 'url': match_pydantic_error_url('missing')}]}) | IsDict({'detail': [{'loc': ['body', 'file'], 'msg': 'field required', 'type': 'value_error.missing'}]})
