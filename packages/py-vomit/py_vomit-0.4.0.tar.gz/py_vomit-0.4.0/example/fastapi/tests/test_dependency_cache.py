...
@app.get('/sub-counter/')
async def get_sub_counter(subcount: int=Depends(super_dep), count: int=Depends(dep_counter)):
    return {'counter': count, 'subcounter': subcount}

@app.get('/sub-counter-no-cache/')
async def get_sub_counter_no_cache(subcount: int=Depends(super_dep), count: int=Depends(dep_counter, use_cache=False)):
    return {'counter': count, 'subcounter': subcount}

@app.get('/scope-counter')
async def get_scope_counter(count: int=Security(dep_counter), scope_count_1: int=Security(dep_counter, scopes=['scope']), scope_count_2: int=Security(dep_counter, scopes=['scope'])):
    return {'counter': count, 'scope_counter_1': scope_count_1, 'scope_counter_2': scope_count_2}
