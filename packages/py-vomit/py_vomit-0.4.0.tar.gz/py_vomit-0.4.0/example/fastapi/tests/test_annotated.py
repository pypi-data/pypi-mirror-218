...
foo_is_missing = {'detail': [IsDict({'loc': ['query', 'foo'], 'msg': 'Field required', 'type': 'missing', 'input': None, 'url': match_pydantic_error_url('missing')}) | IsDict({'loc': ['query', 'foo'], 'msg': 'field required', 'type': 'value_error.missing'})]}
foo_is_short = {'detail': [IsDict({'ctx': {'min_length': 1}, 'loc': ['query', 'foo'], 'msg': 'String should have at least 1 characters', 'type': 'string_too_short', 'input': '', 'url': match_pydantic_error_url('string_too_short')}) | IsDict({'ctx': {'limit_value': 1}, 'loc': ['query', 'foo'], 'msg': 'ensure this value has at least 1 characters', 'type': 'value_error.any_str.min_length'})]}

@pytest.mark.parametrize('path,expected_status,expected_response', [('/default', 200, {'foo': 'foo'}), ('/default?foo=bar', 200, {'foo': 'bar'}), ('/required?foo=bar', 200, {'foo': 'bar'}), ('/required', 422, foo_is_missing), ('/required?foo=', 422, foo_is_short), ('/multiple?foo=bar', 200, {'foo': 'bar'}), ('/multiple', 422, foo_is_missing), ('/multiple?foo=', 422, foo_is_short), ('/unrelated?foo=bar', 200, {'foo': 'bar'}), ('/unrelated', 422, foo_is_missing)])
