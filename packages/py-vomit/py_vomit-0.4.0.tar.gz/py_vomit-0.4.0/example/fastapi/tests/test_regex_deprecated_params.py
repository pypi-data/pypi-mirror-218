...
def get_client():
    ...
    with pytest.warns(DeprecationWarning):
        ...
        @app.get('/items/')
        async def read_items(q: Annotated[str | None, Query(regex='^fixedquery$')]=None):
            if q:
                return f'Hello {q}'
            else:
                return 'Hello World'
...
def test_query_params_str_validations_item_query_nonregexquery():
    ...
    assert response.json() == IsDict({'detail': [{'type': 'string_pattern_mismatch', 'loc': ['query', 'q'], 'msg': "String should match pattern '^fixedquery$'", 'input': 'nonregexquery', 'ctx': {'pattern': '^fixedquery$'}, 'url': match_pydantic_error_url('string_pattern_mismatch')}]}) | IsDict({'detail': [{'ctx': {'pattern': '^fixedquery$'}, 'loc': ['query', 'q'], 'msg': 'string does not match regex "^fixedquery$"', 'type': 'value_error.str.regex'}]})
