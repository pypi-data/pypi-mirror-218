...
class FilesPipeline(ImagesPipelineBase, FilesPipelineBase):
    ...
    2. This pipeline generates paths of format
    ...
    3. This pipeline can use image processing capabilities from Scrapy's ImagePipeline. To enable it, one needs to
    ...
    def __init__(self, store_uri, download_func=None, settings=None):
        ...
        self._process_images = settings.getbool(self._resolve_key('PROCESS_IMAGES', settings), self.PROCESS_IMAGES)
    ...
    def get_media_requests(self, item, info):
        ...
        def attachment_request(attachment: ItemAdapter):
            return Request(
                attachment['url'],
                meta={
                    ...
                    'is_image_request': (self._process_images
                                         and attachment.get('attachment_type') in self._image_attachment_types)
                }
            )
    ...
    def _file_path(self, request, response=None, info=None):
        ...
        if resource_tag := request.meta.get('resource_tag') or self._default_resource_tag:
            path += '/' + resource_tag
        if type_tag := request.meta.get('attachment', {}).get('attachment_type') or self._default_type_tag:
            path += '/' + type_tag
        if self._digest_path_segment_lengths:
