...
class GBRenderer:
    ...
    def scan_assets(self):
        ...
        if exists_on_disk("assets", self._working_dir):
            css_files = get_matches_from_dir("assets", self._working_dir, r"^.+\.css$", relative=True)
            self.custom_stylesheets += css_files
            ico_favicons = get_matches_from_dir("assets", self._working_dir, r"^favicon\.ico$", relative=True)
            self.custom_favicons['ico'] += ico_favicons
            png_regex = r"favicon-([0-9]+)x[0-9]+\.png$"
            png_favicons = get_matches_from_dir("assets", self._working_dir, r"^"+png_regex, relative=True)
    ...
    def contents_page(self, file_list):
        ...
        html = tmpl.render(md_pages=md_pages, root="../")
    ...
    def new_page(self, error_message=None):
        ...
        html = tmpl.render(error_message=error_message, **self._get_variables(link="-/new-page/"))
    ...
    def new_page_form(self, template, error_message=None):
        ...
        html = tmpl.render(template=template, error_message=error_message, **self._get_variables(link="-/new-page/"))
