...
class FLAME(nn.Module):
    ...
    """
    def __init__(self, ckpt_folder_path):
        super(FLAME, self).__init__()
        print("creating the FLAME Decoder")
        with open(os.path.join(ckpt_folder_path, 'generic_model.pkl'), 'rb') as f:
        ...
        lmk_embeddings = np.load(os.path.join(ckpt_folder_path, 'landmark_embedding.npy'), allow_pickle=True, encoding='latin1')
...
class FLAMETex(nn.Module):
    ...
    """
    def __init__(self, dir_folder_path, tex_type='BFM'):
        super(FLAMETex, self).__init__()
        if tex_type == 'BFM':
            mu_key = 'MU'
            pc_key = 'PC'
            n_pc = 199
            tex_path = os.path.join(dir_folder_path, 'FLAME_albedo_from_BFM.npz') 
