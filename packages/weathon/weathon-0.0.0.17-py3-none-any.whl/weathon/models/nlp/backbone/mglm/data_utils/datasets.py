...
"""dataset objects for jsons, csvs, and BERT dataset"""
...
class ConcatDataset(data.Dataset):
    """
    Dataset to concatenate multiple dataset.
    Purpose: useful to assemble different existing dataset, possibly
    large-scale dataset as the concatenation operation is done in an
    on-the-fly manner.
    Arguments:
        datasets (sequence): List of dataset to be concatenated.
    """
...
class SplitDataset(data.Dataset):
    ...
    Purpose: useful to index into existing dataset, possibly
    large-scale dataset as the subindexing operation is done in an
    on-the-fly manner.
    Arguments:
        ds (Dataset or array-like): List of dataset to be subindexed
...
class csv_dataset(data.Dataset):
    """
    Class for loading dataset from csv files.
...
class json_dataset(data.Dataset):
    """
    Class for loading dataset from a json dump.
    Purpose: Useful for loading data for unsupervised modeling or transfer tasks
    Arguments:
