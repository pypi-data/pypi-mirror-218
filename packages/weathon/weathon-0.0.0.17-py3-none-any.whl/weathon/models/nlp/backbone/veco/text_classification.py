...
class VecoForSequenceClassification(TorchModel,
    ...
    Trainer:
        ...
        and evaluated by dataset of languages one by one.
        For example, if the training dataset is xnli (which has sub dataset of multiple languages), then you
        should mix the sub-dataset with the languages you want to train to one training dataset, and evaluate
        the model one sub-dataset by one sub-dataset of different languages.
        This procedure can be done by custom code. If you are using trainers of ModelScope,
        the `VecoTrainer` is suggested to use to train this model. This trainers overrides the basic evaluation
        loop, and will call the evaluation dataset one by one. Besides, this trainers will use the `VecoTaskDataset`
        to mix the input dataset to one, you can check the API Doc for the details.
