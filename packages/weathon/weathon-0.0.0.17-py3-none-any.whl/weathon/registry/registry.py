...
class Registry(object):
    ...
    def register_module(self, group_key: str = default_group, module_name: str = None, module_cls: type = None,
                        force=False):
        """ Register module
...
def build_from_cfg(cfg, registry: Registry, group_key: str = default_group, default_args: dict = None) -> object:
    ...
    if isinstance(obj_type, str):
        ...
        if obj_cls is None:
            raise KeyError(
                f'{obj_type} is not in the {registry.name} registry group {group_key}. Please make sure the correct version of weathon library is used.')
