...
class BasePipeline(ABC):
    ...
    def initiate_single_model(self, model):
        if isinstance(model, str):
            ...
            return BaseModel.from_pretrained(model, device=self.device_name)
        else:
            return model
    ...
    def __init__(self, config_file: str = None, model: Union[InputModel, List[InputModel]] = None,
                 preprocessor: Union[BaseProcessor, List[BaseProcessor]] = None,
                 device: str = 'gpu',
                 auto_collate=True,
                 **kwargs):
        ...
        if config_file is not None:
            ...
            self.cfg = Config.from_file(osp.join(model_dir, ModelFile.CONFIGURATION_YAML))

        if preprocessor is None and not self.has_multiple_models:
            self.preprocessor = BaseProcessor.from_pretrained(model_dir)
        else:
    ...
    def _get_framework(self) -> str:
        ...
        if not all(x == frameworks[0] for x in frameworks):
            logger.warning(f'got multiple models, but they are in different frameworks {frameworks}')
            return None
    ...
    def _check_output(self, input):
        ...
        missing_keys = []
        input = input.keys() if isinstance(input, dict) else input
...
class DistributedPipeline(BasePipeline):
    ...
    def __init__(self,
        ...
        self.cfg = Config.from_file(osp.join(self.model_dir, ModelFile.CONFIGURATION_YAML))
