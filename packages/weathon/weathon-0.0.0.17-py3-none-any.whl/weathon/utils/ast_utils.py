...
def _save_index(index, file_path, file_list=None, with_template=False):
    ...
    index[MD5_KEY], index[FILES_MTIME_KEY] = file_scanner.files_mtime_md5(file_list=file_list)
...
def generate_ast_template(file_path=None, force_rebuild=True):
    ...
    if not os.path.exists(file_path):
        raise Exception('The index file is not create correctly, please double check')
    return index
