...
@dataclass
class BackboneModelOutput(BaseOutput):
...
@dataclass
class Seq2SeqModelOutput(BaseOutput):
    """
...
@dataclass
class FaqQuestionAnsweringOutput(BaseOutput):
...
@dataclass
class FeatureExtractionOutput(BaseOutput):
...
@dataclass
class FillMaskModelOutput(BaseOutput):
...
@dataclass
class InformationExtractionOutput(BaseOutput):
...
@dataclass
class Seq2SeqLMOutput(BaseOutput):
    """
...
@dataclass
class TextClassificationModelOutput(BaseOutput):
    """The output class for text classification models.
...
@dataclass
class TextErrorCorrectionOutput(BaseOutput):
    """The output class for information extraction models.
    """
...
@dataclass
class WordAlignmentOutput(BaseOutput):
...
@dataclass
class TextGenerationModelOutput(BaseOutput):
...
@dataclass
class TokenGeneratorOutput(BaseOutput):
    """
...
@dataclass
class TokenClassificationModelOutput(BaseOutput):
...
@dataclass
class DialogueUserSatisfactionEstimationModelOutput(BaseOutput):
...
@dataclass
class SentencEmbeddingModelOutput(BaseOutput):
    """The output class for text classification models.
...
@dataclass
class TranslationEvaluationOutput(BaseOutput):
