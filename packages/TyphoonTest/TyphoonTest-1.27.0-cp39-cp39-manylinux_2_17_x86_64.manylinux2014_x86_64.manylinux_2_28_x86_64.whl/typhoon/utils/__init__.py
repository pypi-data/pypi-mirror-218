...
def validate_range(item, reference=None, type_cast=None, limit=False):
    ...
    if reference is not None:
        ...
        if type_cast != type(item0):
            try:
                ...
                raise TypeError(
                    "Arguments {} couldn't be converted to type {}".format(
                        item, type_cast
                    )
                )
    ...
    if not range_consistent:
        raise ValueError(
            "Lower value is greater than the upper value for item {}. Range is nonexistant.".format(
                item
            )
        )
    ...
    if reference is not None and limit:
        ...
        if item1 > maximum:
            logger.warning(
                "Desired time [{}] is later to latest signal time. Using latest signal time [{}]".format(
                    item1, maximum
                )
            )
            item1 = maximum
