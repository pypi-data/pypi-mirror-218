...
def abc_to_symmetrical_components(signals: pandas.DataFrame, method="Fortescue"):
    ...
    balanced signal with the zero, positive and negative sequences that sum results
    in the original signals.

    The sequences (zero, positive, and negative) from the symmetrical componmnts are
    represented by complex values.
    ...
    method : string
        Enables the choice of the transformation method. It can be 'Fortescue', or
        'Power invariant'. Another value will raise exception.
    ...
    * ``"negative"`` with the balanced negative phasor components
    ...
    This example of code show how to visualize, using ``matplotlib.pyplot.subplots``, the real and
    imaginary components of Symmetrical Components:
    ...
    """
    return _impl.abc_to_symmetrical_components(signals, method)
...
def symmetrical_components_to_abc(signals: pandas.DataFrame, method="Fortescue"):
    ...
    The Inverse Symmetrical transformation recreates the three-phase signal from the
    three components of balanced signals from the Symetrical transformation.

    Parameters
    ----------
    signals : pandas.DataFrame
        Signals with three columns:
        * First column: The Zero sequence,
        * Second column: The Positive sequence and,
        * Third column: The Negative sequence.
    method : string
        Enables the choice of the inverse transformation method. It can be 'Fortescue', or
        'Power invariant'. Another value will raise exception.
    ...
    The ``output1`` and ``output2`` will both be pandas.DataFrame with three columns each
    corresponding the three-phase coordinates.
