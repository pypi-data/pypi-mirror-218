...
class PreprocPhase(Enum):
    ...
    TRAINING = "train"
    INFERENCE = "infer"
    OTHER = "other" # FIXME REPLACE BY SIGNAL CONDITIONNINF
...
#            self.defaultData = yaml.safe_load(file)
    ...
    def _update_graph_sizes(self, graph):
        ...
        logger.debug("graph.sizeout %d "% (graph.sizeout))
        if (graph.sizeout == 0):
            print("INFO OW Sizeout")
