...
class DashboardWidgetException(Exception):
    def __init__(self, message, error, run_url):
                    ...
                    if i >= 99:
                        ...
                        for task in r.json()['tasks']:
                            if task['state']['result_state'] == 'FAILED':
                                o = requests.get(target_client.url+"/api/2.1/jobs/runs/get-output", params={'run_id': task['run_id']}, headers=target_client.headers).json()
                                if 'error' not in o:
                                    raise Exception(f"can't get task error - see job run for more details - {o}")
                                raise DashboardWidgetException(f"Dashboard import fail - could not run the widget queries: SQL query error.", o['error'], o['metadata']['run_page_url'])
                    else:
