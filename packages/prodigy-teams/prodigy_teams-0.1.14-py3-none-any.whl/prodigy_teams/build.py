...
class Venv:
    ...
    def run_module(
        self, entrypoint: str, *args: str, cwd: ty.Optional[Path] = None
    ) -> subprocess.CompletedProcess:
        return _subprocess_run([self.python, "-m", entrypoint, *args], self.cwd(cwd))
    ...
    def cached(
        ...
        if python_bin is None or not python_bin.exists():
            ...
            ), f"Failed to locate python binary in venv: {cached_venv}"
...
def _subprocess_run(
    args: ty.List[str], cwd: Path, input: ty.Optional[str] = None
) -> subprocess.CompletedProcess:
