...
{
    const __m128i characters = _mm_loadu_si128((__m128i_u *)input_string);
    ...
    const __m128i parsed_integers = integers * 2561;

    char result[16];
    _mm_storeu_si128((__m128i *)result, parsed_integers);

    int year = (result[1] * 100) + result[3];
    int month = result[6];
    int day = result[9];
    int hour = result[12];
    int minute = result[15];
