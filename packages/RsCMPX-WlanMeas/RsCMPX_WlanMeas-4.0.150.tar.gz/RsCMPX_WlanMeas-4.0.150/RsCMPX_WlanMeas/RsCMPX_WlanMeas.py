...
class RsCMPX_WlanMeas:
	"""1158 total commands, 4 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMW500/CMW100/CMW270/CMW280/CMX500/CMP180, SupportedIdnPatterns = CMP, SimulationIdnString = 'Rohde&Schwarz,CMP180,100001,4.0.150.0001'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsCMPX_WlanMeas._driver_options, options, direct_session)
		self._core.driver_version = '4.0.150.0001'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_WlanMeas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_WlanMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_WlanMeas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.150.0001'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_WlanMeas version failed. Current version: '4.0.150.0001', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_WlanMeas') -> None:
	...
	@property
	def wlanMeas(self):
		"""wlanMeas commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_wlanMeas'):
			from .Implementations.WlanMeas import WlanMeasCls
	...
	def clone(self) -> 'RsCMPX_WlanMeas':
		"""Creates a deep copy of the RsCMPX_WlanMeas object. Also copies:
		...
		cloned = RsCMPX_WlanMeas.from_existing_session(self.get_session_handle(), self._options)
