...
SCENARIO_FOLDER = "./tests/e2etest/scenarios"
...
class SparkJobE2EBase(unittest.TestCase):
    ...
    def _get_spark_job(self, test_file, sdk_package_version=None, spark_version="3.2.0") -> Spark:
    ...
    def _test_spark_jobs(self, sdk_version=None, scenarios_to_test: List[str] = None):
        scenario_files = [f for f in os.listdir(SCENARIO_FOLDER) if os.path.isfile(os.path.join(SCENARIO_FOLDER, f))]
        ...
        for f in scenario_files:
            ...
            with self.subTest(test_case_name):
                ...
                spark_job = self._get_spark_job(test_file=f, sdk_package_version=sdk_version)
    ...
    def _wait_until_done(
        ...
        def print_job_status(job: Job):
            if verbose:
                print(f"[{job.display_name}] {time.time() - poll_start_time:.2f}: {job.status}")
