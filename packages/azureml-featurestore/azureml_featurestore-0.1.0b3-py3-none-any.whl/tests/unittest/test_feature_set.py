...
class FeaturesetTest(unittest.TestCase):
    ...
    def test_feature_set(self, mock_copy_rename_and_zip, mock_requests):
        ...
        mock_ml_client._credential = mock_credential
        ...
        assert fset.offline_store.target == "offline_store_url"
    ...
    def test_feature_set_failure(self, mock_copy_rename_and_zip, mock_download_file, mock_ml_client, mock_requests):
        ...
        # fail get token
        mock_credential.get_token.side_effect = Exception("Fail to get token")
        with self.assertRaises(Exception) as ex:
            FeatureSet._from_rest_object(rest_obj, mock_ml_client)
        assert "Fail to get token" in str(ex.exception)
        mock_credential.get_token.side_effect = None
