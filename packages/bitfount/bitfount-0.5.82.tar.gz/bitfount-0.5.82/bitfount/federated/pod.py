...
class Pod:
    ...
    def _load_view_schema_if_necessary(
        ...
        # been requested
        if update_schema is True or schema is None:
            source_dataset = self.base_datasources[ds.datasource.source_dataset]
    ...
    def _get_target_datasource_schema(
        ...
        if isinstance(target_datasource_container.datasource, ViewDatasourceConfig):
            ...
            try:
                ...
                elif hasattr(target_datasource_container.datasource, "source_dataset"):
                    # The API way of specifying the source dataset
                    base_datasource = self.datasources[
                        target_datasource_container.datasource.source_dataset
                    ].datasource
                else:
                    raise PodViewError(
        ...
        if datasources:
            ...
            for ds in datasources:
                if isinstance(ds.datasource, BaseSource):
                    ...
                    if (
                        hasattr(ds.datasource, "source_dataset")
                        and ds.datasource.source_dataset in base_datasources.keys()
                    ):
                        source_ds = ds.datasource.source_dataset
                    elif (
                        hasattr(ds.data_config, "datasource_args")
                        and "source_dataset" in ds.data_config.datasource_args  # type: ignore[union-attr] # Reason: hasattr check # noqa: B950
                        and ds.data_config.datasource_args["source_dataset"]  # type: ignore[union-attr] # Reason: hasattr check # noqa: B950
                        in base_datasources.keys()
                    ):
                        source_ds = ds.data_config.datasource_args["source_dataset"]  # type: ignore[union-attr] # Reason: hasattr check # noqa: B950
