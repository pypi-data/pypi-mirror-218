...
class SQLDataView(DataView):
    ...
    def __init__(
        self, datasource: BaseSource, query: str, pod_name: str, source_dataset: str
    ) -> None:
        super().__init__(datasource)
        self.query = query
...
class DropColViewConfig(ViewDatasourceConfig[BaseSource]):
    """Config class for _DropColsDropColView."""
    ...
    def generate_schema(
        ...
        view = self.build(view_datasource)
...
class SQLViewConfig(ViewDatasourceConfig[BaseSource]):
    ...
    def generate_schema(
        ...
        """Schema generation for _SQLDataViewConfig."""
