...
class Mock_GPS_Config:
    """gps config"""

    nmea_out: Optional[str] = None  # serial port or None for stdout
    baudrate: int = 115200
    n_valid: int = 100
    n_invalid: int = 0
...
class Mock_GPS:
    ...
    def __init__(
        ...
        # fix quality list
        self._fix_qual = [4] * self.config.n_valid + [0] * self.config.n_invalid
    ...
    async def main(self, freq: float = 1.0):
        ...
        if self.config.nmea_out is None:
            writer = None
        else:
            _, writer = await serial_asyncio.open_serial_connection(
                url=self.config.nmea_out, baudrate=self.config.baudrate
            )
