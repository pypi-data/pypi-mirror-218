...
class downLoadBam:
    def __init__(self, results, rawdirs: str, threads: int) -> None:
        self.results = results
        self.rawdirs = rawdirs
        self.threads = threads
        self.bamfolder = f"{rawdirs}/bam"
        self.makbamdirs()
        self.addBamName()
        self.getmd5Map()
    ...
    def isDownloadAll(self):
        exitCount = sum(1 for study in self.results if os.path.exists(
            f"{self.bamfolder}/{study['bamName']}.bam"))
    ...
    def DLBAM(self) -> bool:
        ...
        for study in self.results:
            ...
            bam_ftp = f"https://sra-pub-src-1.s3.amazonaws.com/{run_accession}/{bamName}.1"
            print(bam_ftp)
