...
class LoginService:
    ...
    def template_send(type, template_type, replacements=None, touser=None):
        ...
        file_json = {"subscribe": {"register": {"template_id": "g2tS70DLZz-t6dP34pbgkAsHrYfNIEwuCQeKA57Yd9w",
                                                "data": {"character_string4": {"value": "{{}}"},
                                                         "character_string1": {"value": "{{}}"},
                                                         "character_string6": {"value": "{{}}"},
                                                         "date3": {"value": "{{}}"}}}}, "applet": {
            "register": {"template_id": "rJYVHus-L5L_9EV1olbFr6lIvikVt1TrOgNcMLcGNgA",
                         "data": {"thing6": {"value": "{{}}"}, "character_string4": {"value": "{{}}"},
                                  "time1": {"value": "{{}}"}}}}}
        # 测试数据
        data = file_json[type][template_type]
        # 调用函数
        new_data = replace_placeholders(data, replacements)
        new_data['touser'] = touser
        if type == "subscribe":
            wechat_user_info = subscribe_message(new_data)
        elif type == "applet":
            wechat_user_info = applet_subscribe_message(new_data)
        print("template_send", wechat_user_info)
    ...
    def logical_processing(current_user, phone, sso_serve_id, sso_app_id, openid, unionid, platform_id, platform_code,
        ...
        print(">>>>>>logical_processing", current_user)
        """
          登录逻辑处理
