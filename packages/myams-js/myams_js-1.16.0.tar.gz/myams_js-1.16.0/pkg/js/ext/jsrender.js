    ...
    */
    ...
    function compileTag(name, tagDef, parentTmpl) {
      ...
      if (baseTag = tagDef.baseTag) {
        ...
        baseTag = typeof baseTag === STRING ? parentTmpl && parentTmpl.tags[baseTag] || $tags[baseTag] : baseTag;
        if (!baseTag) {
    ...
    function compileViewModel(name, type) {
      ...
      function map(data) {
        data = typeof data === STRING ? JSON.parse(data) // Accept JSON string
        ...
        if ($isArray(model)) {
          ...
          for (; k < l; k++) {
            ...
            for (j = 0; j < m && !found; j++) {
              ...
              mod = model[j];
              if (id) {
                assigned[j] = found = typeof id === STRING ? ob[id] && (getterNames[id] ? mod[id]() : mod[id]) === ob[id] : id(mod, ob);
              }
            }
            if (found) {
              mod.merge(ob);
    ...
    function getTargetSorted(value, tagCtx) {
      ...
      if (directSort || sort && typeof sort === STRING) {
        ...
        mapped = value.map(function (item, i) {
          ...
          return {
            i: i,
            v: typeof item === STRING ? item.toLowerCase() : item
          };
        });
        // Sort mapped array
    ...
    if (!(jsr || $ && $.render)) {
      ...
      */
      ($viewsSettings.debugMode = function (debugMode) {
        ...
        $subSettings.debugMode = debugMode, $subSettings.onError = typeof debugMode === STRING ? function () {
          return debugMode;
