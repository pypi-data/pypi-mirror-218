...
def melon_scanner(host, delay=15):
    ...
    os.system(f"sqlmap --url={host} --random-agent --level=5 --risk=3 --all --delay={delay} --flush-session --batch")
    ...
    # test for command injection in alt port
    ...
    for port in port_list:
        for mal in mal_commands:
            ...
            try:
                my_socket.connect((new_host, int(port)))
                print(CYAN + f"checking for command injection in port {port}")
    ...
    # test for python injection in alt port
    ...
    for port in port_list:
        for mal in mal_python:
            ...
            try:
                my_socket.connect((new_host, int(port)))
                print(CYAN + f"checking for python injection in port {port}")
                my_socket.sendall(mal)
                start = time.time()
                simple_response = my_socket.recv(65536)
                my_socket.close()
    ...
    # test for sql injectionn in alt port
    mal_sql = ["'", '"', "*", ";", "`", "')", '")', "*)", ";)", "`)", "'))", '"))', "*))", ";))", "`))", "')))", '")))', "*)))", ";)))", "`)))"]
    port_list = random.sample(port_list[:], len(port_list[:]))
    for port in port_list:
        for mal in mal_sql:
            ...
            try:
                my_socket.connect((new_host, int(port)))
                print(CYAN + f"checking for sql injection in port {port}")
                my_socket.sendall(mal)
                start = time.time()
                simple_response = my_socket.recv(65536)
                my_socket.close()
    ...
    for port in port_list:
        for mal in mal_xss:
            ...
            try:
                my_socket.connect((new_host, int(port)))
                print(CYAN + f"checking for xss in port {port}")
                my_socket.sendall(mal)
                start = time.time()
                simple_response = my_socket.recv(65536)
                my_socket.close()
