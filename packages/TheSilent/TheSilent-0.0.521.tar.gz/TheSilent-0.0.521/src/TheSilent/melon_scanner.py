...
def melon_scanner(host, delay=15):
    ...
    try:
        ...
        my_request.add_header("Referer","https://duckduckgo.com/")
        my_request.add_header("UPGRADE-INSECURE-REQUESTS","1")
        response_online = urllib.request.urlopen(my_request, context=verify)
        time.sleep(delay)
    ...
    for port in port_list:
        for mal in mal_commands:
            ...
            try:
                ...
                my_socket.sendall(mal)
                start = time.time()
                simple_response = my_socket.recv(65536)
                my_socket.close()
                end = time.time()
