...
def melon_scanner(host, delay=1):
    ...
    try:
        ...
        response_online = urllib.request.urlopen(my_request, context=verify, timeout=15)
        time.sleep(delay)

        my_request = urllib.request.Request(host, method="GET")
        my_request.add_header("User-Agent",return_user_agent())
        false_positive_check = str(urllib.request.urlopen(my_request, context=verify, timeout=15).read())
        time.sleep(delay)
        ...
        print(RED + "")
        clear()
        return "We are being blocked!"

    clear()
    print(CYAN + "running nmap")
    clear()
    print(CYAN + "attempting to get banners")
    ...
    # test for command injection on alt port
    mal_commands = ["echo TheSilent", "ping -c 60 127.0.0.1"]
    port_list = random.sample(port_list[:], len(port_list[:]))
    for port in port_list:
        print(CYAN + f"checking for command injection on port {port}")
    ...
    # test for python injection on alt port
    ...
    port_list = random.sample(port_list[:], len(port_list[:]))
    for port in port_list:
        print(CYAN + f"checking for python injection on port {port}")
    ...
    # test for sql injectionn on alt port
    mal_sql = ["'", '"', "*", ";", "`", "')", '")', "*)", ";)", "`)", "'))", '"))', "*))", ";))", "`))", "')))", '")))', "*)))", ";)))", "`)))"]
    port_list = random.sample(port_list[:], len(port_list[:]))
    for port in port_list:
        print(CYAN + f"checking for sql injection on port {port}")
        for mal in mal_sql:
            time.sleep(delay)
            my_socket = socket.socket()
            my_socket.settimeout(15)
    ...
    port_list = random.sample(port_list[:], len(port_list[:]))
    for port in port_list:
        print(CYAN + f"checking for xss on port {port}")
        for mal in mal_xss:
            time.sleep(delay)
            my_socket = socket.socket()
            my_socket.settimeout(15)
    ...
    clear()
    print(CYAN + "running sqlmap")
    os.system(f"sqlmap --url={host} --headers='User-Agent: {return_user_agent()}\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8\nAccept-Encoding: deflate\nAccept-Language: en-US,en;q=0.5\nReferer: https://www.google.com/\nUPGRADE-INSECURE-REQUESTS: 1' --level=5 --risk=3 --all --delay={delay} --flush-session --batch")
    clear()
