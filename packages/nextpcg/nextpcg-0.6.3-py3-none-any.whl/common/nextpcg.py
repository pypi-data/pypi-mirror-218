                        ...
                        if 'Tags' in param_val.keys() and 'filechooser_mode' in param_val['Tags']:
                            if 'read' in param_val['Tags']['filechooser_mode']:
                                if label_file_name in param_val.keys():
                                    ...
                                    param_val[label_file_data] = request_upload(send_path, param_val[
                                        label_file_name], upload_file_headers)
                                elif label_folder_name in param_val.keys():
                                    print("Upload Param Folder: " + param_val[label_folder_name])
                                    param_val[label_folder_data] = request_upload_folder(send_path,
                                                                                         param_val[label_folder_name],
                                                                                         upload_file_headers)
                                else:
                                    print('Notice add {0},{1} tag to input file parameters.'.format(label_file_name,
                                                                                                    label_folder_name))
