...
def tagCorpus():
    ...
    from model.syntax import compiler
    import MeCab
    posTagger = MeCab.Tagger()
    
    lines = []
    
    corpus = {}
    for infix, *_ in MySQL.instance.select('select text from tbl_claim_parser_jp limit 5000 offset 15000'):
        
        tree = compiler.TreeNode.compile(infix)
        assert str(tree) == infix
        
        assert isinstance(tree, compiler.TreeNodeArray)
        
        _, *claims = tree.array
        claims = [compiler.original_text(claim) for claim in claims]
        ...
        for text in claims:
            for line in split_into_sentences(text):
                ...
                arr = [*split_sentence_limited(line, 128)]
                
                for text in arr:
                    result = posTagger.parse(text).split('\n')
                    *words, EOS, _ = (lexeme.split('\t')[0] for lexeme in result)                    
                    
#                     print(words)
                    text_ = convertToOriginal(words)                    
                    if text_ != text:
                        corpus[text_] = convertFromSegmentation(words)
                    else:
                        corpus[text] = convertFromSegmentation(words)
                        
                    
                assert ''.join(arr) == line, 'result is not the same as original text!'
                lines.extend(arr)
    ...
    data = []
    for text, segment in corpus.items():
        data.append((text, segment, 1))
        
    MySQL.instance.load_data('tbl_segment_jp', data)
#             preamble = self.split(preamble)
#             claims = self.split(claims)
