...
from .exceptions import AsyncFunctionNotSupported, DuplicateFuncSpec, InvalidFuncSpecSchema
...
class Copilot:
    ...
    async def arun(self, ipt: str):
        ...
        if function_call := resp_msg.get("function_call"):
            ...
            arguments = json.loads(function_call["arguments"])
            func_spec = self._func_mapping.get(func_name)
            if not asyncio.iscoroutinefunction(func_spec.handler):
                raise AsyncFunctionNotSupported("Try `Copilot.run()`")
            else:
                result = func_spec.handler(**arguments, headers=self.headers)
