...
def _decode_bluescreen(image: Image[np.uint8]) -> Image[np.uint8]:
    ...
    return Image(raw=image.raw, data=data, dtype=np.uint8, channels="rgba")
...
def decode_bgra_uint8_hdr(
    ...
    return Image(raw=encoded_image, data=Deferred(thunk), dtype=np.float32, channels="rgb")
...
def decode_rgba_uint16_le_hdr(encoded_image: bytes, width: int, height: int) -> Image[np.float32]:
    ...
    return Image(raw=encoded_image, data=Deferred(thunk), dtype=np.float32, channels="rgba")
