...
class ChannelSeparator:
    def __call__(self, image: Image[_I_contra]) -> Iterable[Image[_I_contra]]:
        ...
        if data_rgb is not None:
            yield Image(raw=None, data=data_rgb, dtype=image.dtype, channels="rgb")

        if data_l is not None:
            yield Image(raw=None, data=data_l, dtype=image.dtype, channels="l")

        if data_a is not None:
            yield Image(raw=None, data=data_a, dtype=image.dtype, channels="a")
