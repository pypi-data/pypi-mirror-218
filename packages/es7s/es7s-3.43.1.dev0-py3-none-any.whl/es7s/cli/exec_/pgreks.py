...
def compose_clear_line_fill_bg(basis: pt.SequenceSGR, line: int = None) -> str:
    """
    ...
    """
    if line is not None:
        curset = pt.make_set_cursor(line, 1)
    else:
        curset = pt.make_set_cursor_column(1)
    return f'{curset}{basis}{pt.make_clear_line()}'

pt.compose_clear_line_fill_bg = compose_clear_line_fill_bg
...
class ABFSAutomaton(metaclass=ABCMeta):
    ...
    def _show_help(self):
        self._state = State.DISPLAY_HELP
