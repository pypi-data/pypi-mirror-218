...
class IoInjector:
    ...
    ------------------------------------------------------------------------------------------------
    It turned out that intercepting IS actually NEEDED, for example, when IO proxy is not utilized
    ...
    def run_expect_ok(self, args: str | t.Sequence[str] | None):
        ...
        assert (not stderr) and (exit_code == 0), "Non-empty stderr or exit code > 0"

        return exit_code, stdout, stderr

    def run(self, args: str | t.Sequence[str] | None):
        try:
            result = self._cli_runner.invoke(self, args, catch_exceptions=False)
...
class TestCommand:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param("exec describe es7s", "es7s.cli.__main__", marks=[pytest.mark.skip()]),
            pytest.param("exec get-socket test-topic", "test-topic", marks=[pytest.mark.skip()]),
            ["exec hilight-num --demo", "http://localhost:8087"],
            ["exec l4 /", re.compile("([r-][w-][x-]){3}")],
            ["exec ls /", re.compile("([r-][w-][x-]){3}")],
            pytest.param("exec notify test", "", marks=[pytest.mark.skip()]),
            ["exec nalog s", "Last record"],
            pytest.param("exec esqdb send /dev/zero /dev/zero", re.compile("Dusk|Now"), marks=[pytest.mark.skip()]),
            ["exec watson -s0x2588 -n1", "█"],
            ["exec wrap --demo", "text text"],
            ["help commands", "help commands"],
            ["print block", "⭐"],
            ["print colors legacy", "16 colors fg"],
            ["print colors hsv", "300°"],
            ["print demo-progress-bar .", "Preparing"],
            ["print env", "ES7S_BIN_PATH"],
            ["print env-hosts .", "Searching in"],
            ["print gradient --demo", "solid-cherry"],
            ["print keys tmux --details", "send-keys"],
            ["print keys x11 --details", "terminator-singleton"],
            ["print printscr", "UBUNTU PRINT SCREEN MODIFIERS"],
            ["print regex", "PYTHON REGULAR EXPRESSIONS"],
            ["print rulers", "╷ˌˌˌˌ╷ˌˌ80"],
            ["print shell-param-exp", "Bash parameter"],
            ["print sun", re.compile("Dusk|Now")],
