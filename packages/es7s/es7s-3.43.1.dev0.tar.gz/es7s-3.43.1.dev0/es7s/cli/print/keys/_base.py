...
from es7s.shared.typext import other
...
class StyleRegistry:
    ...
    RAW_SEQ_STYLE = Style(fg=cv.YELLOW)
...
class Sequence(Fragment):
    ...
    def __post_init__(self):
        if fakes := other(KeyCombo, self.combos):
...
class KeyComboFactory:
    ...
    def from_tmux(self, raw_combo: str) -> KeyCombo:
        ...
        if raw_combo.startswith("\\"):
            raw_combo = raw_combo.lstrip("\\")
        mods = []
        while mod_match := self.MODIFIER_REGEXP.match(raw_combo):
...
def char_range(c1, c2):
    ...
    yield from map(chr, range(ord(c1), ord(c2) + 1))
...
class BindKeyTable:
    def __init__(self, name: str, invoker: KeyCombo = None, label="KEY TABLE"):
        self.name = name
...
class BindFactory:
    ...
    def make(
        ...
        *combos: KeyCombo,
        ...
        return Bind(parent_table, Sequence([*combos]), desc, domain, command)
    ...
    def from_tmux(self, raw_bind: str, key_table: BindKeyTable) -> Bind:
        ...
        while len(splitted) > 0:
            desc += self._apply_inline_attribute_format(splitted.pop(0))
            if len(splitted):
                desc += " "

        return self.make(key_table, domain, desc, *combos)
...
class PopupFormatter:
    ...
    def __init__(self, bind_collector: IBindCollector):
        self._bind_collector = bind_collector
    ...
    def print_binds(self):
        ...
        if (termw := get_terminal_width()) <= self.DOMAIN_DISPLAY_TERMW_THRESHOLD:
            ...
            for key_table in self._bind_collector.get_key_tables():
        ...
        for key_table in self._bind_collector.get_key_tables():
            ...
            for bind in key_table.binds:
                ...
                if self._bind_collector._details:
    ...
    def print_cheatsheet(self):
        ...
        pref = self._bind_collector.get_invoker("prefix")
        pref_compat = self._bind_collector.get_invoker("compat")

        kcfact = KeyComboFactory(self._bind_collector.is_invoker)

        kc_any = kcfact.make(key_any)
        kc_any_modc = kcfact.make(key_any, mod_c)
        kc_np = kcfact.make(key_np)
        ...
        self.print_padded("")
        self.print_padded(
            Fragment("ARROW KEYS BINDINGS", StyleRegistry.TABLE_HEADER_STYLE).render()
        )
        self.print_padded("")
        # fmt: off
        self.print_padded(
            ...
            '╚════════════════════════════╧═══════════════╧═══════════════╝', pad_mult=2)
        self.print_padded(get_stdout().render('  The reason for compatibility mode bindings is existence of\n' +
    ...
    def print_padded(cls, s, pad_mult=1):
        get_stdout().echo(
            "\n".join((str(pad_mult * cls.PAGE_PAD) + l) for l in s.splitlines())
        )  # noqa
