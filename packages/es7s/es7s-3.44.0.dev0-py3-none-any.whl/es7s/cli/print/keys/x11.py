...
class invoker:
    ...
    def __init__(self, details: bool, **kwargs):
        self._collector = X11BindCollector({}, details)
        self.run()

    def run(self, **kwargs):
        xbk_data = self.get_raw_binds()
        self._collector.collect(xbk_data)

        formatter = PopupFormatter(self._collector)
        formatter.print_legend()
        formatter.print_binds()

    def get_raw_binds(self) -> str:
