...
try:
	...
	logo=os.path.join("ShockFinder","Addon","GUI","XUIlib","image","A.png")
except:
	from XUIlib.imgtool import add_image
	from XUIlib.page import *
	logo=os.path.join("XUIlib","image","A.png")
from tkinter import *
...
class page(page):
	...
	def GS_Loader(self):
		...
		def fun_Loader_ENG(event):
			...
			self.tkobj.io_recv("Updated Loader-Engine to",self.pars["IO"]["Engine"])
		...
		def fun_Loader_InterVal(event):
			self.pars["Loader"]["Engine"]=int(Loader_InterVal.get())
