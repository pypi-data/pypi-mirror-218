...
try:
	...
	from ShockFinder.Addon.GUI.XUIlib.SystemInfo import SystemInfo
	from ShockFinder.Addon.GUI.XUIlib.Config import Pages
	pages_dir=os.path.join("ShockFinder","Addon","GUI","XUIlib","pages")
	config_dir=os.path.join("ShockFinder","Addon","GUI","XUIlib","Config.py")
	info_image=os.path.join("ShockFinder","Addon","GUI","XUIlib","image","XUI.png")
except:
	from XUIlib.imgtool import add_image
	from XUIlib.SystemInfo import SystemInfo
	from XUIlib.Config import Pages
	pages_dir=os.path.join("XUIlib","pages")
	config_dir=os.path.join("XUIlib","Config.py")
	info_image=os.path.join("XUIlib","image","XUI.png")
...
def CollectPages():
	...
	for i in tuple(set([FileNamePreProcess(i)[0] for i in GetFiles(pages_dir,"py")])-set(pages_except)):
		strc+="try:\n\ttry:\n\t\timport ShockFinder.Addon.GUI.XUIlib.pages."+i+" as "+i+"\n\texcept:\n\t\timport XUIlib.pages."+i+" as "+i+"\n\t"+"Pages[\""+i+"\"]="+i+"\nexcept Exception as err:\n\tprint(\"Page: "+i+" import failure:\",err)\n\n"
	strc+="if __name__==\"__main__\":\n\tprint(\"Testing Model:\",__file__)\n\tprint(\"Pages:\")\n\tfor i in Pages.keys():\n\t\tprint(i,\":\",Pages[i])\n\t\tprint(\"\\t\",Pages[i].page)"
	filename=config_dir
