...
try:
	...
	def Load(filename,MPE=None,print=print,**vargs):
		...
		hdffile.close()
		if MPE==None:
			return DataBase.DataBaseConnect(*[load_fun(dbinfo["projectname"],i,dbinfo,print) for i in range(dbinfo["pnum"])])
		#MPE mode
		mpe=MPE.build(load_fun,**vargs)
		return DataBase.DataBaseConnect(*mpe.fun(dbinfo["projectname"],mpe.Array(range(dbinfo["pnum"])),dbinfo))
except:
