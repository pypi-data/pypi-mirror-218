...
def get(Dataobj,args={},vargs={}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	quantities={"Entropy":Dataobj.quantities["prs"]/Dataobj.quantities["rho"]**Dataobj.quantities["gamma"]}
...
def get_AvgTh(Dataobj,args={},vargs={"Mean_axis":(1,)}):
	try:
		if AvgTh_cal:
			...
			quantities={"AvgTh_"+meanstr+"Entropy":Mean.Mean(Dataobj,"prs",vargs)/Mean.Mean(Dataobj,"rho",vargs)**Dataobj.quantities["gamma"]} #update here
			Dataobj.quantities.update(quantities)
		else:
