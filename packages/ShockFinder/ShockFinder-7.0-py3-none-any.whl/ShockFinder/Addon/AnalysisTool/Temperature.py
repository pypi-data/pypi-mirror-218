...
def get(Dataobj,args={},vargs={}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	quantities={"Temperature":Dataobj.quantities["prs"]/Dataobj.quantities["rho"]*Dataobj.quantities["c"]**2*Dataobj.quantities["mu"]*Dataobj.quantities["mp"]/Dataobj.quantities["k"]}
...
def result(quantity_name=None,anafname=None):
	return ("Temperature","AvgTh_Temperature")
...
def get_AvgTh(Dataobj,args={},vargs={"Mean_axis":(1,)}):
	try:
		if AvgTh_cal:
			...
			quantities={"AvgTh_"+meanstr+"Temperature":Mean.Mean(Dataobj,"prs",vargs)/Mean.Mean(Dataobj,"rho",vargs)*Dataobj.quantities["c"]**2*Dataobj.quantities["mu"]*Dataobj.quantities["mp"]/Dataobj.quantities["k"]} #update here
			Dataobj.quantities.update(quantities)
		else:
			print("Warning: AvgTh mode is not opened:", "Temperature") #update here
	except:
...
if __name__=="__main__":
	...
	print("Testing Result:", TestData.quantities["Temperature"])
	if AvgTh_cal:
		TestData=get_AvgTh(TestData)
		print("Testing Result:", TestData.quantities["AvgTh_Temperature"])