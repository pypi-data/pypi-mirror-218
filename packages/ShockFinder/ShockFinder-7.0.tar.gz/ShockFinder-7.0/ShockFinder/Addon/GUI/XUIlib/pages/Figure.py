...
try:
	...
	logo=os.path.join("XUIlib","image","F.png")
from tkinter import *
...
class page(page):
	...
	def LD(self):
		...
		def load(event):
			if entry.get()!="":
				if self.pageargs["Infobj"].load(entry.get()):
					...
					index.config(values=self.usefultindex)
		...
		def view(event):
			self.tkobj.io_recv("Collecting data in",index.get())
		...
		index=ttk.Combobox(box,width=5,height=10)
	...
	def LD_GS(self):	
		...
		MPE_PNUM.insert(0,self.pageargs["Infobj"].LMPEINFO["pnum"])
		MPE_PNUM.pack(side="left")
		Label(box,text="| ").pack(side="left")
		def fun_MPE_PNUM(event):
			if int(MPE_PNUM.get())>cpu_count()-self.pageargs["Infobj"].LMPEINFO["cpu_leave"]:
				MPE_PNUM.delete(0,"end")
				MPE_PNUM.insert(0,cpu_count()-self.pageargs["Infobj"].LMPEINFO["cpu_leave"])
	...
	def UnitSet(self):
		...
		button.pack(side="left")
		button.bind("<ButtonRelease>",fun_r)
		...
		button=Button(box,text="Set",width=5)
		button.pack(side="left")
		button.bind("<ButtonRelease>",fun_t)
		Label(self.add_row(bigbox,bx=150),text="-"*500).place(x=0,y=0,anchor="nw") #next
		box=self.add_row(bigbox,bx=150)
		...
		box=self.add_row(bigbox,bx=320)
...
#			if True:
	...
	def Surface3d(self):
		...
		Label(self.add_row(bigbox),text="-"*500).place(x=0,y=0,anchor="nw") #next
		box=self.add_row(bigbox)
		self.P3DSur_Qt=ttk.Combobox(box,width=17,height=10,values=self.avqt[0])
