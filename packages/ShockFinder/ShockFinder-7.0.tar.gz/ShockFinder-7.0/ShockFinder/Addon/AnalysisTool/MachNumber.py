...
def get(Dataobj,args={},vargs={}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	quantities={"MachNumber":(Dataobj.quantities["vx1"]**2+Dataobj.quantities["vx2"]**2+Dataobj.quantities["vx3"]**2)**0.5/Dataobj.quantities["SoundSpeed"]}
		...
		if AvgTh_cal:
			...
			for i in newneed:
				if i not in Dataobj.quantities.keys() and i not in vargs.keys():
					...
					return Dataobj
			meanstr=""
			meanaxis=get_par(Dataobj,vargs,"Mean_axis",Dataobj.quantities["AvgTh_SoundSpeed"].ndim-1)
			...
			quantities={"AvgTh_"+meanstr+"MachNumber":(Mean.Mean(Dataobj,"vx1",vargs)**2+Mean.Mean(Dataobj,"vx2",vargs)**2+Mean.Mean(Dataobj,"vx3",vargs)**2)**0.5/Dataobj.quantities["AvgTh_SoundSpeed"]} #update here
			Dataobj.quantities.update(quantities)
		else:
