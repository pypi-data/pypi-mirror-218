...
def get(Dataobj,args={},vargs={}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	quantities={"Bremsstrahlung":Dataobj.quantities["rho"]**2*Dataobj.quantities["Temperature"]**0.5*2*np.pi*1.4e-27/Dataobj.quantities["mp"]**2}
	...
	if Dataobj.quantities["Bremsstrahlung"].ndim==3:#3d
		quantities={"Total_Bremsstrahlung":intfun(Dataobj.quantities["Bremsstrahlung"],Dataobj.grid["x1"],Dataobj.grid["x2"],Dataobj.grid["x3"])}
	elif Dataobj.quantities["Bremsstrahlung"].ndim==2:#2d
		quantities={"Total_Bremsstrahlung":intfun(Dataobj.quantities["Bremsstrahlung"],Dataobj.grid["x1"],Dataobj.grid["x2"])}
	elif Dataobj.quantities["Bremsstrahlung"].ndim==1:#1d
		quantities={"Total_Bremsstrahlung":intfun(Dataobj.quantities["Bremsstrahlung"],Dataobj.grid["x1"])}
	Dataobj.quantities.update(quantities)
	return Dataobj
...
if __name__=="__main__":
	...
	print(Differential.integrate_sph_body(TestData.quantities["Bremsstrahlung"],TestData.grid["x1"],TestData.grid["x2"]))