...
class ShockFinder:
	...
	def __init__(self,config_file=None):
		if config_file!=None: #MPA MODE
			...
			self.LMPEINFO["pnum"]=cpu_count()-self.LMPEINFO["cpu_leave"] if cpu_count()>self.LMPEINFO["cpu_leave"] else 1 
			if self.GUI!=None:
				self.GUIobj=self.GUI.show(**{"Infobj":self,"windows_title":"ShockFinder 7.0"})
	...
	def load(self,filename):
		try:
			if self.IO!=None:
				self.database=self.IO.Load(filename,self.MultiprocessEngine,**self.LMPEINFO)
				return True
		...
		for i in command.keys():
			...
			if i=="MultiprocessEngine":
				...
				for ii in command.keys():
					if ii not in ("MultiprocessEngine","IO","Testdata","Update","Test"):
						qtname+=ii
				...
				analyze_commands[i]=Loader[values["Engine"]]
				allfiles=LoaderLib["FPP"].GetFiles(get_par(values,"FileDir",os.getcwd()),get_par(values,"FileType","dbl"))
				if self.MPE!=None:
					filesname=[]
					TrueIndex=args_cut(range(len(0,allfiles,get_par(values,"Interval",1))),self.MPE.pnum)
					for i in range(self.MPE.pnum):
						filesname.append({})
						for j in TrueIndex:
							filesname[i][j]=allfiles[j]
				else:
				...
				if self.MPE!=None:
					...
					for i in range(self.MPE.pnum):
						filesname.append({})
						for j in TrueIndex[i]:
							filesname[i][j]=j
					#	num.append(len(self.TrueIndex[i]))
				else:
					#num=[values["num"]]
