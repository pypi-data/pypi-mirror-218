...
class ComposedTransformation(Transformation):
    ...
    def compose(self):
        ...
        cur_affine = None
        for t in self.trans_list:
            if not isinstance(t, SpatialTransformation): out_list.append(t)
            aff = t.affine(n_dim)
...
class Rescaling(SpatialTransformation):
    ...
    def affine(self, n_dim=None):
        ...
        return bt.diag(bt.cat(scale, bt.ones([scale.n_batch], 1), 1))
...
class Translation(SpatialTransformation):
    ...
    def affine(self, n_dim=None):
        ...
        return bt.cat(bt.cat(bt.eye(self.n_dim).multiply(n_batch, []), self.translation.with_channeldim(None).unsqueeze(-1), -1), bt.one_hot(-1, n_dim+1).unsqueeze(0).multiply(n_batch, []), 1)
