...
class ImageFolderDataset(Dataset):
    def __init__(self, root, transform=None,color='RGB',batch_mode=False):
        self.root = root
                ...
                for img_name in os.listdir(class_folder):
                    ...
                    if self.batch_mode:
                        ...
                        if self.color == "grayscale":
                            ...
                            img = torch.from_numpy(cv2.imread(img_path,cv2.IMREAD_GRAYSCALE)).unsqueeze(0).to(torch.float32)

                        else:
                            # img = torch.from_numpy(np.array(Image.open(img_path).convert('RGB'))).permute(2,0,1).to(torch.float32)
                            img = torch.from_numpy(cv2.imread(img_path)).permute(2,0,1).to(torch.float32)
    ...
    def __getitem__(self, idx):
        if self.color == "grayscale":
            if self.batch_mode:
                ...
                img = torch.from_numpy(cv2.imread(self.image_paths[idx])).unsqueeze(0).to(torch.float32)
            else:
            ...
            if self.batch_mode:
                ...
                img = torch.from_numpy(cv2.imread(self.image_paths[idx])).permute(2,0,1).to(torch.float32)
