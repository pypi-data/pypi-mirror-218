...
class RsCMPX_WcdmaMeas:
	...
	_driver_options = "SupportedInstrModels = CMW/CMP/CMX, SupportedIdnPatterns = CMW/CMP/CMX, SimulationIdnString = 'Rohde&Schwarz,CMW500,100001,4.0.185.0001'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.185.0001'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.185.0001'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_WcdmaMeas version failed. Current version: '4.0.185.0001', minimum required version: '{min_version}'")
