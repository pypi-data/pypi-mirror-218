...
class ObjectAnnotation(dataobject):
    ...
    def _init_labels(self, labels):
        ...
        # Make sure that the type of the class field is either a string or an integer.
        if (not isinstance(self._label, str)) and (not isinstance(self._label, int)):
            class_type = type(self._label)
            raise AutoMLVisionDataException(
