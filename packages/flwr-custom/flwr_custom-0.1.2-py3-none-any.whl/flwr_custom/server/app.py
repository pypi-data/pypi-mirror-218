...
from flwr_custom.common import GRPC_MAX_MESSAGE_LENGTH, EventType, event
from flwr_custom.common.address import parse_address
from flwr_custom.common.constant import MISSING_EXTRA_REST
from flwr_custom.common.logger import log
from flwr_custom.proto.driver_pb2_grpc import add_DriverServicer_to_server
from flwr_custom.proto.transport_pb2_grpc import add_FlowerServiceServicer_to_server
from flwr_custom.server.client_manager import ClientManager, SimpleClientManager
from flwr_custom.server.driver.driver_servicer import DriverServicer
from flwr_custom.server.grpc_server.driver_client_manager import DriverClientManager
from flwr_custom.server.grpc_server.flower_service_servicer import FlowerServiceServicer
from flwr_custom.server.grpc_server.grpc_server import (
    generic_create_grpc_server,
    start_grpc_server,
)
from flwr_custom.server.history import History
from flwr_custom.server.server import Server
from flwr_custom.server.state import StateFactory
from flwr_custom.server.strategy import FedAvg, Strategy
...
def _fl(
    ...
    status_dict[LOG] += f"app_fit: losses_distributed %s, {str(hist.losses_distributed)}\n" \
                        f"app_fit: metrics_distributed_fit %s, {str(hist.metrics_distributed_fit)}\n" \
                        f"app_fit: metrics_distributed %s, {str(hist.metrics_distributed)}\n" \
                        f"app_fit: losses_centralized %s, {str(hist.losses_centralized)}\n" \
                        f"app_fit: metrics_centralized %s, {str(hist.metrics_centralized)}\n"
...
def _run_fleet_api_rest(
    ...
    try:
        ...
        from flwr_custom.server.rest_server.rest_api import app as fast_api_app
