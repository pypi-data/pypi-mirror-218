...
from flwr_custom.proto import transport_pb2_grpc
from flwr_custom.proto.transport_pb2 import ClientMessage, ServerMessage
from flwr_custom.server.client_manager import ClientManager
from flwr_custom.server.grpc_server.grpc_bridge import GrpcBridge, InsWrapper, ResWrapper
from flwr_custom.server.grpc_server.grpc_client_proxy import GrpcClientProxy
        ...
        if is_success:
            ...
            while True:
                try:
                    ...
                    for id in range(self.status_dict[CLIENT_NUM]):
                        for round in range(self.status_dict[ROUND_NUM]):
                            for type in TYPE_LIST:
                                key = (id, round, type, "msg")
                                if key in self.status_dict and client_message == self.status_dict[key]:
                                    if type == TRAIN:
                                        result_type = "training"
                                    else:
                                        result_type = "evaluating"
                                    self.status_dict[LOG] += f"{date.now_custom()}, round {round}: client {id} {result_type} result arrived to server\n"
