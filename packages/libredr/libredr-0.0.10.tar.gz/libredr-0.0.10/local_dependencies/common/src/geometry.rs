...
pub fn uv_xyz(vertex: ArrayView3<f32>, texture_resolution: usize, padding: usize) -> Result<Array3<f32>> {
  ...
  for vertex in vertex.axis_iter(Axis(2)) {
    ...
    let u_min = f32::max(0.0, vt_curr.column(0).min() - padding as f32) as usize;
    let u_max = f32::min(texture_resolution as f32, vt_curr.column(0).max() + padding as f32 + 1.0) as usize;
    let v_min = f32::max(0.0, vt_curr.column(1).min() - padding as f32) as usize;
    let v_max = f32::min(texture_resolution as f32, vt_curr.column(1).max() + padding as f32 + 1.0) as usize;
    for u in u_min..u_max {
      for v in v_min..v_max {
        ...
        let w = f32::max(1e-6, f32::min(1.0 - 1e-6, -area_2 / area));
        let v = f32::max(1e-6, f32::min(1.0 - 1e-6 - w, -area_1 / area));
        let u = 1.0 - 1e-6 - w - v;
...
impl Geometry {
  ...
  ///
  /// `v` and `vt` are hashed and cached for better performance
  pub fn add_trimesh(
      &mut self,
      vertex: Array3<f32>,
      transform_v: Array2<f32>,
      transform_vt: Array2<f32>,
      data_cache: &DataCache) -> Result<&mut Self> {
    ...
    let trimesh = TriMesh {
      transform_v,
      transform_vt,
      trimesh_data: trimesh_data_hash.to_owned(),
    };
    self.trimesh.push(trimesh);
    {
      let mut data_cache = data_cache.lock().expect("No task should panic");
      data_cache.insert(trimesh_data_hash, (Utc::now().timestamp(), trimesh_data));
    }
    Ok(self)
  }
