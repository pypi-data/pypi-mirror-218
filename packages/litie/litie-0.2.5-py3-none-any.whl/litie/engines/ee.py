...
from ..metrics import EventExtractionScore
from ..nn.ee import AutoEventExtractionTaskModel, AutoEventExtractionTaskModelConfig
from ..pipelines import EventExtractionPipeline


class EventExtractionEngine(TaskEngine):
    def __init__(
        self,
        model_type: str,
        task_model_name: str,
    ...
    def get_auto_model_config(self, task_model_name, labels, **kwargs):
        return AutoEventExtractionTaskModelConfig.create(task_model_name, labels, **kwargs)

    def get_auto_model(self, model_type, task_model_name):
        return AutoEventExtractionTaskModel.create(
            task_model_name,
            model_type,
    ...
    def configure_metrics(self, _) -> None:
        self.metric = EventExtractionScore()
    ...
    def compute_metrics(self, mode="val") -> Dict[str, float]:
        ...
        self.metric.reset()
        return {
            f"{mode}_precision": values["event"][0],
            f"{mode}_recall": values["event"][1],
            f"{mode}_f1": values["event"][2],
            f"{mode}_argu_precision": values["argu"][0],
            f"{mode}_argu_recall": values["argu"][1],
            f"{mode}_argu_f1": values["argu"][2]
        }

    @property
    def pipeline(self) -> Any:
        if self._pipeline is None:
            self._pipeline = EventExtractionPipeline(
