...
class DataCollatorForMrcNer:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        flattened_features = [
            [{k: v[i] for k, v in feature.items() if k not in self.ignore_list} for i in range(self.num_labels)] for
            feature
            in features]
...
class MrcForNerDataModule(NerDataModule):
    ...
    def get_process_fct(self, text_column_name, label_column_name, mode):
        ...
        convert_to_features = partial(
            MrcForNerDataModule.convert_to_features,
            schema2prompt=self.labels,
    ...
    def convert_to_features(
        ...
        if mode == "train":
            ...
            for entity_list in examples[label_column_name]:
                label_dict = {k: [] for k in schema2prompt}
