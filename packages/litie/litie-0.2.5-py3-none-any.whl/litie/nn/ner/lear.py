...
from ...layers import LabelFusionForToken, Classifier, MLPForMultiLabel
...
def get_auto_lear_ner_model(
    ...
    class LearForNer(parent_model):
        ...
        def decode(self, start_logits, end_logits, span_logits, attention_mask, texts, offset_mapping):
            decode_labels = []
            id2label = {int(i): v for i, v in enumerate(self.config.labels)}
        ...
        def compute_loss(self, inputs):
            ...
            return loss_fct((start_logits, end_logits, span_logits), (start_labels, end_labels, span_labels), mask,
                            nested=True)

    return LearForNer


def get_lear_ner_model_config(labels: dict, **kwargs):
