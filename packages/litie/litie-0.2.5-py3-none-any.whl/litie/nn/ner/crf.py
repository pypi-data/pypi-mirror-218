...
from ...datasets.utils import sequence_padding, tensor_to_cpu
...
def get_auto_crf_ner_model(
    ...
    class CrfForNer(parent_model):
        ...
        def __init__(self, config):
            ...
            self.crf = CrfLayer(num_tags=config.num_labels, batch_first=True)
...
def get_auto_cascade_crf_ner_model(model_type: str = "bert"):
    ...
    class CascadeCrfForNer(parent_model):
        ...
        def __init__(self, config):
            ...
            self.crf = CrfLayer(num_tags=3, batch_first=True)
    ...
    class SoftmaxForNer(parent_model):
        ...
        def __init__(self, config):
            ...
            loss_fcts = {
                ...
                'label_smoothing_ce': LabelSmoothingCrossEntropy(ignore_index=0)
            }
...
def get_softmax_ner_model_config(labels: list, **kwargs):
...
def get_crf_ner_model_config(labels: list, **kwargs):
    """CRF实体识别模型的主要参数配置
    """
    model_config = get_softmax_ner_model_config(labels, **kwargs)
    special_config = {"use_lstm": False, "mid_hidden_size": 256}
    model_config.update(special_config)
    return model_config


def get_cascade_crf_ner_model_config(labels: list, **kwargs):
