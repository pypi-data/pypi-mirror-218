...
def getLandmarks(img):
    ...
    # Setup the face landmark function for videos
    with mp_face_mesh.FaceMesh(max_num_faces=2, static_image_mode=True, min_detection_confidence=0.5) as face_mesh:
        ...
        # (OpenCV reads the image in BGR whileas MediaPipe accepets image in RGB)
        # Gets a list of 468 facial landmarks for each detected face
        # - x-coordicates, y-coordinates, z-depth
        ...
        # When facial landmarks are found
        # Iterate over the detected faces
        # results.multi_face_landmarks contains the found landmarks coordinates of each face as an element of a list
        for face_landmarks in results.multi_face_landmarks:
            values = np.array(face_landmarks.landmark)
            face_keypnts = np.zeros((len(values), 2))

            # Obtain landmark coordicates
            for idx,value in enumerate(values):
                face_keypnts[idx][0] = value.x
                face_keypnts[idx][1] = value.y

            # The coordinates for the landmark points are normalized to lie between 0 and 1.
            # Convert normalized points to image coordinates by multiplying the x and y coordinates
            # with the image’s width and height, respectively
            face_keypnts = face_keypnts * (width, height)
            face_keypnts = face_keypnts.astype('int')


            # Obtain relevent facial landmarks coordinates
            relevant_keypnts = []
...
# Get alpha channel from filter image for later use adn convert image to BGR
...
# Once we have triangulations of both the images we can transform the triangles from one image(filter)
# to fit onto another image(person’s face).
...
def apply_ar_face(frame,filters,multi_filter_runtime):
