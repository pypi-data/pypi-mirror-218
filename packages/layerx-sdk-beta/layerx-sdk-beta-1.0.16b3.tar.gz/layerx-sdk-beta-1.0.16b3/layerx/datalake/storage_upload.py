...
class StorageUpload:
    ...
    def __init__(self, client, collection_name, upload_id, upload_type, test_write_progress, add_fail_files, application_code):
        self._client = client
        ...
        for file in sub_list:
            ...
            for part in url_array:
                ...
                if is_upload_success == True:
                    ...
                    max_read_count = MULTI_PART_UPLOAD_CHUNK_SIZE if (part["PartNumber"] - 1) < file["part_count"] - 1 else file["size"] - next_byte
                    upload_s3_response = storage_interface.upload_to_storage(part["signedUrl"], file["path"], self.upload_type, next_byte, max_read_count)
