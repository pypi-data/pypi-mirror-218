...
from layerx.datalake.constants import MAX_UPLOAD_RETRY_COUNT, MediaType, FILE_UPLOAD_THREADS, SUB_FILE_LENGTH, MULTI_PART_UPLOAD_CHUNK_SIZE
from layerx.datalake.storage_upload import StorageUpload
...
class FileUpload:
    ...
    def write_progress(self, count=True, uploaded_file_count=1, uploaded_chunk_count=1):
        if count:
            ...
            try:
                ...
                self.progress = 100 * (self.uploaded_chunk_count / self.total_chunk_count)
                sys.stdout.write(
    ...
    def add_fail_files(self, file, part_number = None, uploaded_parts_arr = [], part_number_array = []):
        self.count_lock.acquire()
        try:

            index = next((index for (index, _file) in enumerate(self.fail_upload_set) if _file["key"] == file["key"]), None)
            ...
            if(index != None):
                ...
                if(uploaded_parts_arr != None):
                    for uploaded_part in uploaded_parts_arr:
                        index_upload = next((index for (index, _uploaded_part) in enumerate(uploaded_parts_arr) if _uploaded_part["PartNumber"] == uploaded_part["PartNumber"]), None)
                        if(index_upload == None):
                            self.fail_upload_set[index]["uploaded_parts_arr"].append(uploaded_part)
    ...
    def parallel_upload(self, sublist):
        _upload = StorageUpload(self._client, self.collectionName, self.uploadId, self.upload_type, self.write_progress, self.add_fail_files, self.application_code)
    ...
    def file_upload_initiate(self, path, collection_type, collection_name, meta_data_object, meta_data_override):
        ...
        while self.failed_upload_count > 0 and retry_count < MAX_UPLOAD_RETRY_COUNT:
            ...
            self.failed_upload_count = 0
            retry_count += 1
            print('Retry count: ' + str(retry_count) + ' of ' + str(MAX_UPLOAD_RETRY_COUNT)) 
