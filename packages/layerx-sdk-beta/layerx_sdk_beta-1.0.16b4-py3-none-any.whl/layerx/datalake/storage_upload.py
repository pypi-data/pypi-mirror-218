...
from layerx.datalake.storage_interface import StorageInterface


class StorageUpload:

    def __init__(self, client, storage_type, collection_name, upload_id, upload_type, test_write_progress, add_fail_files, application_code):
        self._client = client
        ...
        for file in sub_list:
            ...
            if(file["file_id"] == None):
                ...
                if multipart_init_res["isSuccess"]:
                    ...
                    file["is_all"] = True
                    self.add_fail_files(file, None, uploaded_parts_arr, file["part_numbers"])
                    continue
            else:
                ...
                if file["is_all"]:
                    if len(file["part_numbers"]) == 0:
                        ...
                        for part_num in file["part_numbers"]:
                            index = next((index for (index, part) in enumerate(pre_signed_url_response["parts"]) if part["PartNumber"] == part_num), None)
            ...
            for part in url_array:
                ...
                if is_upload_success == True:
                    ...
                    max_read_count = MULTI_PART_UPLOAD_CHUNK_SIZE if chunk_id < file["part_count"] - 1 else file["size"] - next_byte
                    #url, path, content_type, chunk_id, file_size, start_byte, read_bytes
                    upload_s3_response = storage_interface.upload_to_storage(part["signedUrl"], file["path"], content_type, chunk_id, file["size"], next_byte, max_read_count)

                if not upload_s3_response["isSuccess"]:
                    file["is_all"] = True
                    file["file_id"] = self.file_id
                    file["file_key"] = self.file_key
                    self.add_fail_files(file, part["PartNumber"], uploaded_parts_arr, None)
                    ...
                    uploaded_parts_arr.append({
                        "PartNumber": upload_s3_response["part_id"],
