...
def adapt_vast(df):
    df = df [["new_topic", "text_s", "label"]]
    df.rename(columns={"new_topic" : "topic", "text_s": "text", "label": "stance"}, inplace=True)
    return df[["topic", "stance", "text"]]

def load_splits(experiment, oversample=True, validate=True):
    """
