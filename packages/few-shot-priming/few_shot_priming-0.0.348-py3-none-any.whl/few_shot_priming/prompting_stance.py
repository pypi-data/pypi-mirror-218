...
def run_experiment_prompting(config=None, experiment="ibmsc", offline=False, validate=True, splits=None, logger=None):
    ...
    labels_text_map = {1: ["pro" ], 0: ["con"], 2: ["neutral"] }
    if offline:
    ...
    for i, record in test_dataset.iterrows():
        ...
        generated = model.generate(seq, max_new_tokens=3, temperature=0)
        resulting_string = tokenizer.decode(generated.tolist()[0])
        log_message(logger, resulting_string, level=logging.INFO)
        resulted_string_truncated = resulting_string[-15:].lower()
        if "pro" in resulted_string_truncated or "supports" in resulted_string_truncated or "positive" in resulted_string_truncated:
            predictions.append(1)
        elif "con" in resulting_string[-15:].lower():
            predictions.append(0)
        else:
            predictions.append(2)
        labels.append(label)
    log_message(logger, f"predictions {predictions}", level=logging.INFO)
    log_message(logger, f"labels {labels}", level=logging.INFO)
...
def run_experiment_prompt_fine_tuning(config=None, experiment="ibmsc", params=None, offline=False, validate=True, splits=None, logger = None):
    """
