#!/usr/bin/env python
#
# results_load.py - The fsleyes MRS plugin - load results from fsl_mrsi
#
...
from fsleyes_plugin_mrs.plugin import MRSView
...
class FSLFitTool(actions.Action):
    ...
    def __init__(self, overlayList, displayCtx, frame):
        actions.Action.__init__(self, overlayList, displayCtx, self.__loadResults)
        self.frame = frame
    ...
    def _identifyResults(self, dir_path):
        """Identify the metabolites used and the images availible.
    ...
    def _loadFit(self, dir_path):
        ...
        def load_and_disp(file_name, lw, colour):
            ...
            if not full_path.exists():
                log.warn(f'{file_name} image not found in {full_path}, skipping.')
...
class FSLMRSResultsControl(ctrlpanel.SettingsPanel):
    ...
    def _selected_result_change(self, *a):
        ...
        if not Path(full_path).exists():
            log.warn(f'{full_path} image not found, skipping.')
            return
