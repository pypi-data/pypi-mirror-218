...
from ..datapreparation.datapreparation_utils import PreprocessingData
import pickle
...
class PreprocessingClass(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,
        ...
        if self.usecase == "teleconsulto":
            self.preprocessing = PreprocessingData()
    ...
    def transform(self,
        ...
        if self.usecase == "teleconsulto":
            if self.model == "prophet":
                X = self.preprocessing.generate_time_series(X, self.datetime_index, self.target, self.time_granularity,
                                                            self.missing_data_strategy, self.normalize_data)
            else:
                pass
        elif self.usecase == "televisita":
            pass
