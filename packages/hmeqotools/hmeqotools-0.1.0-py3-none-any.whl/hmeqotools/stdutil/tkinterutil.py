...
def adapt_widgets_width(widgets, req=True):
    ...
    width = max(i.winfo_reqwidth() if req else i.winfo_width()
                for i in widgets)
    for i in widgets:
...
class Window:
    ...
    def __init__(self, size=None, root=None, start_quit_thr=True):
        ...
        self.resolution = (self.tk.winfo_screenwidth(),
    ...
    def align_center(self, req=False):
        ...
        self.tk.wm_geometry("+{}+{}".format(
            (self.resolution[0] - w) // 2,
            (self.resolution[1] - h) // 2))
...
class Option(WidgetMethods, MouseMessage):
    ...
    def on_mouse_enter(self):
        ...
        if not self.mouse_status_left and not self.selected:
            self.label.configure(fg=self.fg_mouse_enter,
                                 bg=self.bg_mouse_enter)
    ...
    def add(self, value=None, text=None, option=None, **kwargs):
        if isinstance(option, Option):
            ...
            if option is None:
                option = Option
            op = option(
                self.f, text=text if text else "", value=value, **kwargs)
