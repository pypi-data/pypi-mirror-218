...
@t.overload
def sequential_search(lst, item, start=0, stop=None, key=None) -> t.Union[int, None]:
    ...
...
@t.overload
def binary_search(lst, item, low=0, high=None, key=None) -> t.Union[int, None]:
    ...
...
class Sort:
    ...
    def add(self, value):
        """添加."""
        index = binary_search(self.data, self.key(value) if self.key else value,
                              key=self.key, insertion=True)
