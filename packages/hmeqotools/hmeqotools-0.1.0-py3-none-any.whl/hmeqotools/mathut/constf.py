...
from . import funcs as _mfn
try:
    from .. import progressbar as _pgb
except ModuleNotFoundError:
    import hmeqotools.progressbar as _pgb
...
class Pi:
    ...
    def ramanujan(cls, n: int, is_prec=True, space_for_time=False) -> int:
        ...
        with multiprocessing.Pool(processes=cpu_count) as pool:
            ...
            result = _decimal.Decimal(0, 0, prec + cls.k)
            count = 4 * cpu_count
            bar = _pgb.ProgressBar(total=iter_count, **cls.pgb_config)

            if space_for_time:
                ftrs = [i for i in _mfn.factorial_gen(6 * iter_count)]
...
class E:
    ...
    def taylor(cls, precision=16) -> int:
        ...
        with multiprocessing.Pool(processes=cpu_count) as pool:
            ...
            result = _decimal.Decimal(0, 0, p1)
            one = result.convert_int(1)
            count = 64 * cpu_count

            params = [(one, i) for i in _mfn.factorial_gen(p2)]
