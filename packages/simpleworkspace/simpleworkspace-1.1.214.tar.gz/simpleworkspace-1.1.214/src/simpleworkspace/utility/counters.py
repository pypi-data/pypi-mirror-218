...
class ProgressBar:
    ...
    def __init__(self, iterable=None, total=None):
        ...
        self.setting_style_unkownFormat = "Progress: {incremented} [elapsed={elapsedTime}, speed={speed:.1f}/s]"
        ...
        self.setting_style_format = "Progress: |{bar}| {percentage:.1f}% [elapsed={elapsedTime}|remaining={remainingTime}|speed={speed:.1f}/s|pcs={incremented}/{total}]"
        ...
        self.setting_printAsync_refreshDelay = 1
        '''when printasync is used how often to refresh bar stats and visuals, delay is specified in seconds'''
    ...
    def _PrintAsync(self):
        ...
        while(not self._thread_PrintAsync_stopEvent.is_set()):
            if(self.total is not None) and (self.stats_incremented >= self.total):
                break
            self._thread_PrintAsync_stopEvent.wait(self.setting_printAsync_refreshDelay)
            self.Print()
    ...
    def PrintAsync(self):
        ...
        self._thread_PrintAsync = threading.Thread(target=self._PrintAsync)
    ...
    def Print(self):
        ...
        if(self.total is None):
            sys.stdout.write(self.setting_style_unkownFormat.format(
        ...
        sys.stdout.write(self.setting_style_format.format(
            bar=bar, percentage=percentage, elapsedTime=elapsedTime_str, remainingTime=remainingTime_str, speed=incrementsPerSecond, incremented=self.stats_incremented, total=self.total
        ))
