...
def get_nes_url() -> str:
    ...
    nes_url = get_secrets(mount_point="datafabric", path="nes")["url"]

    if nes_url is None or nes_url == "":
        raise Exception("Not exist nes url...")
...
def nes_submit(
    ...
    res = requests.post(get_nes_url(), json=data)
...
def nes_get_status(id):
    res = requests.get(f"{get_nes_url()}/{id}")
    res.raise_for_status()
