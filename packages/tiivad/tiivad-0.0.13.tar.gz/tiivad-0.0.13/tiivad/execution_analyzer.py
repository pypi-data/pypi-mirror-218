...
def extract_numbers(s):
    ...
    for el in s.split():
        el = el.rstrip(".,:;?!)]}'\"%€$£")
        if re.match(r"-?(?!0\d+)\d+$", el):
            numbers.append(int(el))
        elif re.match(r"-?(?!0\d+)\d+\.\d+$", el):
            numbers.append(float(el))
    return numbers

def extract_words(s, word_list):
    e = r"\b(" + "|".join(word_list) + r")\b" if word_list else None
    return [] if e is None else re.findall(e, s)
...
class ProgramExecutionAnalyzer:
    ...
    def analyze_output_with_quantifier(self, file_name, target: str, quantifier, values: list,
                                       nothing_else: bool = False, ordered: bool = False) -> bool:
        if file_name:
        ...
        match target:
            case 'contains_numbers': all_values = extract_numbers(content)
            case 'contains_keywords': all_values = extract_words(content, values)
            case 'contains_lines': all_values = extract_lines(content)
            case 'equals': all_values = [content.rstrip()]
            case _: all_values = []
        match quantifier:
...
if __name__ == "__main__":
    ...
    ea = FunctionExecutionAnalyzer(file_name, function_name, function_type, create_object_body, arguments,
                                   user_inputs, input_files)
    print(ea.all_io)
    print(ea.result)
    print(ea.exception)
    print(arguments)
    print(ea.value_correct(1, [3, 4, 1]))