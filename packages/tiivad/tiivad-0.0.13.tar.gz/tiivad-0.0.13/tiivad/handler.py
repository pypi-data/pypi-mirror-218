...
def execute_test(**kwargs):
    ...
    if test_type in STATIC_TESTS_ONE | STATIC_TESTS_MANY:
        ...
        if ea.raised_exception() and isinstance(ea.exception, OutOfInputsError):
        ...
        elif 'exception_check' in kwargs:
            ...
            test_status = TestResult.FAIL
            checks.append(
                check_result(test_type, test_status, f"{type(ea.exception).__name__}: {str(ea.exception)}"))
        ...
        for i, check_list in enumerate(check_lists):
            ...
            for check in check_list:
                if i == 0:
                    result = ea.analyze_output_with_quantifier(check.get('file_name', None), check['elements_type'],
    ...
    Results({
        ...
        "user_input": kwargs.get("standard_input_data", []),
        "created_files": [{"name": x[0], "content": x[1]} for x in kwargs.get("input_files", [])],
        "converted_submission": False,
        "actual_output": False,
