...
def assign_role_command(workspace, role, user, service_account, file):
    if file is not None:
        if user or service_account:
            msg = "Please use exactly one of: --user, --service-account, or --file"
...
def _update_role(workspace, role, user, service_account, unassign=False):
    ...
    if role_def is None:
        msg = f"Invalid role id. Possible values are: {', '.join(r.id for r in role_defs if _is_role_assignable(r, principal_type, resource_type))}"
        raise click.ClickException(msg)
    ...
    human_readable_principal_name = user if user is not None else service_account
    try:
        if unassign:
...
def _bulk_update_user_role(workspace, file, role):
    for user in [line.strip() for line in file.readlines() if len(line.strip()) > 0]:
        _update_role(workspace, role, user, service_account=None)
...
def get_assigned_roles(user, service_account, resource_type, json_out):
    ...
    for assignment in ws_assignments + org_assignments:
        if len(assignment.roles_granted) > 0:
            roles = [
                _maybe_convert_legacy_role_id(role_defs, role_granted.role) for role_granted in assignment.roles_granted
            ]
...
def get_principal_details(user, service_account):
    if user and service_account:
        msg = "Please mention a single Principal Type using one of --user or --service-account"
        raise click.ClickException(msg)
    if user:
        ...
        msg = "Please mention a Principal Type using --user or --service-account"
        raise click.ClickException(msg)
