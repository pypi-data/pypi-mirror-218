...
def clkq(input_clk_paths, norm, input_bia_paths, reject_re, plot):
    ...
    if reject_re is not None:
        ...
        _logging.info(msg=f"Removed the following satellites from first file: '{clk_a[reject_mask_a].index.get_level_values(2).unique().to_list()}'")
        clk_b = clk_b[~reject_mask_b]
        _logging.info(msg=f"Removed the following satellites from second file: '{clk_b[reject_mask_b].index.get_level_values(2).unique().to_list()}'")
