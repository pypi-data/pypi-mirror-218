...
from tuxsuite.cli.utils import error, file_or_url
...
def handle_get(cmdargs, _, config):
    ...
    if ret.status_code != 200:
        ...
        if not cmdargs.config and cmdargs.plan is None:
            ...
            if cmdargs.config:
                print(
                    f"Tuxtrigger config: config.yaml\n\n{tuxtrigger.get('config', '')}"
                )
            elif cmdargs.plan:
                print(
                    f"Tuxtrigger plan: {cmdargs.plan}\n\n{tuxtrigger.get('plan', '')}"
                )
...
def tuxtrigger_delete_options(sp):
...
def setup_parser(parser):
    ...
    tuxtrigger_add_options(t)

    # "Tuxtrigger get"
    t = parser.add_parser("get")
    tuxtrigger_get_options(t)

    # "Tuxtrigger delete"
    t = parser.add_parser("delete")
    tuxtrigger_delete_options(t)
