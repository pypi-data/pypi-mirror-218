...
def subGeneration(dataSet           # The DataSet
    ...
    print(batchNumber, batches, out_name)
...
def condorGeneration( batches = config['far_config']['batches']     # Number of parallel subscripts scripts to run 
    ...
    while (1):
        ...
        for file in listOfInputs: 
            ...
            print("Jobs and Dags running: ",jobsAndDagsRunning,", Files in temp: ",filesInTemp)
            sys.stdout.flush()
            while (jobsAndDagsRunning >= batches*(parallelGenerations-1)) or filesInTemp >= parallelGenerations*batches:
                ...
                if heldJobs > 0 and releaseJobsTrials < 5:
                    os.system("condor_release --all")
                    releaseJobsTrials += 1
                    print("Held Jobs: ",heldJobs 
                            , " , release trials: ",releaseJobsTrials
                            ,datetime.now().strftime("  %m/%d/%Y %H:%M:%S"))
                elif heldJobs > 0 and releaseJobsTrials >= 5:
                    os.system("condor_qedit "+config['user_name']+" -constraint 'JobStatus == 5' RequestMemory 4096")
                    os.system("condor_release --all")
                    time.sleep(60)
                else:
                    releaseJobsTrials = 0
...
if __name__ == "__main__":
    ...
    if mode == 'generation':
        ...
        # t2 = Process(target = inference, args = (dataSet_Q, dataSetName_Q, batch_size))   
        # t1.daemon=True
        # t2.daemon=True             
