...
def run_mutant(mutant: GambitMutant, src_dir: Path, trg_dir: Path, orig_conf: Path,
    ...
    file_path_to_mutate = trg_dir / orig_file
...
def get_gambit_exec() -> str:
    ...
    # try executing it
    try:
        rc = subprocess.run([exec, "--help"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if rc.returncode == 0:
            return exec
        else:
            # try just "gambit"
...
def get_gambit_out_dir(args: argparse.Namespace) -> Path:
    ...
    # (Gambit in json mode ignores -o. Luckily, it did not ignore skip-validate.
...
def collect(args: argparse.Namespace) -> bool:
    ...
    for mutant, mutant_result_list in mutants_results:
        if not mutant_result_list:
            print("Corrupted mutant results list")
            sys.exit(1)
