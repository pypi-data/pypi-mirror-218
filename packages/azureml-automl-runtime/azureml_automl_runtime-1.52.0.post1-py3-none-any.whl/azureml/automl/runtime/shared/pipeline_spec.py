...
class SklearnObject(ObjectSpec):
    ...
    def prepare_kwargs(self, cls, problem_info,
        ...
        """Set self.prepared_kwargs, a modified copy of param_kwargs."""
        self.prepared_kwargs = copy.deepcopy(self.param_kwargs)
        # The stop gap measure if Miro returns the old spec for SGDRegressor.
        if cls.__name__.startswith('SGD') and 'max_iter' in inspect.signature(
                cls).parameters and 'n_iter' in self.prepared_kwargs:
...
class PipelineSpec:
    ...
    def instantiate_pipeline_spec(self,
                                  ...
                                  dataset_metadata: Optional[Any] = None) -> pipeline.Pipeline:
