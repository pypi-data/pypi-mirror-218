...
def orchestrate_featurization(
    ...
    """
    td_ctx = None  # type: Optional[TransformedDataContext]
    ...
    if is_timeseries:
        ...
        if td_ctx._is_autofeaturization_scenario():
            from azureml.train.automl.runtime._code_generation.utilities import \
                generate_autofeaturization_code_and_notebook
            logger.info("Autofeaturization run, no data split required.")
            timeseries_transformer = td_ctx.transformers[constants.Transformers.TIMESERIES_TRANSFORMER]
            parent_run = Run.get_context()
            if "_setup" in parent_run.id:
                parent_run = Run.get_context().parent
            generate_autofeaturization_code_and_notebook(parent_run, pipeline=timeseries_transformer)
