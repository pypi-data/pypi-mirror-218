...
from scipy.special import laguerre, genlaguerre, gammaln
...
def wigner(rho, min_ = -5, max_ = 5, steps = 400):
    """
    Directly taken from Qutip Module, details available here: https://qutip.org/docs/4.0.2/modules/qutip/wigner.html
    
    _____________
    
    Added a few modifications : 
    - min_ , max_ = minimum and maximum range for the plot of the Wigner distribution.
    - steps = No. of steps between the minimum and the maximum range.
    """
#     rho = sq.density_mat_from_state_vec(sq.norm_state_vec([1, 0, 0]))
    g = np.sqrt(2)
    M = len(rho[0])

    xvec = np.linspace(min_, max_, steps)
    yvec = np.linspace(min_, max_, steps)
...
def rad(img, resolution = 1, axis = 0):
    """
    Function :
    
    Gives the radon transform of a given 2d array.
    
    Arguments : 
    
    - img = 2 dimensional array
    - resolution = precision with which the image is to be rotated each time. 
    Set to 1, meaning each time image is rotated by 1 degree. Increase if higher precision needed. For resolution = 2,
    the image will be rotated by 0.5 degrees and so on.
    - axis = Which axis to be used for marginal distribution. Sums up all the elements along this axis. Set to 0, meaning the
    elements will be summed up along the horizontal axis. For vertical axis, set to 1.
    
    Returns:
    
    The radon transform of the given 2d array.
    """
    sino = []
    padding = int(img.shape[1] * 0.2)
    img = np.pad(img, (padding, padding))
    for theta in np.arange(360, 0, -1 /  resolution):
        one_row = np.sum(transform.rotate(img, theta), axis = axis)
        sino.append(one_row)
...
def marg_dis(arr1d, ratio = 0.4, pad_ratio = 1, dot_size = 20, resize = True, restrict = True):
    """
    Function:
    
    Calculates the marginal distribution of a 1D array and generates a 2D matrix representing the distribution.
    
    Arguments:
    
    - arr1d: The 1D array for which the marginal distribution needs to be calculated. It can be a list or a NumPy array.
    - ratio: The ratio of the output matrix width to its height. Defaults to 0.4.
    - pad_ratio: The padding ratio to be applied to the output matrix. Defaults to 1.
    - dot_size: The size of the dots used to represent the distribution in the matrix. Defaults to 20.
    - resize: Determines whether to resize the output matrix. If True, the output matrix is resized to (1000, 720).
      Defaults to True.
    - restrict: Determines whether to restrict the input array length. If True and the array length exceeds 30000,
      it will be truncated to 30000. Defaults to True.
    
    Returns:
    
    The 2D matrix representing the marginal distribution.
    """
    if restrict == True and len(arr1d) > 30000: arr1d = arr1d[:30000]
    arr1d = np.array(arr1d)
    rows, cols = float(np.max(arr1d) - np.min(arr1d)), len(arr1d)
    new_arr1d = (cols / rows * ratio) * arr1d
    marg2d = np.zeros((int((np.max(new_arr1d) - np.min(new_arr1d))), len(new_arr1d)))
    for i in range(1, marg2d.shape[1]):
        j = int(new_arr1d[i]) + marg2d.shape[0] // 2
        marg2d[j - dot_size: j + dot_size, i - dot_size: i + dot_size] = 1
#                 print(i, j, marg2d[i, j])
    if resize == True:
        return np.pad(transform.resize(marg2d, (1000, 720)), ((int(pad_ratio * 200), int(pad_ratio * 200)), (0, 0)))
    else:
        return marg2d

def smooth_marg(mmm, lower_t = 30, higher_t = 1395):
    smooth_marg_dis = []
    for i in range(mmm.shape[1]):
        one_line = mmm[:, i]
        changed = np.fft.ifft(np.array([x if (x < lower_t or x > higher_t) else 0 for x in range(mmm.shape[0])]) * np.fft.fft(one_line))
        smooth_marg_dis.append(np.real(changed))
    return np.array(smooth_marg_dis) / np.max(smooth_marg_dis)

def factorial(n):
    return math.exp(gammaln(n + 1))

def lag(num, min_ = -1, max_ = 1, steps = 400):
    x_vec = np.linspace(min_, max_, steps)
    p_vec = np.linspace(min_, max_, steps)  
    X, P = np.meshgrid(x_vec, p_vec)
    return (-1)**num * np.exp(-1 * (X**2 + P**2)) * laguerre(num)(2 * (X**2 + P**2))