...
from prompt4all_liteon.utils.chatgpt_utils import *
from prompt4all_liteon.utils.regex_utils import *
import prompt4all_liteon.api.context_type as ContextType
from prompt4all_liteon.api.base_api import *
from prompt4all_liteon.utils.tokens_utils import *
from prompt4all_liteon.utils.summary_utils import *
from prompt4all_liteon.theme import adjust_theme, advanced_css
import prompt4all_liteon.utils.whisper_utils
from prompt4all_liteon.utils.whisper_utils import *
from prompt4all_liteon import context
from prompt4all_liteon.context import *
...
#
os.environ['OPENAI_API_KEY']= "5c1ca0aa453f425683262ee1bd5d7eab"
...
async def rolling_summary(large_inputs, full_history, summary_method):
    _parameters = copy.deepcopy(baseChatGpt.API_PARAMETERS)
    ...
    if summary_method==2:
        _system_prompt = open("prompts/parallel_chunks_summary.md", encoding="utf-8").read()
            #"你是一個萬能文字助手，你擅長使用繁體中文以條列式的格式來整理逐字稿、會議記錄以及長文本文件，你懂得如何將「輸入文字內容」視狀況修正同音錯字，去除口語贅字後，尤其是涉及[人名、公司機構名稱、事物名稱、地點、時間、數量、知識點、事實、數據集、url]這些資訊時，在保持原意不變的前提下進行摘要，請基於縮排或標號(\"-\")來表達摘要資訊間的階層性與相關性，輸出結果應該是「摘要清單」，不解釋原因。請以繁體中文書寫。 #zh-tw"
        ...
        this_system_tokens = estimate_used_tokens(_system_prompt) + estimate_used_tokens('system',model_name=baseChatGpt.API_MODEL) + 4
        this_final_tokens = estimate_used_tokens(_final_prompt) + estimate_used_tokens('system',model_name=baseChatGpt.API_MODEL) + 4
        available_tokens = baseChatGpt.MAX_TOKENS - this_system_tokens - 4 - 2
        ...
        while keep_summary:
            ...
            #摘要量過大需要縮減
            if summary_size_ratio>0.5 and not is_final_stage:
            ...
            try:
                ...
                if len(large_inputs) == 0 :
                    if is_final_stage:
                        ...
                        is_final_stage=True
                        keep_summary = False
                        available_tokens = baseChatGpt.MAX_TOKENS - this_final_tokens - 4 - 2
                ...
                answer = ''
                answer_head = '[第{0}部分摘要 {1:.2%}]\n\r'.format(cnt + 1,float(large_inputs_tokens-remain_tokens)/large_inputs_tokens) if not is_final_stage else "[最終版摘要]"
                while True:
                    try:
                        answer, full_history = next(streaming_answer)
                        if "1. 第一個項目" in answer:
                            streaming_answer.close()
                            _parameters['temperature'] = 0.5
                            _parameters['presence_penalty'] = 1.5
                            streaming_answer = baseChatGpt.post_and_get_streaming_answer(conversation, _parameters,full_history)
                            answer, full_history = next(streaming_answer)
                            _parameters['temperature'] = 0.001
                            _parameters['presence_penalty'] = 1.2
                        if not is_final_stage:
                            yield text2markdown(answer_head +'\n'+('\n'.join(unchanged_summary) if len(unchanged_summary)>0 else '')+'  \n'+answer), full_history
                        else:
                ...
                yield text2markdown(answer_head + '\n'.join(unchanged_summary)+'\n'+answer) if not is_final_stage else text2markdown('\n'.join(cleansed_summary)), full_history
            except Exception as e:
                print(e)
                PrintException()

    elif summary_method==3:
        _system_prompt =open("prompts/mindmap_summary.md", encoding="utf-8").read()
        _final_prompt = open("prompts/summary_final_cleansing.md", encoding="utf-8").read()
        this_system_tokens = estimate_used_tokens(_system_prompt) + estimate_used_tokens('system',model_name=baseChatGpt.API_MODEL) + 4
        this_final_tokens = estimate_used_tokens(_final_prompt) + estimate_used_tokens('system',model_name=baseChatGpt.API_MODEL) + 4

        available_tokens = baseChatGpt.MAX_TOKENS * 0.667 - this_system_tokens - 4 - 2
        summary_history ="mindmap\nroot((摘要心智圖)) "
        partial_words = ''
        unchanged_summary = []
        keep_summary = True
        cnt = 0
        try:
            while keep_summary:
                this_system_tokens = estimate_used_tokens(str(summary_history))
                ...
                conversation = [
                    {
                        "role": "system",
                        "content": _system_prompt if keep_summary else  _final_prompt
                    },
                    {
                        "role": "user",
                        "content": passage if keep_summary else  _final_prompt
                    }
                ]
                print(conversation)
                streaming_answer = baseChatGpt.post_and_get_streaming_answer(conversation, _parameters, full_history)
                answer = ''
                answer_head = '[第{0}版摘要心智圖]\n\r'.format(cnt + 1)
                while True:
                    try:
                        answer, full_history = next(streaming_answer)

                        if "摘要心智圖:\n```\n" in answer:
                            answer=answer.replace("摘要心智圖:\n```\n","")
                        if "輸入文字內容:\n```\n" in answer:
                            answer=answer.replace("輸入文字內容:\n```\n","")
                            break
                ...
                available_tokens = int(
                    (baseChatGpt.MAX_TOKENS - 200 - estimate_used_tokens(answer) - this_system_tokens - 4 - 2) * 0.667)
                cnt += 1
...
if __name__ == '__main__':
    PORT = 7860
    title = """<h1 align="center">🔥🏴‍☠️ Liteon Prompt is All You Need! 🚣</h1>"""
    # if "OPENAI_API_KEY" not in os.environ:
    ...
    with gr.Blocks(title="Prompt is what you need!", css=advanced_css, analytics_enabled=False,
        ...
        baseChatGpt = GptBaseApi(model="azure 2023-03-15-preview")
                                ...
                                with gr.Tabs():
                                    ...
                                    with gr.TabItem("即時whisper"):
                                        with gr.Row():
                                            ...
                                            whisper_timestamp_checkbox = gr.Checkbox(label="附加時間戳", value=False,
                                                                                     scale=1)
                                ...
                                with gr.Column():
                                    with gr.Row():
                                        summary_radio = gr.Dropdown(
                                            ["滾動式整合摘要", "滾動式累加摘要", "平行分塊摘要", "心智圖摘要",
                                             "主題式摘要"], multiselect=False, label="摘要技術", type="index", value=0,
                                            interactive=True)
                                    with gr.Row():
                                        rolling_button = gr.Button("➡️", size='sm', scale=1)
                                        rolling_clear_button = gr.ClearButton([rolliing_source_file], value="🚮",
                                                                              size='sm', scale=1)
                                        rolling_cancel_button = gr.Button("🗙", size='sm', scale=1)

                        with gr.Row():
                            with gr.Column(scale=1):
                                large_inputs = gr.Text(label="來源文字", lines=30, max_lines=5000)
                            with gr.Column(scale=1):
                                summary_output = gr.Markdown(label="摘要", interactive=True)
        ...
        rolling_inputs_event = rolling_button.click(rolling_summary, [large_inputs, rolling_state, summary_radio],
        ...
        def save_file(contents, state):
            # fname = "summary_{0}.md".format(str(datetime.now()).replace(' ', '').replace(':', '').replace('-','').replace('.', ''))
    ...
    auto_opentab_delay()
    demo.queue(concurrency_count=3, api_open=False).launch(show_error=True, max_threads=200, share=True,server_port=PORT)
