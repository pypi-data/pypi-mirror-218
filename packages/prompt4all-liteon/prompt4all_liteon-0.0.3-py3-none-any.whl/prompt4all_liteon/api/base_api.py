...
from prompt4all_liteon.utils.chatgpt_utils import process_context, process_chat
from prompt4all_liteon.utils.tokens_utils import num_tokens_from_history, estimate_used_tokens
...
class GptBaseApi:
    def __init__(self, model="azure 2023-03-15-preview", temperature=0.5, system_message='所有內容以繁體中文書寫'):

        if model.startswith('azure '):
            self.API_MODEL = "gpt-3.5-turbo"  # model.replace('azure ','')
            self.API_TYPE = 'azure'
            openai.api_type = 'azure'
        else:
            self.API_MODEL = model
        if "api_version" in model_info[model]:
            openai.api_version = model.replace('azure ', '')

        self.BASE_URL = "https://ltc-to-openai.openai.azure.com/openai/deployments/gpt-35-turbo-0301/chat/completions?api-version=2023-03-15-preview"
        openai.api_base = self.BASE_URL  # model_info[model]["endpoint"].strip()
            ...
            try:
                for chunk in response.iter_lines():
                    try:
                        ...
                        if  len(chunk_decoded) >0 and'content' in  json.loads(chunk_decoded[6:])['choices'][0]['delta']:
