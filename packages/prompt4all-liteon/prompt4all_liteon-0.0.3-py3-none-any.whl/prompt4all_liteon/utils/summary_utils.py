...
from prompt4all_liteon.utils.regex_utils import is_numbered_list_member,extract_numbered_list_member
from prompt4all_liteon.utils.tokens_utils import estimate_used_tokens
__all__ = [ "get_rolling_summary_results", "get_last_ordered_index", "split_summary","text2markdown"]
    ...
    for i in range(len(lines)):
        if is_numbered_list_member(lines[i]):
            if extract_numbered_list_member(lines[i]).endswith('.'):
                ...
                lines[i] = ''.join(['        ']*len([ t for t in ex_value if t=='.']))+lines[i].lstrip()
