...
class JWTAuthenticationPolicy(CallbackAuthenticationPolicy):
    ...
    def get_claims(self, request):
        # print(f'JWTAuthenticationPolicy.get claims called')
        if self.http_header == "Authorization-X":
            try:
            ...
            if auth_type != self.auth_type:
                # print(f'JWTAuthenticationPolicy auth_type mismatch')
                return {}
        else:
            token = request.headers.get(self.http_header)
        if not token:
            return {}
        # print(f'JWTAuthenticationPolicy.get claims returning decoded jwt {self.jwt_decode(request, token)}')
...
class JWTCookieAuthenticationPolicy(JWTAuthenticationPolicy):
    def __init__(
        ...
        self.cookie_name = cookie_name or "Authorization-X"
        ...
        # print(f'Cookie Profile {self.cookie_profile.cookie_name}')
    ...
    def get_claims(self, request):
        # print(f'JWTCookieAuthenticationPolicy get_claims called {self.accept_header, self.header_first}')
        if self.accept_header:
            if self.header_first:
                claims = self.get_cookie_claims(request)
                if not claims:
                    return super().get_claims(request)
                else:
                    return claims
                # ternary... love em, hate em
