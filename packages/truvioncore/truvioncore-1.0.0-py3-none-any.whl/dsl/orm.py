...
class PyaellaDataModelMetaclass(type):
	...
	def __new__(meta, classname, bases, classDict, *, partition_by=None):
		...
		#. PyaellaDataModelMetaclass.__init__ classname BackgroundEventStatusHistory bases (<class 'pyaella.orm.PyaellaDataModel'>,) classDict {'__module__': 'Kasayama.domain', '__qualname__': 'BackgroundEventStatusHistory', '__doc__': '\n\tAn BackgroundEvent tracks the state of an background event.\n\t', '__init__': <function BackgroundEventStatusHistory.__init__ at 0x10711a5e0>}
...
def create_default_tablename(s, do_pluralize=True):
	...
	pyaella format for classes and tables names are:

		ClassName for a class
		class_names for a table

	This module function creates the default pyaella table name, with an
	attempt to handle plurarity... which can be very complicated.

	pyaella does a bit of side-effective magic if a Python class name ends
...
class QueryPlan(object):
	...
	def __init__(self, *models, **kwds):
		...
		if 'logicals' in kwds:
			...
			self._mdls = models
			self._daos = [~m for m in models]  # model objects must be pyaella logical models
