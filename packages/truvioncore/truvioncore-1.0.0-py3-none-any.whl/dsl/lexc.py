...
from dsl.dinj import get_lexical_tokens, LexicalToken, parse_lgcl_mdl_rl_name
...
def output_html_form():
    ...
    model_tmpl_file_lu = TemplateLookup(directories=['/Users/mat/Documents/pyaella/pyaella/server/defaults/templates/'])
    ...
    for n, dm in data_models.items():
        ...
        try:
            os.makedirs('/Users/mat/Documents/pyaella/Kasayama/svelte/src/models/', exist_ok=True)
        except:
            print(traceback.format_exc())
            pass

        output_js_file = os.path.join(
            '/Users/mat/Documents/pyaella/Kasayama/svelte/src/models/',
            n + '.html'
        )
...
if __name__ == '__main__':
    ...
    print('starting pyaella domain compilation')
    options = parseArgs()
    print('generating pyaella domain {options.OUTPUTFILE} from lexicon {options.LEXICON}')
    r = doit()
