...
class IdCoder__(object):
    """
        DEPRECATED
        IdCode is based on an id and recipe created by Paulo Scardine.
        Re-designed here as a class and callable with custom sequences
        for more obfuscation, usage and 64bit.
    ...
    def __init__(self, kseq=None, use_default=False):
        ...
        '''
        if not kseq and use_default:
            self._kseq = MASTER_KSEQ
        else:
            self._kseq = kseq if kseq else self._gen_kseq()
        klist = list(self._kseq)
        self._fw = {
            '00000': klist[0],
            '00001': klist[1]
        }
        ...
        for i in range(1, len(self._kseq) - 1):
            n = abs(i)
            length = int(math.ceil(math.log(int(n), 64)))
            p = bin(n)[2:].rjust(5 * length, '0')
            self._fw[p] = klist[i + 1]
            self._bw[klist[i + 1]] = p

    def _gen_kseq(self):
        s = string.ascii_letters[0:26] + string.digits
        n = 0
        st = []
        while n != len(s):
            st.append(s[n])
    ...
    def encode(self, n):
        ...
        '''
        if n > ULIMIT:
            raise Exception('Number too large to encode')
        n = abs(n)
        length = int(math.ceil(math.log(int(n), 64))) if n > 1 else 1
        p = bin(n)[2:].rjust(5 * (length * 2), '0')
        id_ = ''.join([self._fw[p[i * 5:i * 5 + 5]] for i in range(0, length * 2)])
        r_id = id_.strip(id_[0])
        if r_id == '':
            return id_[0]
        else:
            return id_[0] + r_id
    ...
    def decode(self, s):
        ...
        '''
        return int(''.join([self._bw[l] for l in str(s)]), 2)

    def __repr__(self):
        return self._kseq

__salt_text = """Salt, also known as table salt or rock salt (halite),
    is a crystalline mineral that is composed primarily
    of sodium chloride (NaCl), a chemical compound belonging
    to the larger class of ionic salts. It is essential for
    animal life, but can be harmful to animals and plants in
    excess. Salt is one of the oldest, most ubiquitous food
    ...
    unrefined salt (such as sea salt), refined salt (table salt),
    and iodized salt. It is a crystalline solid, white, pale
    pink or light gray in color, normally obtained from sea water
    or rock deposits. Edible rock salts may be slightly grayish
    in color because of mineral content.
    Because of its importance to survival, salt has often been
    considered a valuable commodity during human history. However,
    as salt consumption has increased during modern times, scientists
    have become aware of the health risks associated with high salt
    intake, including high blood pressure in sensitive individuals.
    Therefore, some health authorities have recommended limitations
    of dietary sodium, although others state the risk is
    minimal for typical western diets.
