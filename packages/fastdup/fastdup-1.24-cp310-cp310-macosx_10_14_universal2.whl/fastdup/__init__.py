...
if is_mac:
    if "3.8" not in sys_version and "3.9" not in sys_version:
        print(f"fastdup identified you are running on Python {sys_version} while on MACOS only python3.8 and python3.9 are supported")
        fastdup_capture_exception("Unsupported MACOS system", RuntimeError(f"Found unsupported MAC OS system {sys_version}"))
    else:
        print(
            f"fastdup identified you are running on supported Python using unsupported OS version {OS_SUBVER}. "
            f"We support 10.X to 12.X. Try to upgrade pip using `pip install -U pip` and try again.")
        fastdup_capture_exception("Unsupported MACOS version",
                                  RuntimeError(f"fastdup identified you are running on supported Python using unsupported OS version {OS_SUBVER}, we support 10.X to 12.X please reach out to support"))

elif is_linux:
    if "3.8" not in sys_version and "3.9" not in sys_version and "3.10" not in sys_version and "3.7" not in sys_version:
        print(f"fastdup identified you are running on Python {sys_version} while on Linux only python3.7-python3.10 are supported {OSVER}")
        fastdup_capture_exception("Unsupported Linux system", RuntimeError(f"fastdup identified you are running on Python {sys_version} while on Linux only python3.7-python3.10 are supported {OSVER}"))
    elif OSVER == "10.12" or OSVER == "7.9.2009":
        print(
            f"fastdup supports installation on Centos7, Redhat 4.8, Amazon Linux 2 via the release page (https://github.com/visual-layer/fastdup/releases). "
            f"Look for Centos 7 releases. You system is {OSVER}")
        ...
        print(
            f"fastdup identified you are running unsupported Linux OS version {OS_SUBVER}, we support Ubuntu 18-22, Centos7, Redhat 4.8, Amazon Linux 2, python3.7-python3.10. "
            f"The later 3 needs to be install via our gihtub release page.")
