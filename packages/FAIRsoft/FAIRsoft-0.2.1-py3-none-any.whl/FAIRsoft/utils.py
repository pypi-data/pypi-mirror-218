...
# import pymongo/json only if necessary
if os.getenv('STORAGE_MODE', 'db') == 'db':
...
def push_entry(tool:dict, collection:'pymongo.collection.Collection', log:dict):
    ...
    try:
        insertResult = collection.insert_one(tool)
    except Exception as e:
        log['errors'].append({'file':tool,'error':e})
        logging.error(f"ERROR while pushing to database the following entry: {tool}. Exception: {e}")
    else:
        log['n_ok'] += 1
    finally:
        return(log)

def update_entry(tool:dict, collection:'pymongo.collection.Collection', log:dict):
    '''Update tool to collection.
    ...
    if 'about' in tool.keys():
            tool['about'].pop('date', None)
    try:
        updateResult = collection.update_one({'@id': tool['@id']}, {'$set': tool}, upsert=True)
    except Exception as e:
        log['errors'].append({'file':tool,'error':e})
        logging.error(f"ERROR while pushing to database the following entry: {tool}. Exception: {e}")
...
def save_many(tools, output_file, log):
    '''Save tool to file.
