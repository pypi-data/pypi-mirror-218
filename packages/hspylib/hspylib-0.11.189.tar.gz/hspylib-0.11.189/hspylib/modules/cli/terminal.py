...
class Terminal(ABC):
    ...
    def restore() -> None:
        ...
        Terminal.set_attributes(show_cursor=True, auto_wrap=True, enable_echo=True)
    ...
    @classmethod
    def get_dimensions(cls, fallback: os.terminal_size = (80, 24)) -> Tuple[int, int]:
        ...
        if not sys.stdout.isatty():
            ...
            return fallback
        dim = os.get_terminal_size(fallback)
        return dim.lines, dim.columns

    @classmethod
    def get_cursor_position(cls, fallback: os.terminal_size = (0, 0)) -> Tuple[int, int]:
        """Get the terminal cursor position.
        inspiRED by: https://stackoverflow.com/questions/46651602/determine-the-terminal-cursor-position-with-an-ansi-sequence-in-python-3
        :return line, column
        """
        pos, buf = fallback, ""
