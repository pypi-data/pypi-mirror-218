...
class AMPL {
  ...
  */
  ...
  void writeTable(fmt::CStringRef tableName) {
   ...
   \param	filename	The name of the file to write; the first letter indicates which 
                    filetype to write (see the output of ``ampl -o?``)
   \param auxfiles   The auxiliary files to write. Most notably, 'cr' instructs
                     %AMPL to write out column and row names respectively
   \throws	PresolveException if the model is not exported because of the presolver
                              (most notably if the model is trivial)
   \throws  InfeasibilityException if the model is not exported because detected
                                   infeasible by the presolver
   */
