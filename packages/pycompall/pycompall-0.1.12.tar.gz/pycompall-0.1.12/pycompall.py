...
@click.option('--ignore-symlinks', multiple=True, help='Sometimes symlinks cause problems. Enable this flag to ignore symlinks.')
@click.argument('paths', type=click.Path(exists=True), nargs=-1)
def compile(paths, recursive, in_place, create_empty_init, exclude_files, exclude_dirs, ignore_symlinks):
    for path in paths:
        compile_command(
            Path(path),
            recursive=recursive,
