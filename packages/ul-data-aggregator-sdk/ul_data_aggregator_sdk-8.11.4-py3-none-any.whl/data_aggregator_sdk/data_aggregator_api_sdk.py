...
#         return self._api_base_station.request_get('/base-station').check().payload_raw    # type: ignore
    ...
    def get_bs_downlink_tasks(
        ...
        if only_active:
            ...
            filters.append({"name": "schedule_effective_date_from", "op": "gte", "val": f"{now}"})
            filters.append({"name": "schedule_effective_date_to", "op": "lte", "val": f"{now}"})
        if bs_ids:
            filters.append({"name": "bs_id", "op": "in", "val": tuple(bs_ids)})  # type: ignore
        if limit is not None:
            query['limit'] = limit
        if offset is not None:
            query['offset'] = offset
