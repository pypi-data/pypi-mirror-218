...
from ul_api_utils.validators.custom_fields import WhiteSpaceStrippedStr, PgTypeInt32, CronScheduleStr
...
#         return self._api_base_station.request_get('/base-station').check().payload_raw    # type: ignore
    ...
    def get_bs_downlink_tasks(
        self,
        task_current_status: Optional[DownlinkTaskStatus],
        only_active_tasks: Optional[bool],
        bs_id: Optional[UUID],
        ...
        if task_current_status:
            query['task_current_status'] = task_current_status
        if only_active_tasks:
            filters.append({"name": "schedule_effective_date_from", "op": "gte", "val": datetime.utcnow().isoformat()})
            filters.append({"name": "schedule_effective_date_to", "op": "lte", "val": datetime.utcnow().isoformat()})
        if bs_id:
            filters.append({"name": "bs_id", "op": "==", "val": f"{bs_id}"})
        if filters:
            filters = json.dumps(filters)  # type: ignore
        return self._api_device.request_get(
            '/bs-downlink-tasks',
