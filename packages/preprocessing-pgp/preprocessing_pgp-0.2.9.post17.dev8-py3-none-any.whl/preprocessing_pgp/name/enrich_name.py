...
import pandas as pd
from preprocessing_pgp.const import hdfs
...
class EnrichName:
    ...
    def __init__(
        ...
        self.fname_rb = pd.read_parquet(
            f"{name_rb_pth}/firstname_dict.parquet", filesystem=hdfs
        )
        self.mname_rb = pd.read_parquet(
            f"{name_rb_pth}/middlename_dict.parquet", filesystem=hdfs
        )
        self.lname_rb = pd.read_parquet(
            f"{name_rb_pth}/lastname_dict.parquet", filesystem=hdfs
        )
    ...
    def load_model(
        ...
        transformer.build_model(
            optimizer=keras.optimizers.Adam(learning_rate=config_dict["LEARNING_RATE"]),
    ...
    def refill_accent(self, name_df: pd.DataFrame, name_col: str) -> pd.DataFrame:
...
def enrich_clean_data(
    clean_df: pd.DataFrame, name_col: str, model_version: str = "trial-31"
) -> pd.DataFrame:
    """
    ...
    if clean_df.empty:
        return clean_df
    model_weight_path = f"{MODEL_PATH}/{model_version}/best_transformer_model.h5"
    vectorization_paths = (
        f"{MODEL_PATH}/{model_version}/vecs/source_vectorization_layer.pkl",
        f"{MODEL_PATH}/{model_version}/vecs/target_vectorization_layer.pkl",
    )
    model_config_path = f"{MODEL_PATH}/{model_version}/hp.json"
...
def prepare_name(
    ...
    final_data = pd.concat([data[orig_cols], final_data], axis=1)
...
def process_enrich(
    ...
    final_data.loc[
        (final_data["customer_type"] == "customer") & (final_data["final"].isna()),
        "customer_type",
    ] = None
    ...
    for col in name_ele_cols:
        final_data[col] = final_data[col].str.title()
