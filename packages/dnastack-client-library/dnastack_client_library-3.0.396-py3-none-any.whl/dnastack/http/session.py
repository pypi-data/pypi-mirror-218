...
class HttpError(RuntimeError):
    ...
    @property
    def trace_context(self) -> Span:
        return self.args[1]

    def __str__(self):
        response: Response = self.response
        if self.trace_context:
            return f'Trace {self.trace_context.trace_id}: HTTP {response.status_code}: {response.text}'
        else:
...
class HttpSession(AbstractContextManager):
    ...
    def submit(self,
        ...
        with trace_context.new_span() as sub_span:
            ...
            # sub_logger.warning(f'***** PANDA: Response/URL {url}')
            # sub_logger.warning(f'***** PANDA: Response/HTTP {response.status_code} ({len(response.text)}B)')
            # sub_logger.warning(f'***** PANDA: Response/Body:\n{response.text}')
