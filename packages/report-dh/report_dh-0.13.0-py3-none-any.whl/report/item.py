...
def story(name: str, link: Optional[str] = None):
    def actual_decorator(cls):
        ...
        item_id = Launch.create_report_item(
                ...
                parent_item=parent,
                type='story',
                description=f'[{name}]({link})' if link else cls.__doc__)
...
def attachment(*, name: str, attachment: Union[str, bytes], item: str = '', attachment_type: str, level: Literal["ERROR", "INFO", "DEBUG"] = "ERROR"):
    ...
    """
    # item_id = Launch.add_attachment_entry(message=name, level=level, item=item)
    # attachment_name = f"{name}_attachment"
    # Launch.add_item(attachment_name, item_id)
    item_id = Launch.add_attachment(item=item, message=name, level=level, attachment=attachment, attachment_type=attachment_type)
    print(Launch.items())