...
from client.client_utils import send_message, get_message
...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username, passwd, keys):
        ...
        try:
            ...
            if err.errno:
                LOGGER.critical(f'Connecton to server lost.')
                raise ServerError('Connecton to server lost')
            LOGGER.error(
                'Connection Timeout for userlist update')
    ...
    def connection_init(self, port, ip):
        ...
        with socket_lock:
            ...
            try:
                send_message(self.transport, presence_msg)
        ...
        while self.running:
            ...
            with socket_lock:
                ...
                # Проблемы с соединением
                    ...
                    LOGGER.debug(f'Connecton to server lost.')
                    self.running = False
                    self.connection_lost.emit()
                finally:
                    self.transport.settimeout(5)
