...
from sj_tool.fjsp.erp_next.constant import unlock_str
...
def transfer_sale_order(conn, owner="Administrator", **kwargs):
    ...
    sale_items_dict = select_data_from_table(
        ...
        where_params=(owner, unlock_str, 1),
        to_dict=True,
        dict_key="parent",
    )
            ...
            results.append(
                {
                    ...
                    "ORDER_ENTRY": item["creation"].strftime("%Y-%m-%d %H:%M:%S"),
...
def transfer_bom(conn, owner="Administrator", **kwargs):
    ...
    bom_children_dict = select_data_from_table(
        ...
        columns=["item_code", "qty_consumed_per_unit", "parent","operation"],
        where_clause=f"owner = %s and docstatus = %s",
        where_params=(owner, 1),
        to_dict=True,
        dict_key="parent",
...
def transfer_process_table(conn, owner="Administrator", **kwargs):
    ...
    process_table = select_data_from_table(
        conn, table_process, columns=["operationnumber", "name", "operation_name"], where_clause=f"owner = %s", where_params=(owner,)
    )
    results = []
    for row in process_table:
        results.append({"id": row["operationnumber"], "name": row["operation_name"]})
...
def transfer_machine_table(conn, owner="Administrator", **kwargs):
    ...
    machine_table = select_data_from_table(
        conn, table_machine, columns=["workstation_id", "name"], where_clause=f"owner = %s", where_params=(owner,)
    )

    results = []
    for row in machine_table:
        results.append({"id": row["name"], "name": row["workstation_id"]})
