...
def to_gantt_json(pool: FjspPool, save_folder: str, machine_rate: dict,
                  sorted_dict={"加弹": 1, "整经": 2, "经编": 3, "剖绒": 4, "前整": 5}):
    gantt_data = []
    i = 0
    ...
    for machine_id in sorted(pool.machine_dict.keys(), key=lambda x: (
            sorted_dict.get(pool.machine_dict[x].name[:2], 0), pool.machine_dict[x].name[-1])):
        machine_dict = {}
        ...
        for d in pd.date_range(min_day, max_day):
            calendar = (
                    machine_object.calendar.get(d-timedelta(days=1), {}).get(
                        "c_list", [{"s_t": d-timedelta(days=1), "e_t": d}])[-1:]
                    + machine_object.calendar.get(d, {}).get(
                "c_list", [{"s_t": d, "e_t": d + timedelta(days=1)}])
                    + machine_object.calendar.get(d+timedelta(days=1), {}).get(
                        "c_list", [{"s_t": d+timedelta(days=1), "e_t": d+timedelta(days=2)}])[:1]
            )
