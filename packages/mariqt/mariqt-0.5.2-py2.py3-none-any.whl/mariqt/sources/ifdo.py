...
class iFDO:
    ...
    def createiFDO(self, header: dict, items: list, updateExisting=False,headerOnly=False,allow_missing_required=False):
    ...
    def _set_rep_header_fields_dt_lat_lon_alt(self,ifdo:dict):
        """ Fill header fields image-datetime, -latitude, -longituede, -altitude-meters, if empty,
        representatively with values from first item. """
        first_image = sorted(ifdo[self.imageSetItemsKey].keys())[0]
        rep_header_fields = ['image-datetime','image-latitude','image-longitude','image-altitude-meters']
        for field in rep_header_fields:
        ...
        if len(image_dts) > 0:
            ...
            for file in image_dts:
                for timepoint in image_dts[file]:
                    ...
                    if has_alt:
                        val = timepoint.dep
                        if 'dep' in nav_header:
                            val *= -1
                            if 'dep' in records2beInverted:
                                val *= -1
                        elif  'alt' in nav_header and 'alt' in records2beInverted:
                            val *= -1
                        res.write("\t"+str(val))
                    if hgt_not_zero and 'hgt' in nav_header:
                        val = timepoint.hgt
    ...
    def createImageNavigationFile(self, nav_path: str, nav_header=miqtv.pos_header['pangaea'], date_format=miqtv.date_formats['pangaea'], overwrite=False, col_separator = "\t", videoSampleSeconds=1,
        ...
        self.create_image_navigation_file(nav_path, nav_header, date_format, overwrite, col_separator, videoSampleSeconds, offset_x, offset_y, offset_z, angles_in_rad, records2beInverted)
