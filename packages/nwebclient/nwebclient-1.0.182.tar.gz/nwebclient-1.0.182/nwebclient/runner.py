...
class JobRunner(base.Base):
    ...
    def execute_rest(self, port=8080, run=True, route='/', app=None):
        ...
        app.add_url_rule(route, 'job_runner', view_func=lambda: json.dumps(self.execute_data(request.form.to_dict())))
        if run:
...
if __name__ == '__main__':
    args = util.Args()
    print("Usage: "+sys.executable+" -m nwebclient.runner --executor module:Class --in in.json --out out.json")
    executor = args.getValue('executor')
    if executor is None:
        print("No executor found.")
        exit(1)
    print("Executor: " + executor)
    runner = JobRunner(util.load_class(executor, create=True))
