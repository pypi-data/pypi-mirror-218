...
class LookupTable:
    ...
    @staticmethod
    def __ConvertUnicodeValue(value:str) -> bytearray:
        """
        ...
        return BinaryFormatter._EncodeString(value)
    ...
    def GetBytesValue(self, key:str, size:int, defaultValue:bytearray) -> bytearray:
        ...
        if (value != None):
            ...
            valuebytes:bytearray = LookupTable.__ConvertUnicodeValue(value.strip())
    ...
    def GetColorValue(self, key:str, defaultValue:Color) -> Color:
        """
    ...
    def GetIntegerValue(self, key:str, defaultValue:int) -> int:
        ...
        if (value != None):
            value = value.strip();
            if (LookupTable.IsValidInteger(value)):
                try:
                    result = int(value)
    ...
    def GetLevelValue(self, key:str, defaultValue:Level) -> Level:
        """
        ...
        available values.  
        """
        result:Level = defaultValue;
        value:str = self.GetStringValue(key, None);

        if (value != None):
            result = Level.Parse(value, defaultValue)
        return result;


    def GetRotateValue(self, key:str, defaultValue:FileRotate) -> FileRotate:
        """
        Returns a value of an element converted to a FileRotate value for
        a given key.
        ...
        This method returns the defaultValue argument if either the
        supplied key is unknown or the found value is not a valid FileRotate
        value. Please see the FileRotate enum for more information on the
        available values.  
        """
        result:FileRotate = defaultValue;
        value:str = self.GetStringValue(key, None);

        if (value != None):
            result = FileRotate.Parse(value, defaultValue)
        return result;
    ...
    def GetSizeValue(self, key:str, defaultValue:int) -> int:
        ...
        result:int = defaultValue * LookupTable._KB_FACTOR;
        value:str = self.GetStringValue(key, None);

        if (value != None):
            value = value.strip().lower()
            factor:int = LookupTable._KB_FACTOR

            if (len(value) >= 2):
                unit = value[-2:]
                if (LookupTable.IsValidSizeUnit(unit)):
                    value = value[0:len(value)-2]
                    if (unit == "kb"):
                        factor = LookupTable._KB_FACTOR
                    elif (unit == "mb"):
                        factor = LookupTable._MB_FACTOR
                    elif (unit == "gb"):
                        factor = LookupTable._GB_FACTOR
    ...
    def GetTimespanValue(self, key:str, defaultValue:int) -> int:
        ...
        result:int = defaultValue * LookupTable._SECONDS_FACTOR;
        value:str = self.GetStringValue(key, None);

        if (value != None):
            value = value.strip().lower()
            factor:int = LookupTable._SECONDS_FACTOR

            if (len(value) >= 1):
                unit = value[-1:]
                if (LookupTable.IsValidTimespanUnit(unit)):
                    value = value[0:len(value)-1]
                    if (unit == "s"):
                        factor = LookupTable._SECONDS_FACTOR
                    elif (unit == "m"):
                        factor = LookupTable._MINUTES_FACTOR
                    elif (unit == "h"):
                        factor = LookupTable._HOURS_FACTOR
                    elif (unit == "d"):
                        factor = LookupTable._DAYS_FACTOR
