...
class FileHelper:
    ...
    @staticmethod
    def __ExpandFileName(baseName:str) -> str:
        """
        ...
        while (os.path.isfile(result)):
            ...
            result:str = filePrefix + \
                         FileHelper.ALREADY_EXISTS_SUFFIX + \
                         fileExtn
    ...
    @staticmethod
    def __FindFileName(baseName:str) -> str:
        """
        ...
        """
        files:list[str] = FileHelper.__GetFiles(baseName)
    ...
    @staticmethod
    def __GetFiles(baseName:str) -> list[str]:
        """
        ...
        # __IsValidFile) and ignore all others.
        ...
        return FileHelper.__ValidateFiles(baseName, files)


    @staticmethod
    def __IsValidFile(baseName:str, path:str) -> bool:
        """ 
        ...
        fileDateFound,fileDate = FileHelper.__TryGetFileDate(baseName, path)
        return fileDateFound


    @staticmethod
    def __TryGetFileDate(baseName:str, path:str) -> bool:
        """ 
        ...
        if (len(value) > FileHelper.DATETIME_FORMAT_LEN):
            value = value.Substring(0, FileHelper.DATETIME_FORMAT_LEN)

        # try to create a datetime object from the file timestamp.
        fileDate:datetime
        fileDateParsedOk:bool = False
        fileDateParsedOk,fileDate = FileHelper.__TryParseFileDate(value)
    ...
    @staticmethod
    def __TryParseFileDate(fileDate:str) -> bool:
        """
        ...
        if (len(fileDate) != FileHelper.DATETIME_FORMAT_LEN):
            return False, None
    ...
    @staticmethod
    def __ValidateFiles(baseName:str, files:list[str]) -> list[str]:
        """
        ...
        for i in range(len(files)):
            ...
            if (FileHelper.__IsValidFile(baseName, files[i])):
                valid = valid + 1
            else:
                files[i] = ""       # See below
    ...
    def GetFileName(baseName:str, append:bool) -> str:
        ...
        if (append):
            ...
            fileName:str = FileHelper.__FindFileName(baseName)
        ...
        return FileHelper.__ExpandFileName(baseName)
