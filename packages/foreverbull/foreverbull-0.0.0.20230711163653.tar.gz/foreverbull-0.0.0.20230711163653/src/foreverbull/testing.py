...
class Backtest:
    def __init__(self, service_name: str):
        ...
        self._service: models.service.Service = None

    def __enter__(self):
        self._service = broker.service.get(self._service_name)
        if self._service is None:
            raise Exception(f"service {self._service_name} does not exist")

        self._backtest = broker.backtest.create(self._service_name)
        while self._backtest["stage"] != "RUNNING":
            if self._backtest["error"]:
                raise Exception("backtest failed to start: ", self._backtest["error"])
            time.sleep(0.2)
            self._backtest = broker.backtest.get(self._backtest["id"])
    ...
    def ingest(self, ingest_config: models.backtest.IngestConfig):
        self.process(models.service.Request(task="ingest", data=ingest_config))

    @contextmanager
    def new_execution(self, execution: models.backtest.Execution, **routes) -> Execution:
        execution.backtest_service = self._service_name
