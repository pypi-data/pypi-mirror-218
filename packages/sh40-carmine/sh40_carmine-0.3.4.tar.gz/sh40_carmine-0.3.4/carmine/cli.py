...
from typing import Type, Any, NamedTuple, TypeVar, Optional
from types import TracebackType, FrameType, UnionType
...
def carmine_cli(
    ...
    try:
        ...
        if command is EMPTY_COMMAND:
            ...
            prefix = f"error executing "

        else:
            prefix = f"error executing {command.real_name!r}"

        line = prefix + exc.message + ": "
        ...
        print(
            "\n".join(
                wrap(
                    ...
                    subsequent_indent=min(line.index(":"), line.index("'")) * " "
                    + 2 * INDENT,
                )
            )
        )
