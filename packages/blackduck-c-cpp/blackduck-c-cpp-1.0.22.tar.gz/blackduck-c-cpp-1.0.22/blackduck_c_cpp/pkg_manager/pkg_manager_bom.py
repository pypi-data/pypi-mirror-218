...
class PkgManagerBom:
    ...
    def pkg_mgr_batch_process(self, batch_list):
        ...
        with concurrent.futures.ProcessPoolExecutor(self.MAX_WORKERS) as executor:
            futures = {executor.submit(self.run_pkg_query, paths_list): basefile for basefile,paths_list in batch_dict.items()}
            for future in concurrent.futures.as_completed(futures):
                key = futures[future]
                result = future.result()
                # result[3] - True if found package manager result, else False
                if result[3]:
                    # result[0] is sha1sum of file, result[1] is confidence and result[2] is src, pkg,version and architecture information
                    pkg_result = self.join_components(result[0], result[1], result[2], MatchType)
