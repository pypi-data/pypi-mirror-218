...
class TaskScheduler:
    ...
    async def _schedule_task(self, wrapper, queue):
        try:
            ...
            await asyncio.sleep(.5)  # wait for 500 ms
            try:
                # re-enqueue task
            ...
            # retry enqueue task:
            self.logger.warning(
                f"Task {wrapper!r} was missed for enqueue after {SCHEDULER_WORKER_TIMEOUT} sec"
            )
            await asyncio.sleep(5)  # wait for 5 seconds
            try:
                self.logger.warning(
                    f"Task {self.program}.{self.task}:{wrapper.id}, retrying enqueue after 5 seconds"
                )
                return await queue.queue(wrapper)
            except asyncio.TimeoutError:
                raise
            ...
            try:
                self.logger.warning(
                    f"Task {self.program}.{self.task}:{wrapper.id}, retrying enqueue after 10 seconds"
                )
                return await queue.queue(wrapper)
            except asyncio.TimeoutError:
                raise
