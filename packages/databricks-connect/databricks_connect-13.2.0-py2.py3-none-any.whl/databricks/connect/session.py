...
from .cache import HashableDict, cached
...
class DatabricksSession:
    ...
    class Builder:
        ...
        def getOrCreate(self) -> RemoteSparkSession:
            ...
            if self._profile is not None:
                logging.debug("SDK Config profile is explicitly configured")
        ...
        @staticmethod
        @cached(lambda: os.environ["SPARK_REMOTE"])
        def _from_spark_remote():
            logging.debug("Creating SparkSession from Spark Remote")
            return SparkSession.builder.create()

        @staticmethod
        @cached(lambda s: s)
        def _from_connection_string(conn_string):
            logging.debug("Creating SparkSession from connection string")
        ...
        @staticmethod
        @cached(_cache_from_sdk_config)
        def _from_sdkconfig(config: Config, user_agent: str,
                            headers: dict[str, str]) -> RemoteSparkSession:
            ...
            logging.debug(f"Creating SparkSession from SDK config: {config}")
