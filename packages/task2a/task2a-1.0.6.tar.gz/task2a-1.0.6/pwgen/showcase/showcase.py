...
def print_output(command: str) -> None:
    if not command:
        return

    print('\nAccording to your choice, the command to be invoked will be:')
    ...
    if result.stdout.strip():
        print('\nThe output is:')
        questionary.print(f'{result.stdout}', style='#009b06')
...
def packet_output(command_key_name: str = 'command') -> None:
    ...
    [print_output(el) for el in command_packet]
