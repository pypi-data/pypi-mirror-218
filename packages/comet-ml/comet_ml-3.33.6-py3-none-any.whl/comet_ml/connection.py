...
from ._typing import IO, Any, Callable, Dict, List, Mapping, Optional, Tuple, Type
...
from .messages import StandardOutputMessage, UploadFileMessage, UploadInMemoryMessage
...
from .s3.multipart_upload.rest_api_assets_helper import S3MultipartRestApiAssetsHelper
...
def get_retry_strategy():
    ...
    return Retry(
        total=3, backoff_factor=2, status_forcelist=[500, 502, 503, 504], **kwargs
    )  # Will wait up to 24s
    ...
    def log_experiment_remote_model(
        ...
        try:
            ...
            if on_failed_model_upload is not None:
                try:
                    on_failed_model_upload(sys.exc_info())
                except Exception:
                    LOGGER.warning(
...
def upload_s3_multipart_file(
    ...
    try:
        ...
        if options.on_failed_asset_upload is not None:
            try:
                options.on_failed_asset_upload(sys.exc_info())
            except Exception:
...
def upload_file(
    ...
    try:
        ...
        LOGGER.error(
            "File upload failed: %r, file: %s", e, options.file_path, exc_info=True
        )
        Reporting.report(
            event_name=FILE_UPLOADED_FAILED,
            experiment_key=options.experiment_id,
