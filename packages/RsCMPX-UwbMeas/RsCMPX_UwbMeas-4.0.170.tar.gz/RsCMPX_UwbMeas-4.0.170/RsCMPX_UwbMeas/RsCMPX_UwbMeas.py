...
class RsCMPX_UwbMeas:
	"""533 total commands, 5 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMP/CMX/CMW, SupportedIdnPatterns = CMP/CMX/CMW, SimulationIdnString = 'Rohde&Schwarz,CMP,100001,4.0.170.0018'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.0.170.0018'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.170.0018'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_UwbMeas version failed. Current version: '4.0.170.0018', minimum required version: '{min_version}'")
