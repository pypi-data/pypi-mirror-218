...
class OperatingUnit:
    def __init__(
        ...
        self.hide_travel_details = hide_travel_details
    ...
    def add_travel_details(self):
        # Filter to only travel expenses ('Account' column in TRAVEL)
        df_travel = self.df[
            (self.df["Account"].isin(CODES_TRAVEL)) & (self.df["JRNL Line Ledger"] == "ACTUALS")
        ].sort_values("Accounting Period")

        for index, row in df_travel.iterrows():
            data = {
                "Type": [
                    f"Line {index+2}: {row['Account Descr']} - {row['PINV Business Purpose']} - {row['JRNL Line Descr']} - {row['JH Descr']}"
                ],
                **{
                    calendar.month_abbr[month]: (
