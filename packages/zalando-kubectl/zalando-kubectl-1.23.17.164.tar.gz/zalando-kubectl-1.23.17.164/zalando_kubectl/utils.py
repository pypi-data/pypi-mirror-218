...
class ExternalBinary:
    ...
    def download(self):
        ...
        if self.name == "kubectl" and platform == "darwin":
            if binary.exists():
                kubectl_version = check_output([str(binary), "version", "--client", "--short"], cwd=Path.cwd())
                return str(binary) if "1.21.3" in kubectl_version.decode("UTF-8") else os.remove(binary)

            filename = resource_filename(__name__, "binaries/kubectl")
            with open(filename, "rb") as f:
                data = f.read()
                self.create_binary(binary, data)
            return str(binary)
    ...
    def run(self, args, input=None, stdout=None, stderr=None, check=False, forward_context=True):
