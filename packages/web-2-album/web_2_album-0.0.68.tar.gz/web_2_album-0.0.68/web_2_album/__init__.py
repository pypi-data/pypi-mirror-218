...
def preLoad(img):
	if 'img2.doubanio.com' not in img:
		return
	cached_url.get(img, mode='b', headers={'referer': 'https://www.douban.com/'}, force_cache = True)
...
def get(path, force_cache=False, content = None):
	...
	[preLoad(img) for img in result.imgs]
	result.cap = getCap(b, path)
	result.video = getVideo(b)
	result.url = path
	if 'douban.com' in result.url:
