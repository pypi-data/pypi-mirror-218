...
def watchdog_entrypoint(
    ...
    **kwargs,
    ...
    del kwargs
    return watchdog(run_name=run_name, disable=disable)
...
def watchdog(
    ...
    **kwargs,
    ...
    if disable:
        update_run(run, {'retryOnSystemFailure': False})
        logger.info(f'{OK} Disabled watchdog for run: {run_name}. If a system failure occurs, '
                    'your run will not be retried.')
        return 0

    update_run(run, {'retryOnSystemFailure': True})
    logger.info(f'{OK} Enabled watchdog for run: {run_name}. If a system failure occurs, your run '
