...
def creat_template(client: Elasticsearch, name: str, index_patterns: list, map_fields: dict, is_stream=False, number_of_shards=4, mmdb=None) -> bool:
    """
    创建 es 模版, 可能会新建同名管道
    :param client: es 客户端
    :param name: 模版名称
    ...
    :param mmdb: geo信息使用的 database_file, None代表使用默认 (一般是 GeoLite2-City.mmdb)
    :return: 是否成功
    """
...
def define_map_fields():  # 例子
    return {
        'record_date': {  # es索引名
            ...
            'default': None,  # 默认的提取值
            'es_type': 'date',  # 对应的es类型, 例如 date/boolean/float/integer/long/text/keyword/ip/doubole
        ...
        'text': {  # 
            ...
            'default': None,
            'es_type': 'text',
            'es_keyword': True,  # 针对 text 类型可以选择是否将改文本当关键词处理
            'note': '',
        },
    }
