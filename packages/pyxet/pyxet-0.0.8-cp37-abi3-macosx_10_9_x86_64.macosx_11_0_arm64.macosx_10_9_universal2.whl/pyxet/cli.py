...
def single_file_copy(src_fs, src_path, dest_fs, dest_path,
...
def copy(source, destination, recursive = True, _src_fs=None, _dest_fs=None):
    src_fs, src_path = get_fs_and_path(source)
    dest_fs, dest_path = get_fs_and_path(destination)
    ...
    single_file_copy(src_fs, src_path, dest_fs, dest_path)


def root_copy(source, destination, message, recursive=False):
    dest_fs, dest_path = get_fs_and_path(destination)
...
class PyxetCLI:
    ...
    def mount(source: Annotated[str, typer.Argument(help="Repository and branch of the form xet://user/repo/branch")],
        ...
        if os.name == 'nt':
            ...
            path = letter
        rpyxet.perform_mount(source.remote, path, source.branch)
    ...
    @staticmethod
    @cli.command()
    def clone(source: Annotated[str, typer.Argument(help="Repository and branch of the form xet://user/repo/branch")],
              args: Annotated[list[str], typer.Argument(help="Arguments to be passed to git-xet clone")] = None):
        """
    ...
    def cp(source: Annotated[str, typer.Argument(help="Source file or folder which will be copied")],
        ...
        root_copy(source, target, message, recursive=recursive)
    ...
    def mv(source: Annotated[str, typer.Argument(help="Source Xet file or folder to move")],
        ...
        src_fs, src_path = get_fs_and_path(source)
        dest_fs, dest_path = get_fs_and_path(target)
...
class BranchCLI:
    ...
    def make(repo: Annotated[str, typer.Argument(help="a repository name <user>/<project>")],
        ...
        """
        fs, remote = get_fs_and_path(repo)
...
class RepoCLI:
    @staticmethod
    @repo.command()
    def make(name: Annotated[str, typer.Argument(help="repository name")]):
        """
        make a new empty public repository.
        """
        print("")
        print("Note: The repository created is public by default.")
        print("You can change it to private in the repository settings on https://xethub.com")
        print("")
        fs = XetFS()
