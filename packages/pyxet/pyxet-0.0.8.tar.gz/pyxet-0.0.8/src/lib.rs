...
#[pyfunction]
pub fn perform_mount(remote: String, path: String, reference: String) -> PyResult<()> {
...
impl PyRepo {
    ...
    pub fn begin_write_transaction(&self, branch: &str) -> PyResult<PyWriteTransaction> {
...
impl PyWriteTransaction {
    ...
    pub fn commit(&mut self, message: &str) -> PyResult<()> {
        ...
        Ok(exec_async_anyhow!(transaction.commit(message).await))
    }
