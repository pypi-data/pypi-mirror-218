...
class IdentitySchema(KadiSchema):
    ...
    def _has_visible_email(self, obj):
        # If within a request context, only mark the email address as visible if the
        # current user is a sysadmin, matches the user to be serialized or set their
        # email to be publicly visible.
        return not has_request_context() or (
            current_user.is_sysadmin
            or obj.user == current_user
            or not obj.user.email_is_private
        )
