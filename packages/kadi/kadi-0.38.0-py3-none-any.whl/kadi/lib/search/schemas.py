...
class SavedSearchSchema(KadiSchema):
    ...
    # Only allow one of the searchable models as object type.
    object = FilteredString(required=True, validate=OneOf(list(const.RESOURCE_TYPES)))

    query_string = FilteredString(
        required=True,
        allow_ws_only=True,
        filters=[strip],
        validate=Length(
