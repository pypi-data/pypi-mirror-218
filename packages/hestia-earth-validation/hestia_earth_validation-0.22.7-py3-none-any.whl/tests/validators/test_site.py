...
def test_validate_valid(*args):
    with open(f"{fixtures_path}/site/valid.json") as f:
        node = json.load(f)
    assert validate_site(node) == [True] * 33
...
@patch(f"{class_path}.fetch_data_by_coordinates", return_value={'mean': 25})
...
@patch(f"{class_path}.fetch_data_by_coordinates", return_value={'mean': 30})
