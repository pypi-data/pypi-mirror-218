...
from hestia_earth.validation.validators.product import (
    ...
    validate_product_ha_functional_unit_ha, validate_product_yield, validate_liveAnimal_requires_excreta
)
...
def test_validate_liveAnimal_requires_excreta_valid():
    # no products should be valid
    assert validate_liveAnimal_requires_excreta({}, 'products') is True

    with open(f"{fixtures_path}/product/liveAnimal/excreta/valid.json") as f:
        data = json.load(f)
    assert validate_liveAnimal_requires_excreta(data) is True


def test_validate_liveAnimal_requires_excreta_invalid():
    with open(f"{fixtures_path}/product/liveAnimal/excreta/invalid.json") as f:
        data = json.load(f)
    assert validate_liveAnimal_requires_excreta(data) == {
        'level': 'error',
        'dataPath': '.products',
