...
def test_validate_valid():
    with open(f"{fixtures_path}/cycle/valid.json") as f:
        node = json.load(f)
    assert validate_cycle(node) == [True] * 78
...
def test_validate_siteDuration_valid():
    ...
    with open(f"{fixtures_path}/cycle/siteDuration/valid-otherSites.json") as f:
        data = json.load(f)
    assert validate_siteDuration(data) is True
...
def test_validate_single_linked_impact_assessment_valid():
    with open(f"{fixtures_path}/cycle/product-single-ia/cycle.json") as f:
        cycle = json.load(f)
    with open(f"{fixtures_path}/cycle/product-single-ia/valid.json") as f:
...
def test_validate_single_linked_impact_assessment_invalid():
    with open(f"{fixtures_path}/cycle/product-single-ia/cycle.json") as f:
        cycle = json.load(f)
    with open(f"{fixtures_path}/cycle/product-single-ia/invalid.json") as f:
        nodes = json.load(f).get('nodes')
