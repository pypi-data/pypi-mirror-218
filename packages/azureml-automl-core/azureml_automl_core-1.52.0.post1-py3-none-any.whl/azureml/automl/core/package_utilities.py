    ...
    for pkg in output_json:
        ...
        if pkg_name not in added_set and pkg_name not in constants.Dependencies.PACKAGE_DELETION_LIST:
            ...
            if is_valid_conda_pkg or pkg_channel in constants.Dependencies.CONDA_CHANNELS_LIST:
                ...
                # Vision training runs use torch==1.11.0+cu113 and torchvision==0.12.0+cu113 from the extra-index-url
                # https://download.pytorch.org/whl/cu113. When these packages are exported in model as
                # torch==1.11.0+cu113, batch deployment fails. Batch deployment uses training image as base image and
