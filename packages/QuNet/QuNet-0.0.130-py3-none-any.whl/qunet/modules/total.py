...
class ShiftFeatures(nn.Module):
    ...
    def forward(self, x):           # (B,E) or (B,T,E) or (B,E,H,W) or (B,E,D,H,W)
        ...
        if x.dim() == 2:
            B,E = x.shape
            return x + self.std * torch.randn((B,E), device=x.device)
        if x.dim() == 3:
            B,_,E = x.shape
            return x + self.std * torch.randn((B,1,E), device=x.device)
        if x.dim() == 4:
            B,C,_,_ = x.shape
            return x + self.std * torch.randn((B,C,1,1), device=x.device)
        if x.dim() == 5:
            B,C,_,_,_ = x.shape
            return x + self.std * torch.randn((B,C,1,1,1), device=x.device)
