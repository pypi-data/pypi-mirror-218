...
from .total        import get_activation, get_norm, get_dropout, get_model_layers
...
class ResidualConvBlock(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3,  bias=False, stride=1, drop_d=0, norm=1, num = 2,  fun="relu"):
        """
        ...
        for i in range(num):
            ...
            if i+1 < num:
                if fun:
                    layers.append( get_activation(fun) )
                if drop_d:
                    layers.append( get_dropout(drop_d) )
...
class CNN(nn.Module):
    ...
    def get_blocks(self, token, channels, cfg):
        ...
        if token[0] == 'r' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block= Residual(
                    ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, bias=cfg.bias, norm=cfg.norm, fun=cfg.fun, drop_d=cfg.drop_d_inside),
                    ...
                    drop_d = cfg.drop_d_after, dim=2, name=token )
            channels = Eout
            return [block], channels
        ...
        if token[0] == 'b' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block = ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, bias=cfg.bias, norm=cfg.norm, fun=cfg.fun, drop_d=cfg.drop_d_inside)
            block.name = token
            channels = Eout
            return [block], channels
    ...
    def set_dropout(self, p=0.):
        ...
        """
        layers = get_model_layers(self, kind=(nn.Dropout, nn.Dropout1d, nn.Dropout2d, nn.Dropout3d))
    ...
    def default():
        return copy.deepcopy(Config(
            ...
            bias   = False,
            norm   = 1,
            drop_d = 2,          # тип Dropout для всех токенов `d`
            drop_d_inside = 0,   # тип Dropout между Conv2d в блоках (токены 'r', 'b')
            drop_d_after  = 2,   # тип Dropout после бллоков (токены 'r', 'b')
            fun    = 'relu',
            res    = 1,
            gamma  = 0.,
            avg    = True,
