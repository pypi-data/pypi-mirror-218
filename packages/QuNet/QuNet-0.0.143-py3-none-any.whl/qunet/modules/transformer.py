...
class  TransformerBlock(nn.Module):
    ...
    def default():
        return copy.deepcopy(Config(
            ...
            mlp = Config(MLP.default(), stretch=4, mult=0),
            fft = FFT.default(),
        ))
    ...
    def create(self):
        ...
        layers = []
        if cfg.is_fft:
            layers.append( Residual(FFT(cfg.fft),       E=cfg.att.E, mult=cfg.mult,  scale=cfg.scale,  norm_before=cfg.norm, name="fft") )
        if cfg.is_att:
            layers.append( Residual(Attention(cfg.att), E=cfg.att.E, mult=cfg.mult,  scale=cfg.scale,  norm_before=cfg.norm, name="att") )
        if cfg.is_mlp:
            layers.append( Residual(MLP(cfg.mlp),       E=cfg.att.E, mult=cfg.mult,  scale=cfg.scale,  norm_before=cfg.norm, name="mlp") )
...
class  Transformer(nn.Module):
    ...
    def __init__(self,  *args, **kvargs) -> None:
        ...
        mult = kvargs['mult'] if 'mult' in kvargs else (self.cfg.mult if self.cfg.has('mult') else self.cfg.block.att.mult)
        self.cfg.block.fft.mult = self.cfg.block.att.mult = self.cfg.block.mlp.mult = mult
    ...
    def default():
        return copy.deepcopy(Config(
            ...
            mult       = 1,            
            scale     = 0.1,   # initial value of the learning residual multiplier
    ...
    def unit_test():
        B, T, E, H = 1, 10, 128, 16
        cfg = Config(E=E, H=H, n_blocks=2, is_fft=1, is_att=1, res=0, scale=0.)
        m = Transformer(cfg)
