...
class CNN(nn.Module):
    ...
    def get_blocks(self, token, channels, cfg):
        ...
        if token[0] == 'r' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block= Residual(
                    Conv2dBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm_inside, norm_block=cfg.norm_block, fun=cfg.fun_inside, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside),
        ...
        if token[0] == 'b' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block = Conv2dBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm_inside, norm_block=cfg.norm_block, fun=cfg.fun, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside)
            block.name = token
            channels = Eout
            return [block], channels
