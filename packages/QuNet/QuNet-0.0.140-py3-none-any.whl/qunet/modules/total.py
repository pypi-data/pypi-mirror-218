...
class Scaler(nn.Module):
    def __init__(self, kind=1, scale=1, dim=1,  E=None):
        super().__init__()        
        self.kind = kind
        if   kind == 3:                # training multiplier for each components
            if dim == 2:
                self.scale = nn.Parameter( torch.empty(1, E, 1,1).fill_(float(scale)) )
            else:
                self.scale = nn.Parameter( torch.empty(1, 1, E ).fill_(float(scale)) )
        elif kind == 2:                # training common multiplier
            self.scale = nn.Parameter( torch.tensor(float(scale)) )     # 0 ??? 
        else:                         # constant multiplayer
            self.register_buffer("scale", torch.tensor(float(scale)) )  # 1 or ...
    #---------------------------------------------------------------------------        
