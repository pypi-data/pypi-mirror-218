...
class ResidualAlign(nn.Module):
    ...
    def __init__(self, Ein, Eout, norm, stride=1, dim=1):
        ...
        if Ein == Eout and stride==1:
            self.layer = nn.Identity() 
        else:
            layers = []
            assert dim in [1,2], f"Wrong dim={dim} in ResidualAlign  {Ein}->{Eout}, stride={stride}"
...
class Residual(nn.Module):
    ...
    (mult==1): scale = 1; (mult==2) scale - trainable scalar; (mult==3) scale - trainable vector(E)
    ```
    """
    def __init__(self, block, E, Eout=None, stride=1, 
        ...
        self.scaler = Scaler(kind=mult, scale=scale, dim=dim, E=E)

        self.align = ResidualAlign(E, Eout, norm_align, stride, dim)        
