...
class ResidualConvBlock(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3,  bias=False, stride=1, stride_first=True,
                drop_d=0, shift=0, norm=1, norm_block=1, num=2,  fun="relu"):
        """
...
class CNN(nn.Module):
    ...
    def default():
        return copy.deepcopy(Config(
            ...
            mult          = 1,      # тип множителя scale (1-константа, 2-обучаемое число, 3-обучаемый вектор)
    ...
    def get_blocks(self, token, channels, cfg):
        ...
        if token[0] == 'r' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block= Residual(
                    ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm_inside, norm_block=cfg.norm_block, fun=cfg.fun_inside, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside),
        ...
        if token[0] == 'b' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block = ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm_inside, norm_block=cfg.norm_block, fun=cfg.fun, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside)
            block.name = token
            channels = Eout
            return [block], channels
