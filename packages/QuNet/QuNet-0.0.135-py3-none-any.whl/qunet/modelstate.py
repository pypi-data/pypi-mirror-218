...
class ModelState:
        ...
        for lr in self.__layers:
            ...
            if count:
                ...
                if layer == "Linear":                       
                    ...
                    elif info > 1: descr += f"({mo.in_channels}->{mo.out_channels}, k:{mo.kernel_size}, s:{mo.stride}, p:{mo.padding}, {'T' if mo.bias is not None else 'F'})"                    
                    ...
                    elif info > 1: descr += f"(k:{mo.kernel_size}, s:{mo.stride}, p:{mo.padding})"
    ...
    def layers(self, info=1, is_names=True, input_size=None, input_data=None):
        ...
        if is_input:
            ...
            inp_w = max( [len(str(ln['input']))  for ln in self.__layers ])
            out_w = max( [len(str(ln['output'])) for ln in self.__layers ])
        ...
        for ln in self.__layers:
            ...
            if input_size is not None or  input_data is not None:
                shapes  = str(ln['input'])  +' '*(inp_w-len(str(ln['input']))) + ' -> '
                shapes += str(ln['output']) +' '*(out_w-len(str(ln['output'])))
                data = ""

            tot_ops += ln['ops'] if ln and ln['ops'] is not None else 0
            ops = f"{ModelState.i2s(ln['ops'],12)}" if 'ops' in ln and ln['ops'] is not None else " "*12
            ...
            if len(ln['params']) > 0:
                ...
                print(f"{ln['st']+' '*(ma-len(ln['st']))}  {num_st}  {prs_st} | {data} {shapes} {ops if is_input else ''} {'' if is_input else name}")
            else:
                w = 19 if  input_size is not None or  input_data is not None else 24
                print(f"{ln['st']+' '*(ma-len(ln['st']))} {' '*w}    {shapes} {ops if is_input else ''} {'' if is_input else name}")
