...
from .total        import get_activation, get_norm, get_dropout, ShiftFeatures
...
class CNN(nn.Module):
    ...
    def default():
        return copy.deepcopy(Config(
            ...
            stride_first  = True,   # если в блоке stride>1 делать его на первом Conv2d, иначе на последнем
            ...
            res           = 2,      # тип множителя gamma (1-константа, 2-обучаемое число, 3-обучаемый вектор)
    ...
    def get_blocks(self, token, channels, cfg):
        ...
        if token[0] == 'r' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block= Residual(
                    ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm, fun=cfg.fun, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside),
        ...
        if token[0] == 'b' and ( len(token) == 1 or token[1].isdigit() ):
            ...
            block = ResidualConvBlock(channels, Eout, kernel_size=kern, stride=stride, stride_first=cfg.stride_first, bias=cfg.bias, norm=cfg.norm, fun=cfg.fun, drop_d=cfg.drop_d_inside, shift=cfg.shift_inside)
            block.name = token
            channels = Eout
            return [block], channels
