...
def get_union_columns(
    ...
    """
    return list(set().union(*[get_columns(s) for s in signals_list]))
...
class JSONPair(IndexingFormat[ObjectInfo]):
    ...
    class InsertSignals(Operation):
        ...
        async def __call__(self, items):
            items = [
                flatten_signals(item, delimiter=self.subobject_delimiter)
                for item in items
            ]
            items = [
            ...
            columns = get_union_columns(items)
            for col in columns:
