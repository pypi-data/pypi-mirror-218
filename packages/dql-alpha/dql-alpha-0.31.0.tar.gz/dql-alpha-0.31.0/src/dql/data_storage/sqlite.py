...
class SQLiteDataStorage(AbstractDataStorage):
    ...
    def extend_index_with_signals(self, index: "schema.Table", signals: SignalsTable):
        ...
        """
        # Rename index table, since we will be replacing it.
        index_tbl_name = "tmp_" + index.name
        self._rename_table(index.name, index_tbl_name)
        self.metadata.remove(index.table)
        with self.db:
            self.db.execute("begin")
            ...
            q = sqlalchemy.select(*index_tbl.c, *signal_columns).select_from(
                index_tbl.outerjoin(signals.table, index_tbl.c.id == signals.table.c.id)
            )
            new_index_tbl = self.schema.node_cls.new_table(
                index.name,
                [self.schema.node_cls.copy_signal_column(c) for c in signal_columns],
                self.metadata,
            )
            self.execute(CreateTable(new_index_tbl))

            # Write results of query to the new index table.
            cols = [c.name for c in new_index_tbl.c]
            self.execute(sqlalchemy.insert(new_index_tbl).from_select(cols, q))
            self.execute(DropTable(index_tbl))
