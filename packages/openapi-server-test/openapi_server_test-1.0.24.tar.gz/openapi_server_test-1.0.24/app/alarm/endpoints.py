...
class ShowRequest(common.endpoints.ShowRequest):
    directive: ClassVar[str] = 'alarm'

@common.endpoints.logging_middleware("show", "alarm")
...
class AddRequest(WriteRequest):
    """AddRequest encapsulates fields adding a alarm
    """
    cmd_type: ClassVar[str] = "add"

@dataclass
class DeleteRequest(WriteRequest):
    """DeleteRequest encapsulates fields deleting a alarm
    """
    cmd_type: ClassVar[str] = "del"

@dataclass
class SetRequest(WriteRequest):
    """SetRequest encapsulates fields updating a alarm
    """
    cmd_type: ClassVar[str] = "set"


@common.endpoints.logging_middleware("add", "alarm")
def add(cli: Dingman_grpc_cli, req: AddRequest) -> Response:
    """Processes the user's request and adds a new alarm
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("delete", "alarm")
def delete(cli: Dingman_grpc_cli, req: DeleteRequest) -> Response:
    """Process the user's request and deletes a new alarm
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("set", "alarm")
def set(cli: Dingman_grpc_cli, req: SetRequest) -> Response:
    """Processes the user's request and sets a new alarm
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())
