...
class ShowRequest(common.endpoints.ShowRequest):
    directive: ClassVar[str] = 'cdn-service'

@common.endpoints.logging_middleware("show", "cdn-service")
...
class WriteRequest(common.endpoints.WriteRequest):
    directive: str = "cdn-service"

@dataclass
class AddRequest(WriteRequest):
    """AddRequest encapsulates fields adding a cdn-service
    """
    cmd_type: ClassVar[str] = "add"

@dataclass
class DeleteRequest(WriteRequest):
    """DeleteRequest encapsulates fields deleting a cdn-service
    """
    cmd_type: ClassVar[str] = "del"

@dataclass
class SetRequest(WriteRequest):
    """SetRequest encapsulates fields updating a cdn-service
    """
    cmd_type: ClassVar[str] = "set"


@common.endpoints.logging_middleware("add", "cdn-service")
def add(cli: Dingman_grpc_cli, req: AddRequest) -> Response:
    """Processes the user's request and adds a new cdn-service
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())


@common.endpoints.logging_middleware("delete", "cdn-service")
def delete(cli: Dingman_grpc_cli, req: DeleteRequest) -> Response:
    """Process the user's request and deletes a new cdn-service
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())


@common.endpoints.logging_middleware("set", "cdn-service")
def set(cli: Dingman_grpc_cli, req: SetRequest) -> Response:
    """Processes the user's request and sets a new cdn-service
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())
