...
class ShowRequest(common.endpoints.ShowRequest):
    directive: ClassVar[str] = 'cdn-service-group'

@common.endpoints.logging_middleware("show", "cdn-service-group")
...
class WriteRequest(common.endpoints.WriteRequest):
    directive: str = "cdn-service-group"

@dataclass
class AddRequest(WriteRequest):
    """AddRequest encapsulates fields adding a cdn-service-group
    """
    cmd_type: ClassVar[str] = "add"

@dataclass
class DeleteRequest(WriteRequest):
    """DeleteRequest encapsulates fields deleting a cdn-service-group
    """
...
class SetRequest(WriteRequest):
    """SetRequest encapsulates fields updating a cdn-service-group
    """
    cmd_type: ClassVar[str] = "set"


@common.endpoints.logging_middleware("add", "cdn-service-group")
def add(cli: Dingman_grpc_cli, req: AddRequest) -> Response:
    """Processes the user's request and adds a new cdn-service-group
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("delete", "cdn-service-group")
def delete(cli: Dingman_grpc_cli, req: DeleteRequest) -> Response:
    """Process the user's request and deletes a new cdn-service-group
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("set", "cdn-service-group")
def set(cli: Dingman_grpc_cli, req: SetRequest) -> Response:
    """Processes the user's request and sets a new cdn-service-group
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())
