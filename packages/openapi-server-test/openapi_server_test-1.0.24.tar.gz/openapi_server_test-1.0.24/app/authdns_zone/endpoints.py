...
class ShowRequest(common.endpoints.ShowRequest):
    directive: ClassVar[str] = 'auth-dns-zone'

@common.endpoints.logging_middleware("show", "auth-dns-zone")
...
class WriteRequest(common.endpoints.WriteRequest):
    directive: str = "auth-dns-zone"

@dataclass
class AddRequest(WriteRequest):
    """AddRequest encapsulates fields adding a auth-dns-zone
    """
    cmd_type: ClassVar[str] = "add"

@dataclass
class DeleteRequest(WriteRequest):
    """DeleteRequest encapsulates fields deleting a auth-dns-zone
    """
    cmd_type: ClassVar[str] = "del"

@dataclass
class SetRequest(WriteRequest):
    """SetRequest encapsulates fields updating a auth-dns-zone
    """
    cmd_type: ClassVar[str] = "set"


@common.endpoints.logging_middleware("add", "auth-dns-zone")
def add(cli: Dingman_grpc_cli, req: AddRequest) -> Response:
    """Processes the user's request and adds a new auth dns zone
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("delete", "auth-dns-zone")
def delete(cli: Dingman_grpc_cli, req: DeleteRequest) -> Response:
    """Process the user's request and deletes a new auth dns zone
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("set", "auth-dns-zone")
def set(cli: Dingman_grpc_cli, req: SetRequest) -> Response:
    """Processes the user's request and sets a new auth dns zone
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())
