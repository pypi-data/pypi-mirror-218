...
class ShowRequest(common.endpoints.ShowRequest):
    directive: ClassVar[str] = 'app'

@common.endpoints.logging_middleware("show", "app")
...
class AddRequest(WriteRequest):
    """AddRequest encapsulates fields adding a app
    """
    cmd_type: ClassVar[str] = "add"

@dataclass
class DeleteRequest(WriteRequest):
    """DeleteRequest encapsulates fields deleting a app
    """
    cmd_type: ClassVar[str] = "del"

@dataclass
class SetRequest(WriteRequest):
    """SetRequest encapsulates fields updating a app
    """
    cmd_type: ClassVar[str] = "set"


@common.endpoints.logging_middleware("add", "app")
def add(cli: Dingman_grpc_cli, req: AddRequest) -> Response:
    """Processes the user's request and adds a new app
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("delete", "app")
def delete(cli: Dingman_grpc_cli, req: DeleteRequest) -> Response:
    """Process the user's request and deletes a new app
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())

@common.endpoints.logging_middleware("set", "app")
def set(cli: Dingman_grpc_cli, req: SetRequest) -> Response:
    """Processes the user's request and sets a new app
    """
    return common.endpoints.call_general_dingman_cmd(cli, req.to_cmd())
