...
class UniqueQueue(queue.Queue):
    ...
    def put(self, item, block=True, timeout=None):
        ...
        一个对象重复put将会忽略
        """
        if item not in self.queue_set:  # 如果元素不在队列中，则添加
...
def read_bytes(filepath, encoding='utf-8', not_exists_ok: bool = False) -> bytes:
...
def read_json(filepath, encoding='utf-8', not_exists_ok: bool = False) -> dict:
    if not_exists_ok and not Path(filepath).exists():
        return {}
    with open(filepath, 'r', encoding=encoding) as f:
        return json.load(f)
...
def write_json(filepath, json_obj: dict, encoding='utf-8'):
    if json_obj is None:
