...
async def pesan_user(user_id):
    if user_id in BASICID:
        pesan = MSG_ON_BASIC.format(cmd)
    elif user_id in MEDIUMID:
        pesan = MSG_ON_MEDIUM.format(cmd)
    elif user_id in PREMIUMID or user_id in DEVS:
        pesan = MSG_ON_PREM.format(cmd)
...
async def babi(bot):
    ...
    if not user_data or not botlog_chat_id:
        ...
        await set_botlog(user_id, botlog_chat_id)
        await asyncio.sleep(1)
        pesan = pesan_user(user_id)
        await bot.send_message(botlog_chat_id, pesan)
        await asyncio.sleep(1)
