...
def _get_daily_returns(
    ...
    if not (daily_return.index ^ dates).empty:
        daily_return = daily_return.reindex(dates)

    if not (daily_return.columns ^ set(order_book_ids)).empty:
...
def _adjust_daily_return(price: pd.DataFrame) -> pd.DataFrame:
    ...
    if splits is not None and not splits.empty:
        ...
        splits = splits.pivot("date", "order_book_id", "factor")
    ...
    if dividends is not None and not dividends.empty:
        ...
        dividends = dividends.pivot("date", "order_book_id", "cash").fillna(0.0)
...
def get_bond_daily_returns(
    ...
    chinabond_valuations.index.set_levels(pd.to_datetime(chinabond_valuations.index.levels[1]), inplace=True, level=1)

    shift_valuation = chinabond_valuations.groupby(level='order_book_id').apply(lambda x: x.shift(1))
    ...
    day_interval = chinabond_valuations['accrued_interest_eod'].groupby(level="order_book_id").apply(
    ...
    holding_return = holding_return.groupby(level='order_book_id').apply(lambda x: x.iloc[1:].droplevel(0))
