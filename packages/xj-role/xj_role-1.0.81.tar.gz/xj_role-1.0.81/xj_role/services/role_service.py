...
class RoleService:
    ...
    def get_role_list(params: dict = None, need_pagination: bool = True, only_first: bool = False, get_tree=False, filter_fields: "str|list" = None, **kwargs):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "role_id|int", "role", "user_group_id|int", "parent_role_id|int", "user_id|int", "is_using|int", "is_delete|int",
                "user_group_id_list|list_int", "role_id_list|list_int", "user_id_list|list_int"
            ],
            split_list=["user_group_id_list", "role_id_list", "user_id_list"],
            alias_dict={"user_group_id_list": "user_group_id__in", "role_id_list": "role_id__in", "role": "role_key"}
        )
    ...
    def bind_user_role(user_id: int, role_list):
        ...
        try:
            UserToRole.objects.filter(user_id=user_id).delete()
            print(list(UserToRole.objects.filter(user_id=user_id).values()))
            for i in role_list:
