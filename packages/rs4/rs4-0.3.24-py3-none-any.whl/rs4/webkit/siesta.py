...
class Resource (object):
    ...
    def __init__(self, uri, api, logger, reraise_http_error = True, callback = None, headers = None):
        self._api = api
        self._uri = uri
        self._logger = logger
        ...
        self._headers.update ({'User-Agent': USER_AGENT, "Accept": self.ACCEPT})
...
class API(object):
    ...
    REQ_TIMEOUT = 60
    def __init__(self, base_url, auth=None, logger = None, reraise_http_error = True, callback = None, session = None, headers = None):
    ...
    def __getattr__(self, name):
        if name in ('get', 'post', 'put', 'patch', 'delete', 'options'):
            r = self.RESOURCE_CLASS (uri='', api=self, logger = self._logger, reraise_http_error = self._reraise_http_error, callback = self._callback, headers = self._headers.copy ())
            return getattr (r, name)
        return self.RESOURCE_CLASS (uri=name, api=self, logger = self._logger, reraise_http_error = self._reraise_http_error, callback = self._callback, headers = self._headers.copy ())
