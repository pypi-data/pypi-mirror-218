...
class Prefix:
    def set_prefix(self: "meta.Client", handler):
    ...
    def set_prefix_user(self: "meta.Client", user_id, handler):
    ...
    def del_prefix(self: "meta.Client"):
        hndlrdb = self.mongo_sync.prefix
        hndlrdb.update_one({"user_id": self.me.id}, {"$set": {"hndlr": [".", "!", "*", "^", "-", "?"]}})

    def get_prefix(self: "meta.Client"):
        hndlrdb = self.mongo_sync.prefix
    ...
    def get_prefix_user(self: "meta.Client", user_id):
        hndlrdb = self.mongo_sync.prefix
