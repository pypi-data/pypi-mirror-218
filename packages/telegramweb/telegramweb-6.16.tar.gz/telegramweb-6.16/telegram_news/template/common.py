...
requests.packages.urllib3.disable_warnings() #remove unverified warning
...
class NewsPostman(object):
    ...
    def _get_full(self, url, item):
        ...
        data['reply_markup'] = InlineKeyboardMarkup([[InlineKeyboardButton(text=self._tag, url=url)]])
        return data
    ...
    def _data_format(self, item, news_id):
        ...
        if self._DEBUG:
            print(data)
        data['reply_markup'] = InlineKeyboardMarkup([[InlineKeyboardButton(text=self._tag, url=item['link'])]])
        return data, method

    @sleep_and_retry
    @limits(calls=1, period=1)
    def _real_post(self, token, method, data):
        #data['reply_markup'] = InlineKeyboardMarkup([[InlineKeyboardButton(text="CLICK HERE", url=item['link'])]])
