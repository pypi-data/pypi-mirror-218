...
from truss.patch import PatchApplier


class ModelContainerPatchApplier(PatchApplier):
    ...
    def __init__(
        ...
        super().__init__(inference_server_home, app_logger)
        self._model_module_dir = self._truss_dir / self._truss_config.model_module_dir
        self._bundled_packages_dir = (
            self._truss_dir / ".." / self._truss_config.bundled_packages_dir
        ).resolve()
        self._data_dir = self._truss_dir / self._truss_config.data_dir
    ...
    def __call__(self, patch: Patch, inf_env: dict):
        ...
        if isinstance(patch.body, ModelCodePatch):
            ...
            apply_code_patch(self._model_module_dir, model_code_patch, self._logger)
            ...
            apply_code_patch(self._bundled_packages_dir, package_patch, self._logger)
        else:
    ...
    def _apply_config_patch(self, config_patch: ConfigPatch):
        self._logger.debug(f"Applying config patch {config_patch.to_dict()}")
        TrussConfig.from_dict(config_patch.config).write_to_yaml_file(
            Path(self._truss_dir / config_patch.path)
        )
    ...
    def _apply_external_data_patch(self, external_data_patch: ExternalDataPatch):
        ...
        if action == Action.REMOVE:
            ...
            if not filepath.exists():
                self._logger.warning(f"Could not delete file {filepath}: not found.")
            else:
                self._logger.debug(f"Deleting file {filepath}")
                filepath.unlink()
