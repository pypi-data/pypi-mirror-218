...
class _PerspectiveManager:
    def __init__(self, homography_matrix: Union[np.ndarray, List[List[float]]],
                       default_w: int, default_h: int, crop_boundaries: bool = False,
                       boundaries_color: Tuple[Union[float, int], Union[float, int], Union[float, int]] = (0., 0., 0.)):
        ...
        self.inverse_homography_matrix = np.linalg.inv(a=self.homography_matrix)
    ...
    def calculate_output_shape(self, w: int, h: int, cropping_boundaries: Optional[bool] = None) -> tuple[int, int]:
        """
    ...
    def get_hw_magnification_at_point(self, x: Union[int, float], y: Union[int, float]) -> Tuple[float, float]:
        """
        ...
        vec_x_transformed = np.dot(a=self.homography_matrix, b=vec_x)
        vec_y_transformed = np.dot(a=self.homography_matrix, b=vec_y)
        ...
        length_transformed_x = np.linalg.norm(x=vec_x_transformed[:, -1] - vec_x_transformed[:, 0])
        length_transformed_y = np.linalg.norm(x=vec_y_transformed[:, -1] - vec_y_transformed[:, 0])
    ...
    def get_hw_magnification_for_line(self, xyxy_line: Union[np.ndarray, Tuple[Union[int, float], Union[int, float],
    ...
    def output_space_points_to_input_space(
        self, points_xy: Union[np.ndarray, Tuple[Union[int, float], Union[int, float],
                                                 Union[int, float], Union[int, float]]]) -> np.ndarray:
        """
    ...
    @lru_cache(maxsize=16)
    def __get_crop_bbox_xyxy(self, h: Union[int, float], w: Union[int, float], return_as_int: bool = True) -> \
    ...
    def __build_corners(self, w: Union[float, int], h: Union[float, int]) -> np.ndarray:
        """
