...
class GraphsGenerator:
    def get_graphs_as_pdf(self, values_all, values_subset_all, pdf_file_name = 'metrics_graphs.pdf'):
...
def generate_confusion_matrix_for_pdf(fig,metric_values,elements,keys_in_subset_all,values_subset_all,metric_name):
    ...
    for subset_key in keys_in_subset_all:
        generate_subsets_for_confusion_matrix(fig,metric_name,subset_key,values_subset_all,elements)

def generate_subsets_for_confusion_matrix(fig,metric_name,subset_key,values_subset_all,elements):
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        y=['Positive', 'Negative'],
        colorscale='Viridis'))
    fig.update_layout(title=f'Confusion Matrix for {subset_key.capitalize()} Data', xaxis_title='Predicted', yaxis_title='Actual',showlegend=False)
...
def generate_graph_for_performance_for_html(fig,metric_values,keys_in_subset_all,values_subset_all,metric_name):
        ...
        for subset_key in keys_in_subset_all:
            ...
            fig.add_trace(go.Bar(x=[subset_key], y=[subset_values['value']], name=subset_key.capitalize()))
...
def generate_graphs_for_fairness_for_html(fig,metric_values,keys_in_subset_all,values_subset_all,metric_name):
        fig = go.Figure()
        # Add complete value
        color = '#52BD94' if metric_values['status'] == 'Pass' else '#D14343'
