...
from nerdcore.defs import NERD_CONFIG_CLASS_PATH, nl


def update_nerd_config_class():
    config = get_nerd_config_defaults()
    ...
    # Load the NerdConfig class
    with open(NERD_CONFIG_CLASS_PATH, 'r') as class_file:
    ...
    props_start_re = re.compile(r'\[\s*NERD_CONFIG_PROPS_START\s*\]')
    props_end_re = re.compile(r'\[\s*NERD_CONFIG_PROPS_END\s*\]')
    validations_start_re = re.compile(r'\[\s*NERD_CONFIG_VALIDATIONS_START\s*\]')
    validations_end_re = re.compile(r'\[\s*NERD_CONFIG_VALIDATIONS_END\s*\]')
    ...
    validations_imports = [
        f"        from nerdcore.utils.nerd_config.nerd_config_validations import validate_str, "
    ...
    # Write the updated NerdConfig class
    with open(NERD_CONFIG_CLASS_PATH, 'w') as class_file:
