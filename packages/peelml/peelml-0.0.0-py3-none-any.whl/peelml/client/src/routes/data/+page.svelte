<script>
  ...
  async function handleFileChange(event) {
    ...
    const response = await fetch("http://127.0.0.1:5000/objects/upload", {
      method: "POST",
      body: formData,
    });
    indexFiles();
  ...
  async function loadServerData() {
    const response = await fetch("http://127.0.0.1:5000/objects/get");
  ...
  async function indexFiles() {
    console.log("index!");
    indexing = true;
    const response = await fetch("http://127.0.0.1:5000/objects/index");
    const data = await response.json();
    indexed = true;
    indexing = false;
  }
  ...
  async function getEmbeddings() {
    ...
    const response = await fetch("http://127.0.0.1:5000/embedding/get");
