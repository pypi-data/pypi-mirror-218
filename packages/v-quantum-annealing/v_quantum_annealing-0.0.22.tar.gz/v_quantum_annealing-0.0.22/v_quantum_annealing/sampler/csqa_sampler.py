...
class CSQASampler(SQASampler):
    ...
    def _get_result(self, system, model):
        ...
        state = cxxvqa.result.get_solution(system)
    ...
    def sample_ising(
        ...
        Examples:
            ...
            for Ising case::
                ...
                >>> h = {0: -1, 1: -1, 2: 1, 3: 1}
                >>> J = {(0, 1): -1, (3, 4): -1}
                >>> sampler = v_quantum_annealing.CSQASampler()
                >>> res = sampler.sample_ising(h, J)

            for QUBO case::

                >>> Q = {(0, 0): -1, (1, 1): -1, (2, 2): 1, (3, 3): 1, (4, 4): 1, (0, 1): -1, (3, 4): 1}
                >>> sampler = v_quantum_annealingj.CSQASampler()
        ...
        ising_graph = bqm.get_cxxvqa_ising_graph()
        ...
        # choose updater -------------------------------------------
        sqa_system = cxxvqa.system.make_continuous_time_ising(
            init_generator(), ising_graph, self.gamma
        )
        _updater_name = updater.lower().replace("_", "").replace(" ", "")
        if _updater_name == "swendsenwang":
            algorithm = cxxvqa.algorithm.Algorithm_ContinuousTimeSwendsenWang_run
        else:
