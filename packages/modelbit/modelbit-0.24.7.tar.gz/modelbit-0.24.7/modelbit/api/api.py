...
from modelbit.utils import sizeOfFmt, retry
...
@retry(8, logger)
def _doPost(url: str, headers: Dict[str, str], data: str):
  with requests.post(url, headers=headers, data=data, timeout=defaultRequestTimeout) as url:  # type: ignore
    resp: Any = url.json()  # type: ignore
    return resp
...
class MbApi:
  ...
  def getJson(self, path: str, body: Dict[str, Any] = {}) -> Dict[str, Any]:
    ...
    return _doPost(url=f'{self._api_host}{path}', headers=self._headers(), data=serializedData)
