...
class GlobalVariable:
    ...
    def append_global_variable(self, global_type, init_value):
        global_rewriter = SectionRewriter(self.binary.module, self.binary.module.global_sec)
        global_rewriter.insert(inserted_item=Global(valtype=global_type, val=init_value))
    ...
    def delete_global_variable(self, idx=None, global_type=None, init_value=None):
        if idx is not None:
            self.binary.module.global_sec.pop(idx)
            return
...
class ImportExport:
    ...
    def insert_import_function(self, idx, module_name, func_name, params_type, results_type):
        ...
        type_rewriter = SectionRewriter(self.binary.module, self.binary.module.type_sec)
        result = type_rewriter.select(Type(arg_types=params_type, ret_types=results_type))
        if result == []:
            type_rewriter.insert(inserted_item=Type(arg_types=params_type, ret_types=results_type))
            typeidx = type_rewriter.select()[-1].typeidx
        else:
            typeidx = result[0].typeidx

        import_rewriter = SectionRewriter(self.binary.module, self.binary.module.import_sec)
        import_rewriter.insert(Import(importidx=idx), Import(module_name=module_name, name=func_name, typeidx=typeidx))

    def delete_import_function(self, idx):
    ...
    def append_import_function(self, module_name, func_name, params_type, results_type):
        ...
        import_rewriter = SectionRewriter(self.binary.module, self.binary.module.import_sec)
        import_rewriter.insert(inserted_item=Import(module=module_name, name=func_name, typeidx=typeidx))

    def insert_export_function(self, idx, func_name, funcidx):

        export_rewriter = SectionRewriter(self.binary.module, self.binary.module.export_sec)
        export_rewriter.insert(Export(exportidx=idx), Export(func_name, funcidx))

    def modify_export_function(self, idx, func_name, funcidx):

        export_rewriter = SectionRewriter(self.binary.module, self.binary.module.export_sec)
        export_rewriter.update(Export(exportidx=idx), Export(func_name, funcidx))

    def delete_export_function(self, idx):
    ...
    def append_export_function(self, func_name, funcidx):
        ...
        export_rewriter = SectionRewriter(self.binary.module, self.binary.module.export_sec)
        export_rewriter.insert(inserted_item=Export(name=func_name, funcidx=funcidx))
    ...
    #     for _, import_item in enumerate(self.binary.module.import_sec):
    ...
    #         if import_item == import_variable:
    #             raise Exception("The global variable already exists")
    #     self.binary.module.import_sec[idx] = import_variable
    #
    #     # fix
    ...
    #
    #     import_mem = Import(module_name, memory_name, ImportDesc(2, mem=memidx))
    #     self.binary.module.import_sec[idx] = import_mem
    #
    ...
    #
    #     import_table = Import(module_name, table_name, ImportDesc(1, table=tableidx))
    #     self.binary.module.import_sec[idx] = import_table


class LinearMemory:
    def __init__(self, binary):
        self.binary = binary
        self.indices_fixer = IndicesFixer(self.binary.module)

    def insert_linear_memory(self, offset, bytes):
        memory_rewriter = SectionRewriter(self.binary.module, self.binary.module.mem_sec)
        memory_list = memory_rewriter.select()

        if memory_list[0].max != 0 and offset + len(bytes) + 1 > memory_list[0].max * 65536:
            raise Exception("Memory out of bounds")

        data_rewriter = SectionRewriter(self.binary.module, self.binary.module.data_sec)

        is_overlap = False
        length = len(bytes)
        for data_item in data_rewriter.select():
            ...
            self.indices_fixer.fix_memory_limits(self.binary.module.mem_sec, offset + length)

        if data_rewriter.select() is [] or is_overlap is False:
            data_rewriter.insert(inserted_item=Data(offset=offset, init_data=bytes))

    def append_linear_memory(self, page_num):
        memory_rewriter = SectionRewriter(self.binary.module, self.binary.module.mem_sec)
        mem_list = memory_rewriter.select()
...
class Function:
    ...
    def insert_internal_function(self, idx, params_type, results_type, local_vec, func_body):
        ...
        import_rewriter = SectionRewriter(self.binary.module, self.binary.module.import_sec)
        import_func_num = len(import_rewriter.select())
        ...
        function_rewriter = SectionRewriter(self.binary.module, self.binary.module.func_sec)
        function_rewriter.insert(Function(funcidx=idx - import_func_num), Function(typeidx=typeidx))

        code_rewriter = SectionRewriter(self.binary.module, self.binary.module.code_sec)
        code_rewriter.insert(Code(funcidx=idx - import_func_num), Code(local_vec=local_vec, instr_list=func_body))
    ...
    def insert_indirect_function(self, idx, params_type, results_type, local_vec, func_body):
        ...
        element_rewriter = SectionRewriter(self.binary.module, self.binary.module.elem_sec)

        # get original indirect function list
        indirect_func_list = []
        for elem in element_rewriter.select():
            indirect_func_list.extend(elem.funcidx_list)

        self.indices_fixer.fix_table_limits(self.binary.module.table_sec, len(indirect_func_list) + 1)

        element_list = element_rewriter.select()

        if not element_list:
            element_rewriter.insert(inserted_item=Element(tableidx=0, offset=1, funcidx_list=[idx]))
        else:
    ...
    def insert_hook_function(self, hooked_funcidx, idx, params_type, results_type, locals_vec, func_body):
        ...
        for funcidx in range(import_func_num, import_func_num + len(code_rewriter.select())):
            if funcidx != idx:
    ...
    def delete_func_instr(self, funcidx, offset):
        ...
        code.instr_list.pop(offset)
        code_rewriter.update(Code(funcidx=funcidx), code)
    ...
    def insert_func_instrs(self, funcidx, offset, instrs: list):
        ...
        for i in reversed(instrs):
            code.instr_list.insert(offset, i)
    ...
    def modify_func_instrs(self, funcidx, instr, instrs: list):
        ...
        for index, i in enumerate(code.instr_list):
            if i == instr:
                code.instr_list.pop(index)
                for j in reversed(instrs):
                    code.instr_list.insert(index, j)

        code_rewriter.update(Code(funcidx=funcidx), Code(instr_list=code.instr_list))

    def add_new_local_to_func(self, funcidx, valtype):
        import_rewriter = SectionRewriter(self.binary.module, self.binary.module.import_sec)
        import_func_num = len(import_rewriter.select())
