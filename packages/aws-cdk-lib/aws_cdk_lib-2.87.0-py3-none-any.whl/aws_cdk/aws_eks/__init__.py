...
subnetcount = 0
subnets = [(SpreadElement ...vpc.publicSubnets
  vpc.public_subnets), (SpreadElement ...vpc.privateSubnets
  vpc.private_subnets)]
for subnet in subnets:
    # Wait for the ipv6 cidr to complete
    subnet.node.add_dependency(ipv6cidr)
    self._associate_subnet_with_v6_cidr(subnetcount, subnet)
    subnetcount++

cluster = eks.Cluster(self, "hello-eks",
    vpc=vpc,
    ip_family=eks.IpFamily.IP_V6,
    vpc_subnets=[ec2.SubnetSelection(subnets=[(SpreadElement ...vpc.publicSubnets
      vpc.public_subnets)])]
)
```
