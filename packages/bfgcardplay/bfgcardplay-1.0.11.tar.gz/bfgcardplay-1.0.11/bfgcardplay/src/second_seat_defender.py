...
class SecondSeatDefender(SecondSeat):
    ...
    def selected_card(self) -> Card:
        ...
        # Void
        if not cards:
            return self._select_card_if_void(trick)
        ...
        card = self._play_winner_if_last_opportunity(cards, trick)
        if card:
            return card

        # Play honour if higher honour in dummy
        card = self._beat_dummys_honour(cards, trick)
        if card:
            return card
        ...
        # Play K if doubleton
        card = self._play_doubleton_king(cards, trick)
        if card:
            return card

        # Signal even/odd
        card = self._signal_even_odd(cards, trick)
        if card:
            return card
    ...
    def _play_winner_if_last_opportunity(self, cards, trick) -> Card:
        player = self.player
    ...
    def _play_doubleton_king(self, cards, trick) -> Card:
        player = self.player
        suit = trick.suit.name
    ...
    def _signal_even_odd(self, cards, trick) -> Card:
        player = self.player
    ...
    def _cover_honour_with_honour(self, cards, trick) -> Card:
    ...
    def _beat_dummys_honour(self, cards, trick) -> Card:
        player = self.player
        if player.dummy_on_right:
            for card in cards:
    ...
    def _select_card_if_void(self, trick: Trick) -> Card:
