...
class Dashboard():
    ...
    def __init__(self, player) -> None:
        ...
        cprint(f"{self.sure_tricks}", MODULE_COLOUR)
        # cprint(f"xxx {self.sure_tricks}", MODULE_COLOUR)
    ...
    def _get_split_winners(self) -> dict[str, Card]:
        ...
        for suit in SUITS:
            if len(opponents_cards[suit]) in SPLITS:
                ...
                for split in splits:
                    split_winners[suit].append(self._split_winners(suit, split))
    ...
    def _extra_winners_strength(self):
        ...
        for suit in SUITS:
            ...
            #     cprint(f'x {long_hand}', MODULE_COLOUR)
