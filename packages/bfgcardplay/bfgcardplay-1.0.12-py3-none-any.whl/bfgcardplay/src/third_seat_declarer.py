...
class ThirdSeatDeclarer(ThirdSeat):
    ...
    def selected_card(self) -> Card:
        ...
        winning_card = self._get_winning_card(trick)
        if winning_card:
            return winning_card
    ...
    def _select_card_in_suit_to_develop(self, suit: Suit, trick: Trick) -> Card:
        ...
        # Win trick if possible
        card = self._manager_win_trick(cards, suit)
        if card:
            return card

        # Missing Ace
        card = self._missing_ace(cards, suit, trick)
        if card:
            return card
        ...
        # Missing King
        card = self._missing_king(cards, suit, trick)
        if card:
            return card

        # Missing Queen
        card = self._missing_queen(suit, trick)
        if card:
            return card

        # Play top card if winner and suit nearly played out
        card = self._play_suit_out(cards, trick)
        if card:
            return card
        ...
        if tricks_needed <= dashboard.sure_tricks.count:
            if player.is_winner_declarer(cards[0], trick):
                return log(inspect.stack(), cards[0])
    ...
    def _missing_ace(self, cards, suit, trick) -> Card:
        player = self.player
        manager = global_vars.manager
    ...
    def _missing_queen(self, suit, trick) -> Card:
        ...
        if manager.suit_strategy(player.seat)[suit.name] == 'missing_queen' and missing_queen:
            card = self._get_winning_card(trick)
            if card:
                return card
        return None

    def _play_suit_out(self, cards, trick) -> Card:
        player = self.player
        dashboard = player.dashboard
    ...
    def _get_winning_card(self, trick: Trick) -> Card | None:
        ...
        card = self._card_from_probable_winners(player, cards, trick)
        if card:
            return card
        ...
        card = self._get_card_over_top_of_tenace(cards, trick)
        if card:
            return card

        # Win trick if possible to de-block suit
        card = self._deblock_suit(player, cards, trick)
        if card:
            return card

        # Cover honour with honour
        card = self._cover_honour_with_honour(cards, trick)
        if card:
            return card
        ...
        card = self._overtake_partner(player, cards, trick)
        if card:
            return card

        # Play winner
        card = self._play_winner(cards, trick)
        if card:
            return card

        return None

    def _play_winner(self, cards, trick) -> Card:
        player = self.player
        if not player.is_winner_declarer(trick.cards[0], trick):
            for card in cards[::-1]:
                if player.is_winner_declarer(card, trick):
    ...
    def _get_card_over_top_of_tenace(self, cards, trick) -> Card:
        player = self.player
    ...
    def _card_from_all_winners(self, cards, trick) -> Card:
        player = self.player
        dashboard = player.dashboard
        if (player.holds_all_winners_in_suit(trick.suit, trick) or
                dashboard.sure_tricks.count >= 13 - player.trick_number):
    ...
    def _card_from_probable_winners(self, player, cards, trick) -> Card:
        ...
        if player.trump_suit:
            if not player.is_winner_declarer(trick.cards[0], trick):
                ...
                if player.trump_cards and opponents_trumps:
                    if self._trump_partners_card(player, trick):
