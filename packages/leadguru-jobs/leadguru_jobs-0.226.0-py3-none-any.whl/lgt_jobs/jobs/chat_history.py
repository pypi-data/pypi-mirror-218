...
from .. import BackgroundJobRunner
from ..env import portal_url, save_inbox_leads
...
class LoadChatHistoryJob(BaseBackgroundJob, ABC):
    ...
    def exec(self, data: LoadChatHistoryJobData):
        ...
        dedicated_bots = DedicatedBotRepository().get_user_bots(user.id, only_valid=True) if save_inbox_leads else []
        log.info(f"[LoadChatHistoryJob]: Saving inbox leads enabled: {save_inbox_leads}")
