...
def v_ref_cruise(predecessor, ego):
	...
	speed_i = ego.speed
	e = d - length - h*speed_i
	if e > 0:
		c = min(q / e * (h*a_max - speed_i_1 + speed_i), 2)		# todo: check where negatives come from
	elif e < 0:
		c = min( -q / e * (h*a_max - speed_i + speed_i_1), 2)
	else:
		c = 0
	if c < 0:
		print(c, e, speed_i, speed_i_1)
	return (tau / h * (speed_i_1 - speed_i + c * e) + speed_i) * 3.6
...
lead_navi.set_target_speed(0*3.6)
...
for i in range(simulation_length*pid_rate*sampling_rate):
	...
	if i < 20*sampling_rate*pid_rate and i % pid_rate == 0:
		lead_navi.set_target_speed(3.6 * i / (sampling_rate*pid_rate))

	elif i == 40*sampling_rate*pid_rate:
		lead_navi.set_target_speed(22*3.6)

	elif 60*sampling_rate*pid_rate <= i < 70*sampling_rate*pid_rate:
		lead_navi.set_target_speed(22*3.6 - 3.6*(i - 60*sampling_rate*pid_rate)/(sampling_rate*pid_rate * 2))

	elif 80*sampling_rate*pid_rate <= i < 86*sampling_rate*pid_rate:
		lead_navi.set_target_speed(17*3.6 + 3.6*(i - 80*sampling_rate*pid_rate)/(sampling_rate*pid_rate*2))
...
np.save(f"../logs_for_publishing/dynamic_cacc_length_{simulation_length}.npy", log)
