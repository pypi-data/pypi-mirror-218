...
def addable_etype_empty_rset(cls, req, rset=None, **kwargs):
    if rset is not None and not rset.rowcount:
        ...
        if len(select.defined_vars) == 1 and len(select.solutions) == 1:
            ...
            eschema = req.vreg.schema.entity_schema_for(rset._searched_etype)
            if not (
...
class AddRelatedActions(action.Action):
    ...
    def actual_actions(self):
        ...
        for rschema, teschema, role in self.add_related_schemas(entity):
            if (
                rschema.role_relation_definition(
                    eschema, teschema, role
    ...
    def add_related_schemas(self, entity):
        ...
        for role, rschemas in (
            ("subject", list(eschema.subject_relations.values())),
            ("object", list(eschema.object_relations.values())),
        ):
            for rschema in rschemas:
                ...
                for teschema in rschema.targets(eschema, role):
                    if not appearsin_addmenu.etype_get(
                        ...
                        continue
                    rdef = rschema.role_relation_definition(eschema, teschema, role)
