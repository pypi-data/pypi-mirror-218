...
class VRegistryTC(ViewSelectorTC):
    ...
    def _test_registered(self, registry, content):
        ...
        try:
            ...
            print("no more", [v for v in expected if v not in content])
            print("missing", [v for v in content if v not in expected])
            raise
    ...
    def test_score_entity_selector(self):
        with self.admin_access.web_request() as req:
            ...
            req.create_entity("File", data_name="bim.txt", data=Binary(b"boum"))
            # image/ehtml primary view priority
