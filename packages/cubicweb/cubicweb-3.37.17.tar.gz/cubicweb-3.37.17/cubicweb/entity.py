...
class Entity(AppObject):
    ...
    def _fetch_restrictions(
        ...
        for attr in sorted(fetchattrs):
            ...
            rdef = eschema.relation_definition(attr, takefirst=True)
    ...
    def copy_relations(self, ceid):  # XXX cw_copy_relations
        ...
        for rschema in list(self.e_schema.subject_relations.values()):
            ...
            rdef = self.e_schema.relation_definition(rschema, takefirst=True)
        ...
        for rschema in list(self.e_schema.object_relations.values()):
            ...
            if rschema.type in skip_copy_for["object"]:
                continue
            # XXX takefirst=True to remove warning triggered by ambiguous relations
            rdef = self.e_schema.relation_definition(rschema, "object", takefirst=True)
            # skip composite relation
            if rdef.composite:
                continue
            # skip relation with card in ?1 else we either change the copied
