...
class PropagationAnalyzer:
    ...
    def init(self, cube):
        ...
        eschemas = {
            eschema for eschema in schema.entities() if self.should_include(eschema)
        }
    ...
    def has_problem(self, eschema, edges):
        ...
        if not has_prop_rel and any(edge for edge in edges if edge[1] == eschema):
            warn("%s miss %s but is reached by propagation", eschema, self.prop_rel)
            return True
            ...
            if not edges:
                warn(
                    "%s has %s but isn't reached by " "propagation",
                    eschema,
                    self.prop_rel,
                )
                return True
            ...
            # the same cube
                ...
                warn(
                    "%s has %s relation / propagation rule"
                    " not added by the same cube (%s / %s)",
                    eschema,
                    self.prop_rel,
                    rdef.package,
                    edges[0][-1],
...
class CubeTracerSet:
    ...
    def add(self, value):
        ...
        if value in self.value_cube:
            warn(
                "%s is propagated by cube %s and cube %s",
                value,
                self.value_cube[value],
                cube,
            )
        else:
