...
from cubicweb.utils import json_dumps, xml_escape_from_dict
...
class CubicWebJsonEncoderTC(CubicWebTC):
    def test_json_dumps_with_binary(self):
        data = {
            "binary": Binary(b"something"),
        }
        self.assertEqual(json_dumps(data, sort_keys=True), '{"binary": "c29tZXRoaW5n"}')

    def test_json_dumps_with_datetime(self):
        data = {
            "date": datetime.date(year=2022, month=8, day=10),
            "datetime": datetime.datetime(year=2022, month=8, day=10, hour=17),
            "time": datetime.time(hour=13, minute=37),
            "timedelta": datetime.timedelta(days=42),
        }
        self.assertEqual(
            json_dumps(data, sort_keys=True),
            (
                '{"date": "2022/08/10", "datetime": "2022/08/10 17:00:00", '
                '"time": "13:37:00", "timedelta": 3628800}'
            ),
        )

    def test_json_dumps_with_decimal(self):
        data = {
            "decimal1": decimal.Decimal("42"),
            "decimal2": decimal.Decimal("13.37"),
        }
        self.assertEqual(
            json_dumps(data, sort_keys=True), '{"decimal1": 42.0, "decimal2": 13.37}'
        )

    def test_json_dumps_with_entity(self):
        with self.admin_access.repo_cnx() as cnx:
            user = self.create_user(cnx, "Alice", surname="Bob & co")

            self.assertEqual(
                json_dumps({"user": user}, sort_keys=True),
                (
                    '{"user": {"eid": %s, "firstname": null, '
                    '"last_login_time": null, "surname": "Bob &amp; co", '
                    '"upassword": "Alice"}}' % user.eid
                ),
            )

    def test_json_dumps_with_entity_and_xss_injection(self):
        with self.admin_access.repo_cnx() as cnx:
            user = self.create_user(
                cnx, "Alice", firstname="<script>alert(42);</script>"
            )
            ...
            self.assertEqual(
                ...
                (
                    '{"user": {"eid": %s, '
                    '"firstname": "&lt;script&gt;alert(42);&lt;/script&gt;", '
                    '"last_login_time": null, "surname": null, '
                    '"upassword": "Alice"}}' % user.eid
                ),
            )

    def test_xml_escape_from_dict(self):
        data = {
            "string1": "<i>italic</i>",
            "string2": "Logilab & co",
        }
        self.assertEqual(
            xml_escape_from_dict(data),
            {
                "string1": "&lt;i&gt;italic&lt;/i&gt;",
                "string2": "Logilab &amp; co",
            },
