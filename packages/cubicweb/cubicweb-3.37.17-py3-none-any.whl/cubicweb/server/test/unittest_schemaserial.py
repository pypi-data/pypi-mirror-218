    ...
    def test_rperms2rql2(self):
        self.assertListEqual(
            ...
            [
                (rql, kwargs)
                for rql, kwargs in erperms2rql(
                    schema.relation_schema_for("read_permission").rdef(
                        "CWEType", "CWGroup"
                    ),
                    self.GROUP_MAPPING,
                )
    ...
    def test_rperms2rql3(self):
        self.assertListEqual(
            ...
            [
                (rql, kwargs)
                for rql, kwargs in erperms2rql(
                    schema.relation_schema_for("name").rdef("CWEType", "String"),
                    self.GROUP_MAPPING,
                )
            ],
        )
...
class ComputedAttributeAndRelationTC(CubicWebTC):
    ...
    def test(self):
        ...
        self.assertEqual(
            [("Company", "Int")], list(schema["total_salary"].relation_definitions)
        )
        self.assertEqual(
