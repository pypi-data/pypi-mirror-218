...
def drop_entity_types_fast(*etypes, **kwargs):
    ...
    for etype in etypes:
        ...
        srels = [
            x.type
            for x in etype.subject_relations()
            if x.eid and not (x.final or x.inlined)
        ]
        ...
        for rtype in inlined_rels:
            for subjtype in rtype.subjects(etype):
                if subjtype in etypes:
                    continue
                sql(
                    "UPDATE cw_%(stype)s SET cw_%(rtype)s = NULL "
        ...
        for rel in srels:
            if all(subj in etypes for subj in rel.subjects()) or all(
                obj in etypes for obj in rel.objects()
            ):
                sql("DELETE FROM %s_relation" % rel.type)
            else:
                sql(
                    "DELETE FROM %s_relation WHERE eid_from IN (SELECT eid FROM entities WHERE type = %%s)"
                    % rel.type,
                    (etype.type,),
                )
        for rel in orels:
            if all(subj in etypes for subj in rel.subjects()) or all(
                ...
                sql(
                    "DELETE FROM %s_relation WHERE eid_to IN (SELECT eid FROM entities WHERE type = %%s)"
                    % rel,
                    (etype.type,),
                )

        sql(
            "DELETE FROM appears WHERE uid IN (SELECT eid FROM entities WHERE type = %s)",
            (etype.type,),
        )
