...
#-------------------------------------------------------------------------#
def exec_query_get_res(sql_query_res):
    page_limit = 100
    x = open_sql_query(sql_query_res,page_limit)
    res = x['records']
...
def get_org_id_from_email(lightning_username):
    ...
    return orgId
#-------------------------------------------------------------------------#
def serverDetails(api_url,lightning_username,lightning_pass,folder_path):
    try:
...
def mainMenu(option,arg1,arg2,arg3,lightning_username):
    if option != 10000:
        ...
        exit
#-------------------------------------------------------------------------#
def checkcontainername(container_name):
    ...
    try:
        ...
        res = exec_query_get_res(sql_containername_check)
        if res:
        ...
        logger.info(e)
#-------------------------------------------------------------------------#
def checkcontainer_pipeline_name(container_name,pipeline_name):
    ...
    try:
        ...
        res = exec_query_get_res(sql_container_pipeline_chk)
        if res:
            if isinstance(res[0],dict):
                return True
        else:
        ...
        logger.info(e)
#-------------------------------------------------------------------------#
def chk_datamart_name(datamart_name):
    ...
    try:
        ...
        res = exec_query_get_res(sql_data_mart_ck)
        if res:
            if isinstance(res[0],dict):
                return True
        else:
        ...
        logger.info(e)
#-------------------------------------------------------------------------#    
def chk_datamart_table_name(datamart_name,table_name):
    logger.info("Data Mart Name "+str(datamart_name))
...
def chk_view_name(View_name):
    ...
    try:
        ...
        res = exec_query_get_res(sql_view_name_ck)
        if res:
            if isinstance(res[0],dict):
                return True
        else:
...
def DataPipelineContainer_InsertQueries(arg1,arg2,arg3,lightning_username):
    try:
        ...
        if chk_container_name == False:
            ...
            list_queries = []
            sql_pipeline_container = """select 'INSERT INTO metastore.pipeline_container  (id, case_sensitive_name,description, name, fk_organisation_id)  VALUES (' || id || ',''' || CHAR(39)||COALESCE(case_sensitive_name, '')||CHAR(39)|| ''',''' ||CHAR(39)||COALESCE(description, '')||CHAR(39) || ''',''' ||CHAR(39)||COALESCE(name,'')||CHAR(39) || ''',' || COALESCE(fk_organisation_id, 0) ||''')'  as sql_query_con from metastore.pipeline_container where name='%s';"""%(container_name)
            res = exec_query_get_res(sql_pipeline_container)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_con']))
            
            sql_pipeline_relation = """select 'INSERT INTO metastore.pipeline_relation (id, case_sensitive_name,description, name, fk_pipeline_container_id) VALUES (' || id || ',''' || CHAR(39)||COALESCE(case_sensitive_name, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(description, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(name, '')||CHAR(39) || ''',' || COALESCE(fk_pipeline_container_id, 0) || ')' as sql_query_rel from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s');"""%(container_name)
            res = exec_query_get_res(sql_pipeline_relation)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_rel']))
                
            sql_pipeline_node="""select 'INSERT INTO metastore.pipeline_node (id, case_sensitive_name,description, name, fk_pipeline_relation_id, pipeline_type)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(case_sensitive_name, '')||CHAR(39) || ''',''' ||CHAR(39)||COALESCE(description, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(name,'')||CHAR(39) || ''',' || COALESCE(fk_pipeline_relation_id, 0) ||',''' ||char(39)|| COALESCE(pipeline_type,'')||char(39) ||''')'  as sql_query_node from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s'));"""%(container_name)
            res = exec_query_get_res(sql_pipeline_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_node']))
            
            sql_pipeline_node_schema="""select 'INSERT INTO metastore.pipeline_node_schema (id, column_alias,column_name, data_type, sql_expression, fk_pipeline_node_id)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(column_alias, '')||CHAR(39)|| ''',''' ||CHAR(39)|| COALESCE(column_name, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(data_type,'')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(sql_expression, '')||CHAR(39) ||''',' || COALESCE(fk_pipeline_node_id, 0) ||')'  as sql_query_node_schema FROM metastore.pipeline_node_schema where fk_pipeline_node_id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_pipeline_node_schema)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_pipeline_node_property="""select 'INSERT INTO metastore.pipeline_node_property (id, property_key,property_value, fk_pipeline_node_id)  VALUES (' || id || ',''' ||CHAR(39)||COALESCE(property_key, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(property_value, '')||CHAR(39) ||''',' || COALESCE(fk_pipeline_node_id, 0) ||')'  as sql_query_prop FROM metastore.pipeline_node_property  where fk_pipeline_node_id in (select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_pipeline_node_property)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_prop']))

            sql_datasource_Node="""select 'INSERT INTO metastore.pipeline_datasource(id,datasource_table) VALUES(' || id || ',''' ||CHAR(39)|| COALESCE(datasource_table, '')||CHAR(39) || ''')' as sql_query_ds FROM metastore.pipeline_datasource where id in (select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s')));"""%(container_name) 
            res = exec_query_get_res(sql_datasource_Node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_sql_table="""select 'INSERT INTO metastore.pipeline_sqltable (id, sql_query,source_tables)
             ...
             COALESCE(source_tables, '')||CHAR(39) || ''')' as sql_query_sql FROM metastore.pipeline_sqltable where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s')));"""%(container_name)
            res = exec_query_get_res(sql_sql_table)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        to_substitute = "''"
            ...
            sql_projection_node="""select 'INSERT INTO metastore.pipeline_projection (id, filter_expression,order_by, windows_spec)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(filter_expression, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(order_by, '')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(windows_spec,'')||CHAR(39)||''')'  as sql_query_proj FROM metastore.pipeline_projection where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_projection_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_join_node="""select 'INSERT INTO metastore.pipeline_join (id, filter_expression,join_predicate, join_tables, order_by)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(filter_expression, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(join_predicate, '')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(join_tables,'')||CHAR(39) ||''',''' || CHAR(39)||COALESCE(order_by,'')||CHAR(39) ||''')'  as sql_query_join FROM metastore.pipeline_join where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_join_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_join']))

            sql_aggregation_node="""select 'INSERT INTO metastore.pipeline_aggregation (id, filter_expression,group_expression, having_expression, order_by)  VALUES (' || id || ',''' ||CHAR(39)||COALESCE(filter_expression, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(group_expression, '')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(having_expression,'')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(order_by,'')||CHAR(39) ||''')'  as sql_query_agg FROM metastore.pipeline_aggregation where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_aggregation_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_agg']))

            sql_sink_node="""select 'INSERT INTO metastore.pipeline_sink (id, sink_type)  VALUES (' || id || ',''' ||char(39)|| COALESCE(sink_type, '')||char(39) ||''')'  as sql_query_sink FROM metastore.pipeline_sink where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_sink_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_sink']))

            sql_DQ_pipeline = """select 'INSERT INTO metastore.pipeline_simple_dq (id) VALUES ('||id||')' as sql_query_simdq FROM metastore.pipeline_simple_dq where id in (select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name='%s')));"""%(container_name)
            res = exec_query_get_res(sql_DQ_pipeline)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
...
def IndivdualDataPipeline_InsertQueries(arg1,arg2,arg3,lightning_username):
    try:
        ...
        if chk_con_pipe_name == False:
            ...
            list_queries = []            
            sql_pipeline_relation = """select 'INSERT INTO metastore.pipeline_relation (id, case_sensitive_name,description, name, fk_pipeline_container_id)VALUES(' || id || ',''' ||CHAR(39)|| COALESCE(case_sensitive_name, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(description, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(name, '')||CHAR(39) || ''',' || COALESCE(fk_pipeline_container_id, 0) || ')' as sql_query_rel from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s') and case_sensitive_name = '%s';"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_pipeline_relation)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_pipeline_node="""select 'INSERT INTO metastore.pipeline_node (id, case_sensitive_name,description, name, fk_pipeline_relation_id, pipeline_type)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(case_sensitive_name, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(description, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(name,'')||CHAR(39) || ''',' || COALESCE(fk_pipeline_relation_id, 0) ||',''' ||CHAR(39)|| COALESCE(pipeline_type,'')||CHAR(39) ||''')'  as sql_query_node from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s') and case_sensitive_name = '%s');"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_pipeline_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_datasource_Node="""select 'INSERT INTO metastore.pipeline_datasource(id,datasource_table) VALUES(' || id || ',''' ||CHAR(39)|| COALESCE(datasource_table, '')||CHAR(39) || ''')' as sql_query_ds FROM metastore.pipeline_datasource where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s') and case_sensitive_name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_datasource_Node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_sql_table="""select 'INSERT INTO metastore.pipeline_sqltable (id, sql_query,source_tables) VALUES(' || id || ',''' ||CHAR(39)||COALESCE(sql_query, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(source_tables, '')||CHAR(39) || ''')'  as sql_query_sql FROM metastore.pipeline_sqltable where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name = '%s') and case_sensitive_name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_sql_table)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_projection_node="""select 'INSERT INTO metastore.pipeline_projection (id, filter_expression,order_by, windows_spec)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(filter_expression, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(order_by, '')||CHAR(39) ||''',''' || CHAR(39)||COALESCE(windows_spec,'')||CHAR(39) ||''')'  as sql_query_proj FROM metastore.pipeline_projection where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s') and case_sensitive_name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_projection_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_aggregation_node="""select 'INSERT INTO metastore.pipeline_aggregation (id, filter_expression,group_expression, having_expression, order_by)  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(filter_expression, '')||CHAR(39) || ''',''' ||CHAR(39)|| COALESCE(group_expression, '')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(having_expression,'')||CHAR(39) ||''',''' ||CHAR(39)|| COALESCE(order_by,'')||CHAR(39) ||''')'  as sql_query_agg FROM metastore.pipeline_aggregation where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s') and case_sensitive_name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_aggregation_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_sink_node="""select 'INSERT INTO metastore.pipeline_sink (id, sink_type)  VALUES (' || id || ',''' ||char(39)|| COALESCE(sink_type, '')||char(39) ||''')'  as sql_query_sink FROM metastore.pipeline_sink where id in(select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from  metastore.pipeline_container where name='%s') and case_sensitive_name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_sink_node)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
            ...
            sql_DQ_pipeline = """select 'INSERT INTO metastore.pipeline_simple_dq (id) VALUES ('||id||')' as sql_query_simdq FROM metastore.pipeline_simple_dq where id in (select id from metastore.pipeline_node where fk_pipeline_relation_id in (select id from metastore.pipeline_relation where fk_pipeline_container_id in (select id from metastore.pipeline_container where name='%s') and name = '%s'));"""%(container_name,pipeline_name)
            res = exec_query_get_res(sql_DQ_pipeline)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
...
def PermanentViews_InsertQueries(arg1,arg2,arg3,lightning_username):
    try:
        ...
        if chk_viewname == False:
            ...
            sql_Permanent_View = """select 'INSERT INTO metastore.schema_store_view (id, description ,generator, materialized_table,name,query,fk_organisation_id   )  VALUES (' || id || ',''' || CHAR(39) || COALESCE(description, '') || CHAR(39) || ''',''' || CHAR(39) || COALESCE(generator, '') || char(39) || ''',''' || CHAR(39) || COALESCE(materialized_table, '') || CHAR(39) || ''',''' || CHAR(39) || COALESCE(name, '') || CHAR(39) || ''',''' || CHAR(39) || COALESCE(query, '') || CHAR(39) || ''',' || COALESCE(fk_organisation_id, 0) || ')' as sql_query_pmvw FROM metastore.schema_store_view where name = '%s';"""%(View_name)
            res = exec_query_get_res(sql_Permanent_View)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
...
def DataMart_InsertQueries(arg1,arg2,arg3,lightning_username):
    try:
        ...
        if chk_dm_name == False:
            ...
            list_queries = []
            sql_data_mart = """SELECT 'INSERT INTO metastore.data_mart (id, case_sensitive_name ,description, name , fk_organisation_id ) VALUES (' || id || ',''' || CHAR(39)|| COALESCE(case_sensitive_name, '') ||CHAR(39)|| ''',''' ||CHAR(39)|| COALESCE(description, '')||char(39)||''','''||char(39)||COALESCE(name,'')||CHAR(39) ||''','|| COALESCE(fk_organisation_id,'') ||')' as sql_query_dm FROM metastore.data_mart where name= '%s';"""%(datamart_name)
            res = exec_query_get_res(sql_data_mart)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
                        list_queries.append(str(i['sql_query_dm']))

            sql_data_mart_table = """SELECT 'INSERT INTO metastore.data_mart_table (id, case_sensitive_name , name , source_table  ,fk_data_mart_id  )  VALUES (' || id || ',''' ||CHAR(39)|| COALESCE(case_sensitive_name, '')||CHAR(39)|| ''',''' ||CHAR(39)|| COALESCE(name, '')||CHAR(39)||''',''' ||CHAR(39)||COALESCE(source_table ,'')||CHAR(39) ||''','|| COALESCE(fk_data_mart_id,0) ||')'  as sql_query_dm_table FROM metastore.data_mart_table where fk_data_mart_id in (select id from metastore.data_mart where name = '%s');"""%(datamart_name)
            res = exec_query_get_res(sql_data_mart_table)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
...
def DataMart_IndTable_InsertQueries(arg1,arg2,arg3,lightning_username):
    try:
        ...
        if chk_dm_name == False:
            ...
            list_queries = []
            sql_data_mart_table = """SELECT 'INSERT INTO metastore.data_mart_table (id, case_sensitive_name , name , source_table  ,fk_data_mart_id  )  VALUES (' || id || ',''' || CHAR(39) || COALESCE(case_sensitive_name, '') || CHAR(39) || ''',''' || CHAR(39) || COALESCE(name, '') || CHAR(39) || ''',''' || CHAR(39) || COALESCE(source_table, '') || CHAR(39) || ''',' || COALESCE(fk_data_mart_id, 0) || ')' as sql_query_dm FROM metastore.data_mart_table where fk_data_mart_id in ( select id from metastore.data_mart where name = '%s') and name='%s';"""%(datamart_name,table_name)
            res = exec_query_get_res(sql_data_mart_table)
            if res:
                if isinstance(res[0],dict):
                    for i in res:
