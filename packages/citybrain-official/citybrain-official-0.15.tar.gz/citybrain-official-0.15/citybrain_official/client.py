...
def retrieve_raw(dataAddress: str = '', sql: str = '', sqlHints: Dict[str, str] = None) -> any:
    if sql == '':
        if dataAddress == '':
        ...
        taskID = __create_sql_task(dataAddress=dataAddress, sql=sql, sqlHints=sqlHints)
...
def retrieve_df(dataAddress: str = '', sql: str = '', sqlHints: Dict[str, str] = None) -> pandas.DataFrame:
    if sql == '': # file data
        if dataAddress == '':
            raise Exception('must specify data address')
...
def __create_sql_task(dataAddress: str, sql: str, sqlHints: Dict[str, str]) -> str:
    endpoint = __get_data_endpoint()
    dataAddress = __get_odps_dataaddress(dataAddress=dataAddress)

    if sqlHints is not None:
        reqBody = {'dpAddress': dataAddress,'payload': json.dumps({'payload': sql, 'action':'TASK_Create', 'settings':sqlHints})}
    else:
