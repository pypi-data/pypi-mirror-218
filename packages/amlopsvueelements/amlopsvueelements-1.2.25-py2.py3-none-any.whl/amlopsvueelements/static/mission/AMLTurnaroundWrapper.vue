    ...
    <div :class="[$style['ops-aml-turnaround__content']]">
      <div :class="$style['ops-aml-turnaround__servicing-checkbox']">
        <div @click.stop.prevent.capture="onHandleAmlServicing">
          <UCheckboxWrapper
            :label-text="
              missionFormModel.legs[legIndex]?.servicing
                ? `AML Servicing at ${arrivalAirportShortName}`
                : 'AML Servicing'
            "
            :model-value="isAmlServicingEnabled"
            class="!mb-0 !gap-y-5 flex flex-col text-center"
          />
        </div>
      </div>
      <div class="min-h-[23rem] w-full pb-[1.375rem] relative px-4">
        <div v-if="missionFormModel.legs[legIndex]?.servicing" class="h-full">
          ...
          <div class="flex items-center mt-4">
            ...
            <USelectWrapper
              v-model="missionFormModel.legs[legIndex].servicing.fuel_unit"
          ...
          <div class="flex font-medium text-[1.25rem] text-grey-1000">
            <div class="flex flex-col items-center w-full">
              <div class="flex items-start w-full justify-between relative">
                <USelectWrapper
                ...
                <div class="flex mt-2 mr-2">
                  ...
                  <div
                    ...
                    class="flex justify-between gap-6 font-normal text-base items-center"
                  >
                    <span>Arr</span>
                    <span>Dep</span>
                  </div>
              ...
              <div
                v-if="missionFormModel.legs[legIndex]?.servicing?.services?.length"
                class="w-full border border-grey-100 rounded-md p-2"
              >
                <div
                  ...
                  :class="
                    !service.is_allowed_free_text &&
                    !service.is_allowed_quantity_selection &&
                    missionFormModel.legs[legIndex]?.servicing?.services?.length > 1 &&
                    'border-b border-dotted'
                  "
                >
                  <div class="flex items-center justify-between w-full">
                    <div class="max-w-[310px] w-[76%]">
                      <div class="flex items-center w-full h-[22px] mb-2 last:!mb-0">
                        <img
                          v-if="!isDisabledService(service.service)"
                        ...
                        <ULabel
                          text-class="mb-0"
                          :required="false"
                          :label-text="findServiceById(service.service).name"
                        />
                      </div>
                    </div>
                    <div class="mt-2">
                      <div class="flex justify-between gap-[0.75rem] items-center pr-1 mb-2">
                        <UCheckboxWrapper
                          v-model="service.on_arrival"
                          :disabled="
                            isDisabledField ||
                            !findServiceById(service.service).arrival ||
                            isDisabledService(service.service)
                          "
                          class="gap-0 mr-2 !mb-0 d-flex"
                          size="24px"
                        />
                        <UCheckboxWrapper
                          ...
                          :disabled="
                            isDisabledField ||
                            !findServiceById(service.service).departure ||
                            isDisabledService(service.service)
                          "
                          class="gap-0 !mb-0"
                          size="24px"
                  ...
                  <div v-if="service.is_allowed_free_text" class="flex items-center px-1 mb-2">
                    <p class="text-base whitespace-nowrap min-w-[9rem]">Description:</p>
                    <UInputWrapper v-model="service.booking_text" maxlength="255" class="!mb-0" />
                  </div>
                  <div
                    v-if="service.is_allowed_quantity_selection"
                    class="flex items-center px-1 mb-2"
                  >
                    <p class="text-base whitespace-nowrap min-w-[9rem]">Quantity:</p>
                    <UInputWrapper v-model="service.booking_quantity" type="number" class="!mb-0" />
                    <span class="text-sm lowercase ml-2">{{
                      service?.quantity_selection_uom?.description_plural
                    }}</span>
                  </div>
                </div>
              </div>
...
const computedFilteredServiceOptions = computed<IService[]>(() => {
  ...
  return serviceOptions.value?.filter((option: IService) => {
    return !(computedLegInfo.value?.servicing?.services as ILegService[])?.some(
      (service) => service?.service === option.id
    )
  })
})
...
const findServiceById = (
  id: string
): { name: string; code: string; arrival: boolean; departure: boolean } => {
  const res = (serviceOptions.value as IService[])?.find((option: IService) => option.id === id)
  const availability = res?.attributes?.availability_bool?.find(
    (item) => item.airport === computedLegInfo.value.arrival_location?.id
  )
  return {
    arrival: true,
    departure: true,
...
const isDisabledService = (serviceId: string) =>
  turnaroundNonEditableServices.some(
    (serviceCode) => serviceCode === findServiceById(serviceId).code
  )
...
.ops-aml-turnaround {
  ...
  &__content {
    @apply flex font-medium text-[1.25rem] h-auto  text-grey-1000;
    &__delete-icon {
      @apply w-[18px] mr-2 cursor-pointer aspect-square;
      filter: invert(23%) sepia(85%) saturate(2552%) hue-rotate(330deg) brightness(87%)
        contrast(103%);
    }
  }
  &__servicing-checkbox {
    @apply flex justify-center items-center border-r border-grey-opacity-800/25 min-w-[10rem] px-2;
  }
  &__error {
    @apply absolute z-50 opacity-100 text-[14px] text-[#97650e] w-[97%] text-center p-4 rounded-[0.5rem] top-[48%] right-0 -translate-y-[50%] border-[#fee5ba] bg-[#feeed1];
  }
}
</style>
