      ...
      <div class="flex w-1/2 flex-col">
        <div>
          <UCheckboxWrapper
            ...
            @update:model-value="onHandleLegServicing($event, passengerService?.id)"
          />
          <UInputWrapper
      ...
      <div class="flex w-1/2 flex-col">
        <div>
          <UCheckboxWrapper
            ...
            label-text="Cargo?"
            @update:model-value="onHandleLegServicing($event, cargoService?.id)"
          />
          <UInputWrapper
...
const passengersCheckbox = computed({
  get: () => {
    ...
    return isServiceSelected(passengerService.value?.id) || !!currentLeg?.pob_pax
  },
  set: (value) => {
...
const cargoCheckbox = computed({
  get: () => {
    const currentLeg = missionFormModel.value?.legs?.[props.legIndex]
    return isServiceSelected(cargoService.value?.id) || !!currentLeg?.cob_lbs
  },
  set: (value) => {
