            ...
            if isinstance(item_, SceneGridItem) or isinstance(item_, CoordinateItem):  # 点击的位置没有其他组件
                if self.state == "常规":
                    ...
                    if idx != 0:  # 如果组件列表中选中任何组件，则添加组件到场景中
                        ...
                        if pipe is not None:
                            # todo 管线点无法移动的问题
                            # 在PyQt6中，使用pipe.mousePressEvent(event)，会导致管线的点无法移动，测试发现不会出发PipeItem.mouseMoveEvent事件
                            # 但如果使用super().mousePressEvent(event)，会导致多个管线重叠时，无法正确处理选中的管线。
                            # 解决方法：无论哪条语句，均会触发YkGraphicsScene.mouseMoveEvent()，则可以有Scene的事件中调用pipe对象
                            # 的移动事件
                            # pipe.mousePressEvent(event)
                            super().mousePressEvent(event)
                            # self.dragging_pipe = pipe
...
class YkGraphicsView(QGraphicsView):
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:
        ...
        :return:
        """
        print("mouse press event from View")
    ...
    def mouseMoveEvent(self, event: QtGui.QMouseEvent) -> None:
        # print("mouse move event from YkGraphicsView")
        if self.scene().state == "绘制管道":
