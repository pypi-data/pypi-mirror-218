...
from alignn.ff.ff import default_path, ev_curve, AlignnAtomwiseCalculator
...
for i, j in zip(si, nmsi):
    ...
    x = ev_curve(
        atoms, stress_wt=stress_wt,model_path=model_path, on_relaxed_struct=on_relaxed_struct
    )
    info = {}
    info["data"] = x
    info["jid"] = i
...
for i, j in zip(mote2, nmmote2):
    ...
    print(dat["jid"], j, dat["bulk_modulus_kv"], atoms.num_atoms)
    x = ev_curve(
        atoms, model_path=model_path,stress_wt=stress_wt, on_relaxed_struct=on_relaxed_struct
    )
    info = {}
    info["data"] = x
    info["jid"] = i
