    ...
    for code_ref in code_refs:
        ...
        if code_ref.protocol == "http":
            ...
            if code_ref.ref_ssh.private_key:
                ...
                subprocess.run(["git", "clone", code_ref.ref_ssh.host, str(mount_path)]).check_returncode()

        if code_ref.git_ref:
            subprocess.run(["git", "checkout", code_ref.git_ref], cwd=str(mount_path)).check_returncode()
