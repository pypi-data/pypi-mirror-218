...
def create_workspace(
    ...
    **kwargs,
    ...
    if local_files:
        requests.extend(_configure_volume_mount_request_local_files(local_files, use_vesslignore, **kwargs))
...
def connect_workspace_ssh(private_key_path: str) -> None:
    ...
    os.system(
        f"ssh -p {ssh_endpoint.port}{ssh_private_key_option} " f"root@{ssh_endpoint.hostname}"
    )
