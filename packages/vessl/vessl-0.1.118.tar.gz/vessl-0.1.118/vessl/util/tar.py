...
class Tar:
    ...
    def gzip_using_vesslignore(
        ...
        commands.extend(["--no-recursion"])       # we choose which files to add; do not add all files when given a directory
        for exclude_path in exclude_paths:
            commands.extend(["--exclude", exclude_path])
        ...
        all_files_to_include = list_files_using_vesslignore(path_to_zip, vesslignore_filename=vesslignore_filename)

        subprocess.run(commands,
                       input='\n'.join(all_files_to_include),
