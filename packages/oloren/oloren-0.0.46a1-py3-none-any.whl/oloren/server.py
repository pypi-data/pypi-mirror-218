...
def download_from_registered_file(path, token=None):
    purl = requests.post(
        data=json.dumps({"path": path}),
        headers={"Content-Type": "application/json", "Authorization": f"Bearer {token}"},
    )
    print(purl)
    return download_from_signed_url(purl.json())
...
__all__ = ["register", "run", "handler", "upload_file", "upload_file_purl", "download_from_signed_url", "download_from_file_record", "download_from_registered_file" "map"]
