...
class TelemetryConsumerFilter(Filter):
    ...
    def invoke(self, invoker: Invoker, invocation: Invocation) -> Any:
        ...
        if not telemetry.if_rebuild_span(attachments):
            return invoker.run(invocation)
        trace_id = Mesh.context().get_trace_id()
        span_id = Mesh.context().get_span_id()
        with telemetry.build_via_remote(attachments, invocation.get_inspector().get_name()):
            Mesh.context().get_attachments()['mesh-telemetry-trace-id'] = \
                hex(telemetry.get_current_span().get_span_context().trace_id)
            current_span = telemetry.get_current_span()
            current_span.set_attribute('mesh-trace-id', trace_id)
            current_span.set_attribute('mesh-span-id', span_id)
            return invoker.run(invocation)
