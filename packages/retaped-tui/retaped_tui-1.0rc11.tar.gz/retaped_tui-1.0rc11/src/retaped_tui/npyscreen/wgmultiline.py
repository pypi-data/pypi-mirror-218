...
class FilterPopupHelper(Popup.Popup):
    def create(self):
        ...
        self.nextrely += 1
        self.statusline = self.add(
            textbox.Textfield, color='LABEL', editable=False)
...
object to be passed to the contained widget."""
    ...
    def update(self, clear=True):
        ...
        if not no_change or clear or self.never_cache:
            ...
            if (len(self.values) <= indexer+1):
                ...
                if self.do_colors():
                    self.parent.curses_pad.addstr(
                        self.rely+self.height-1, self.relx, MORE_LABEL, self.parent.theme_manager.findPair(self, 'CONTROL'))
                else:
                    self.parent.curses_pad.addstr(
            ...
            if self.editing or self.always_show_cursor:
                ...
                self._my_widgets[(self.cursor_line -
                                  self.start_display_at)].update(clear=True)
            else:
    ...
    def move_previous_filtered(self, *args):
        ...
        _filtered_values_cache_reversed = copy.copy(
            self._filtered_values_cache)
    ...
    def handle_mouse_event(self, mouse_event):
        ...
        mouse_id, rel_x, rel_y, z, bstate = self.interpret_mouse_event(
            mouse_event)
    ...
    def h_cursor_page_down(self, ch):
        ...
        if not (self.start_display_at + len(self._my_widgets) - 1) > len(self.values):
            ...
            if self.start_display_at > len(self.values) - (len(self._my_widgets)-1):
                self.start_display_at = len(
                    self.values) - (len(self._my_widgets)-1)
...
class MultiLineActionWithShortcuts(MultiLineAction):
    ...
    def set_up_handlers(self):
        ...
        self.add_complex_handlers(
            ((self.h_find_shortcut_action, self.h_execute_shortcut_action),))
...
class BufferPager(Pager):
    ...
    def buffer(self, lines, scroll_end=True, scroll_if_editing=False):
        ...
        if scroll_end:
            if not self.editing:
                self.start_display_at = len(
                    self.values) - len(self._my_widgets)
