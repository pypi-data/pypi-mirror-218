...
def buildChannelCache(data: dict):
    for i in data:
        ...
        tmpChannel.desc = i['description'] if 'description' in i.keys(
        ) else False
...
def wsInit():
    ...
    globals.websocket.send(json.dumps(
        {"type": "Authenticate", "token": globals.token}))
    ...
    def run(self):
        ...
        while True:
            ...
            match(message['type']):
                ...
                case 'MessageDelete':
                    if message['channel'] == globals.currentChannel:
                        globals.messageBox.deleteMessage(message)
                        globals.messageBox.update(globals.messageBox)
                case 'Ready':
...
def sendMessage(totallyanargument):
    ...
    if not globals.editingMessage:
        ...
        if message.status_code != 200:
            npyscreen.notify(
                f'Status code: {message.status_code}', title='Failed to send')
            return 1
    else:
        message = requests.patch(f'{globals.apiEndpoint}/channels/{globals.currentChannel}/messages/{globals.editingMessageMsg.id}',
                                 headers={'x-session-token': globals.token}, data=json.dumps({'content': input}))
        if message.status_code != 200:
            npyscreen.notify(
                f'Status code: {message.status_code}', title='Failed to send')
            return 1
...
def deleteMessage(totallyanargument):
    message = globals.messageBox.values[globals.highlightedIndex]
    returnVal = requests.delete(f'{globals.apiEndpoint}/channels/{globals.currentChannel}/messages/{message.id}',
                                headers={'x-session-token': globals.token})
    try:
        returnVal.json()
        npyscreen.notify(title='Message delete: permission denied', message='')
    except:
        npyscreen.notify(title='Successfully deleted message', message='')
        time.sleep(1)
...
class MainForm(npyscreen.FormBaseNew):
    def create(self):
        ...
        globals.serverList = self.add(
            widgets.serverBox, relx=1, rely=1, width=self.max_x//6, height=self.max_y//2)
        globals.channelList = self.add(
            widgets.channelBox, relx=1, width=self.max_x//6, height=self.max_y//2-2)
        globals.messageBox = self.add(widgets.messageBox, relx=self.max_x //
                                      6+1, rely=1, height=self.max_y-6, width=self.max_x-self.max_x//6-2)
        globals.inputBox = self.add(widgets.inputTextBox, relx=self.max_x //
                                    6+1, name='', rely=-5, height=4, width=self.max_x-self.max_x//6-2)
        ...
        globals.messageBox.add_handlers({"d": deleteMessage})
        ...
        try:
            ...
            npyscreen.notify('Failed to request user profile',
        ...
        globals.localUser.username = f'{tmpUser["username"]}#{tmpUser["discriminator"]}'
