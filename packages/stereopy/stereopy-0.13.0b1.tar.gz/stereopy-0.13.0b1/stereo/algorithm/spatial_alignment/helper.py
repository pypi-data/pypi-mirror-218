...
extract_data_matrix = lambda data, rep: data.exp_matrix if rep is None else data.tl.result[rep].to_numpy()
...
def stack_slices_pairwise(
    ...
    if not output_params:
        ...
        S1, S2,theta,tX,tY  = generalized_procrustes_analysis(slices[0].position, slices[1].position, pis[0],output_params=output_params, matrix=matrix)
        thetas.append(theta)
    ...
    for i in range(1, len(slices) - 1):
        if not output_params:
            ...
            x, y,theta,tX,tY = generalized_procrustes_analysis(new_coor[i], slices[i+1].position, pis[i],output_params=output_params, matrix=matrix)
            thetas.append(theta)
            translations.append(tY)
        new_coor.append(y)
...
def stack_slices_center(
    ...
    for i in range(len(slices)):
        if not output_params:
            ...
            c, y,theta,tX,tY = generalized_procrustes_analysis(center_slice.position, slices[i].position, pis[i],output_params=output_params, matrix=matrix)
            thetas.append(theta)
            translations.append(tY)
        new_coor.append(y)
