...
from .cell_cell_communication.exceptions import PipelineResultInexistent, InvalidMicroEnvInput
    ...
    def main(
        ...
        if threshold is None:
            ...
            if output_path is not None:
                ...
                output_path = os.path.join(output_path, f'micro_envs_{time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())}')
                if not os.path.exists(output_path):
                    os.makedirs(output_path)
            
            cell_type = self.pipeline_res[cluster_res_key]['group']
            meta = pd.DataFrame({'cell': self.stereo_exp_data.cells.cell_name, 'cell_type': cell_type}).reset_index(drop=True)
            ...
            if dimension == 3:
                if self.stereo_exp_data.position_z is None:
                    raise InvalidMicroEnvInput("The position of cells must has the third dimension while setting `dimension` to 3.")
            ...
            if show_dividing_by_thresholds:
                ...
                split_by_different_threshold_copy['subgroup_result'] = split_by_different_threshold_copy['subgroup_result'].astype('U')
                return pn.widgets.DataFrame(split_by_different_threshold_copy, disabled=True, show_index=False, autosize_mode="fit_viewport", frozen_columns=1)
        else:
            ...
            gme = GetMicroEnvs()
            micro_envs = gme.generate_micro_envs(method, threshold=threshold, result_df=result_df, output_path=output_path)
