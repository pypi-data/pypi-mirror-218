...
from stereo.utils.hvg_utils import get_mean_var
...
def pca(
    ...
    adata : anndata.AnnData
        ...
        `.uns['pca']['variance']`
             ...
             covariance matrix.
    """
    logg_start = logger.info('computing PCA')
    ...
    if chunked:
        if not zero_center or random_state or svd_solver != 'arpack':
            logger.debug('Ignoring zero_center, random_state, svd_solver')
...
def _pca_with_sparse(X, npcs, solver='arpack', mu=None, random_state=None):
    ...
    total_var = get_mean_var(X)[1].sum()
