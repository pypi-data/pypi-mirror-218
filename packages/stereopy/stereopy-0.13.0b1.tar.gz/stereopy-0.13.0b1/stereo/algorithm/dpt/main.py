...
def _diffmap(stereo_exp_data, n_comps=15, neighbors_key=None, random_state=0):
    logger.info(f'computing Diffusion Maps using n_comps={n_comps}(=n_dcs)')
    dpt = DPTClass(stereo_exp_data, neighbors_key=neighbors_key)
    dpt.compute_transitions()
    dpt.compute_eigen(n_comps=n_comps, random_state=random_state)
    stereo_exp_data.tl.result['X_diffmap'] = dpt.eigen_basis
    stereo_exp_data.tl.result['diffmap_evals'] = dpt.eigen_values
    logger.info(
        '    finished\n'
        +
        'added\n'
...
class DPTClass(Neighbors):
    ...
    def __init__(
        ...
        super().__init__(stereo_exp_data, n_dcs=n_dcs, neighbors_key=neighbors_key)
        self.flavor = 'haghverdi16'
        self.n_branchings = n_branchings
        self.min_group_size = (
            min_group_size
            if min_group_size >= 1
            else int(min_group_size * self.stereo_exp_data.shape[0])
        )
        self.passedstereo_exp_data = stereo_exp_data  # just for debugging purposes
    ...
    def select_segment(self, segs, segs_tips, segs_undecided) -> Tuple[int, int]:
        ...
        allindices = np.arange(self.stereo_exp_data.shape[0], dtype=int)
    ...
    def detect_branchings(self):
        ...
        indices_all = np.arange(self.stereo_exp_data.shape[0], dtype=int)
    ...
    def postprocess_segments(self):
        ...
        for iseg, seg in enumerate(self.segs):
            mask = np.zeros(self.stereo_exp_data.shape[0], dtype=bool)
            mask[seg] = True
            self.segs[iseg] = mask
        # convert to arrays
    ...
    def set_segs_names(self):
        ...
        segs_names = np.zeros(self.stereo_exp_data.shape[0], dtype=np.int8)
        ...
        if self.iroot is not None:
            for itips, tips in enumerate(self.segs_tips):
                if tips[0] != -1:
                    ...
                    logger.debug(f'    group {itips} is very small')
    ...
    def _detect_branching(
        ...
        for inewseg, newseg in enumerate(ssegs):
            if len(np.flatnonzero(newseg)) <= 1:
                logger.warning(f'detected group with only {np.flatnonzero(newseg)} cells')
    ...
    def detect_branching(
        ...
        if self.flavor == 'haghverdi16':
            ...
            prev_connecting_points = segs_connects[  # noqa: F841  TODO Evaluate whether to assign the iable or not
                iseg
            ]
...
class DPT(AlgorithmBase):
    ...
    def main(
        ...
        if neighbors_key not in stereo_exp_data.tl.result:
            raise ValueError('You need to run `pp.neighbors` and `tl.diffmap` first.')
        if 'iroot' not in stereo_exp_data.tl.result and 'xroot' not in stereo_exp_data.genes:
            logger.warning(
                'No root cell found. To compute pseudotime, pass the index or '
                'expression vector of a root cell, one of:\n'
                '    stereo_exp_data.tl.result[\'iroot\'] = root_cell_index\n'
                '    stereo_exp_data.genes[\'xroot\'] = stereo_exp_data[root_cell_name, :].X'
            )
        if 'X_diffmap' not in stereo_exp_data.cells_matrix.keys():
            logger.warning(
                'Trying to run `tl.dpt` without prior call of `tl.diffmap`. '
                'Falling back to `tl.diffmap` with default parameters.'
            )
            _diffmap(stereo_exp_data, neighbors_key=neighbors_key)
        ...
        if n_branchings > 0:
            ...
            stereo_exp_data.cells['dpt_groups'] = pd.Categorical(
            ...
            stereo_exp_data.tl.result['dpt_changepoints'] = dpt.changepoints
            # the tip points of segments
            stereo_exp_data.tl.result['dpt_grouptips'] = dpt.segs_tips
            # the ordering according to segments and pseudotime
            ordering_id = np.zeros(stereo_exp_data.shape[0], dtype=int)
            for count, idx in enumerate(dpt.indices):
                ordering_id[idx] = count
            stereo_exp_data.cells['dpt_order'] = ordering_id
            stereo_exp_data.cells['dpt_order_indices'] = dpt.indices
        logger.info(
            '    finished\n'
            +
            'added\n'
            ...
            + (
                "\n    'dpt_groups', the branching subgroups of dpt (stereo_exp_data.cells)"
                "\n    'dpt_order', cell order (stereo_exp_data.cells)"
                if n_branchings > 0
                else ''
            )
        )
