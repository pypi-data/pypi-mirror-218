...
from ailab.atp_dataset.modelscope_api import MSDatasetAPI
...
class AILabDataset :
    ...
    def __init__(self,
                 ds_instance: Union[MsDataset, Dataset, IterableDataset]):
    ...
    def to_ailab_dataset(cls,
                      ds_ins: Union[dict, MsDataset, DatasetDict, Dataset, IterableDatasetDict, IterableDataset],
                      ) -> 'AILabDataset':
        if isinstance(ds_ins, MsDataset) :
            return cls(ds_ins)
        elif isinstance(ds_ins, Dataset) :
            return cls(ds_ins)
    ...
    def to_hf_dataset(self) -> Dataset:
        if isinstance(self.ds_instance, MsDataset) :
            return self.ds_instance.to_hf_dataset()
        else :
            return self._hf_ds
    ...
    def load_dataset(dataset_name: Union[str, list],
        ...
        **kwargs) -> Union[dict, 'AILabDataset']:
        ...
        if is_local_path :
            ...
            """
            2.2如果文件不存在,判断src的来源
            2.2.1 如果来源是modelscope 调用modelscope的download dataset
            """
        elif src == Sources.modelscope:
            ms_set = MSDatasetAPI.load(dataset_name, namespace='modelscope')
            dataset_inst = AILabDataset.to_ailab_dataset(ms_set)
            return dataset_inst
            """
