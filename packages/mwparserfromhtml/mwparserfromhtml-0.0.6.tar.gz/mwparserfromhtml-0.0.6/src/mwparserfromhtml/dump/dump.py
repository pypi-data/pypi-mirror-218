...
class HTMLDump:
    ...
    def __init__(self, filepath: str, max_article: int = 100) -> None:
        """
    ...
    def read_dump_local(self, filepath: str, max_article: int = -1):
        """
        Reads a local dump file and returns an iterator of the rows.
        Args:
            filepath (str): path to the dump file
            max_article (int, optional): maximum number of articles to return.
            Defaults to -1, which means we will iterate over the entire dump file.
        Returns:
