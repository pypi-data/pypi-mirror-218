...
version[0]      Print versions of excelrd and Python and exit
...
if __name__ == "__main__":
            ...
            for colx, ty, val, cxfx in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print(
                        ...
                        % (excelrd.colname(colx), rowx + 1, ty, val, cxfx)
                    )
        else:
            for colx, ty, val, _unused in get_row_data(bk, sh, rowx, colrange):
                if printit:
                    print(
                        "cell %s%d: type=%d, data: %r" % (excelrd.colname(colx), rowx + 1, ty, val)
                    )
    ...
    def show(bk, nshow=65535, printit=1):
        bk_header(bk)
        if 0:
            rclist = excelrd.sheet.rc_stats.items()
    ...
    def count_xfs(bk):
        ...
        for shx in range(bk.nsheets):
            ...
            nrows = sh.nrows
            print(
                "sheet %d: name = %r; nrows = %d; ncols = %d" % (shx, sh.name, sh.nrows, sh.ncols)
            )
    ...
    def main(cmd_args):
        ...
        oparser.add_option(
            "-s", "--onesheet", default="", help="restrict output to this sheet (name or index)"
        )
        oparser.add_option(
            "-u",
            "--unnumbered",
