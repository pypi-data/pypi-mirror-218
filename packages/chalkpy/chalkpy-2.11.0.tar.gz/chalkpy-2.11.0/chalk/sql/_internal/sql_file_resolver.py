...
def _get_sql_glot(sql_string: str, path: str, sources: Iterable[BaseSQLSource]) -> GlotResult:
    ...
    if len(comments) == 0:
        errors.append(
            ResolverError(
                display=f"SQL file resolvers require comments that describe key-value pairs in YAML form. "
                f"See https://docs.chalk.ai/docs/sql#sql-file-resolvers for reference",
                path=path,
                parameter=None,
            )
        )
