...
def is_batch_model(model_name):
    return False if model_name.startswith("chat/") or model_name == "gpt-4" or model_name == "chat-gpt" else True


class Augmenter:
    def __init__(self, seed_data, model_name="lamini/open", related_data=None, schema=None, config={}):
        self.seed_data = seed_data
        self.seed_inputs = self.__get_inputs()

        self.model_name = model_name
        self.batch = is_batch_model(self.model_name)
        self.llm = LLMEngine(id="lamini-augmenter", model_name=self.model_name, config=config)
        self.llm.clear_data()
    ...
    def run(self, n, verbose=False):
        if self.batch:
            ...
            for i in tqdm(range(0, n, batch_size)):
                ...
                data_to_llm = [self.seed_inputs[j % len(self.seed_inputs)] for j in range(i, min(i + batch_size, n))]

                attempts = 5
                outs = None
                for _ in range(attempts):
                    try:
                        outs = self.llm(data_to_llm, AugmenterOutput, temperature=0.7, schema=self.schema)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not outs:
                    raise RuntimeError("Too many Lamini API errors.")
                outs = [self.input_type(**{self.first_attribute: out.new}) for out in outs]
            ...
            for i in tqdm(range(n)):
                ...
                for _ in range(attempts):
                    try:
                        out = self.llm(datum_to_llm, AugmenterOutput, temperature=0.7, schema=self.schema)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not out:
                    raise RuntimeError("Too many Lamini API errors.")
...
class AnswerAugmenter:
    def __init__(self, questions, output_type=None, model_name="lamini/instruct", seed_data=None, related_data=None, schema=None, config={}):
        ...
        self.model_name = model_name
        self.batch = is_batch_model(self.model_name)
        self.llm = LLMEngine(id="lamini-answer-augmenter", model_name=self.model_name, config=config)
        self.llm.clear_data()
        data = []
        if seed_data:
            data += seed_data
    ...
    def run(self, verbose=False):
        if self.batch:
            ...
            for i in tqdm(range(0, len(self.questions), batch_size)):
                ...
                data_to_llm = [datum[0] if isinstance(datum, list) else datum for datum in self.questions[i:i + batch_size]]
                attempts = 5
                answers = None
                for _ in range(attempts):
                    try:
                        answers = self.llm(data_to_llm, self.output_type, schema=self.schema)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not answers:
                    raise RuntimeError("Too many Lamini API errors.")
            ...
            for datum in tqdm(self.questions):
                ...
                for _ in range(attempts):
                    try:
                        answer = self.llm(datum_to_llm, self.output_type, schema=self.schema)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not answer:
                    raise RuntimeError("Too many Lamini API errors.")
...
class QuestionAnswerAugmenter: # TODO: eventually inherit from generic DataAugmenter class
    ...
    def __init__(self, seed_data, question_model_name="lamini/open", answer_model_name="lamini/instruct", related_data=None, schema=None, config={}):
    ...
    def run(self, n, verbose=False):
        augmented_questions = Augmenter(self.seed_data, model_name=self.question_model_name, related_data=self.related_data, schema=self.schema, config=self.config).run(n, verbose=verbose)
        augmented_answers = AnswerAugmenter(augmented_questions, output_type=self.answer_type, model_name=self.answer_model_name, seed_data=self.seed_data, related_data=self.related_data, schema=self.schema, config=self.config).run(verbose=verbose)
