...
from abaqusConstants import PNG, ON, OFF, COMPONENT, INVARIANT, NODAL, INTEGRATION_POINT, ELEMENT_NODAL
...
from numpy import array, min, max, mean, empty, savetxt, concatenate
from os import path
import logging
import glob
import itertools
...
class Abml_Session:
    ...
    def historyoutput(self, data, odb):
        ...
        if isinstance(data["step"], str):
            ...
            step_time = odb.steps[odb.steps.keys()[data["step"]]].totalTime + odb.steps[odb.steps.keys()[data["step"]]].timePeriod

        outputVariableName_map = {
            "allse": "Strain energy: ALLSE for Whole Model"
        }
    ...
    def fieldoutput(self, data, odb):
        ...
        xydatalist = session.xyDataListFromField(odb=odb, outputPosition=data["outputposition"], variable=variable, nodeSets=data["nodesets"])

        table_data = concatenate(map(get_data, xydatalist, [step_time]*len(xydatalist))).flatten()
    ...
    def create_extractions(self, **extractions):
        ...
        for line, odb_name in enumerate(session.odbs.keys()):
            ...
            for col, data in enumerate(extractions.values()):
                ...
                df[line, col + 1] = type_map[type_](data, odb)
