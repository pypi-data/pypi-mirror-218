...
class create():
    ...
    def text(prompt, text = 'Firefly', font = 'acumin-pro-wide', background = 'transparent', fit = 'tight'):
...
class random():
    ...
    class color():
        def name(dest='en'):
            if dest.startswith('en'): return choice(info.colors)
...
class info():
    measures_compare_with_byte = {'microbit': 0.000001, 'milibit': 0.001, 'base': 0.0625, 'bit': 0.125, 'nibble': 0.5, 'byte': 1, 'octet': 1, 'kilobyte': 1024, 'megabyte': 1048576, 'gigabyte': 1073741824, 'terabyte': 1099511627776, 'petabyte': 1125899906842624, 'exabyte': 1152921504606846976}
    colors = [
    ...
    class image():
        ...
        def size(image):
            file = imread(image)
            return {'height': file.shape[1], 'width': file.shape[0], 'channels': file.shape[2], 'pixels': file.size}
