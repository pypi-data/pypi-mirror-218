...
class Validator:
    ...
    def validate_comparison_operations(value: Any, comparison_operations: dict[str: int | None]) -> Any | Exception:
        ...
        if result is True:
            ...
            raise ValidationError(
                f"Comparison Failed for ({type(value) = }, {value = }) with  {comparison_operations = }"
            )
    ...
    def validate_age(
        ...
        comparison_operations: dict[str: int | None],
    ) -> str | int | Exception:
        """
    ...
    def validate_email(
        ...
        try:
            ...
            raise ValidationError(f"Invalid Email ({type(value)}), {value}) | {str(err) = }")
    ...
    def validate_ssn(value: str):
        ...
        match len(digits_only):
            ...
            case _:
                raise ValidationError(f"SSN Validation Failed: ({type(value)}, {value})")
    ...
    def validate_zip_code(value: str):
        ...
        match len(digits_only):
            ...
            case _:
                raise ValidationError(
                    f"Zip Code Validation Failed: ({type(value)}, {value}) | ZipCode({digits_only}) does not have 5 digits (Actual: {len(digits_only)})"
                )
