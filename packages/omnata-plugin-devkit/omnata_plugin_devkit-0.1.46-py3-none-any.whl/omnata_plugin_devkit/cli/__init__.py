...
def upload(
    ...
    config.connect_to_snowflake(connection_name=environment)
    uploader = PluginUploader(snowflake_connection=config.snowflake_connection.ctx)
    ...
    packager = NativeAppPackaging(
        snowflake_connection=config.snowflake_connection.ctx,
