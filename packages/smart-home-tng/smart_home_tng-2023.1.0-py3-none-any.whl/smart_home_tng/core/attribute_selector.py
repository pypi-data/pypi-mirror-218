...
class AttributeSelector(Selector):
    ...
    _CONFIG_SCHEMA: typing.Final = vol.Schema({vol.Required("entity_id"): cv.entity_id})

    def config_schema(self, config: typing.Any) -> typing.Callable:
        return AttributeSelector._CONFIG_SCHEMA(config)

    def __init__(self, config: AttributeSelectorConfig) -> None:
        """Instantiate a selector."""
        super().__init__("attribute", config)
