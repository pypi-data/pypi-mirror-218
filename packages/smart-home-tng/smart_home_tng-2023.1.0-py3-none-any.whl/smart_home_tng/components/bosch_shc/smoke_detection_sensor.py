...
class SmokeDetectionSensor(BoschEntity, core.BinarySensor.Entity):
    ...
    def _async_input_events_handler(self):
        ...
        self._owner.controller.bus.async_fire(
            Const.EVENT_BOSCH_SHC,
            {
                ...
                Const.ATTR_EVENT_SUBTYPE: self._device.alarmstate.name,
            },
        )
    ...
    def is_on(self):
        ...
        return (
            ...
            != bosch.SHCSmokeDetector.AlarmService.State.IDLE_OFF
        )
    ...
    async def async_request_smoketest(self):
        ...
        _LOGGER.debug(f"Requesting smoke test on entity {self.name}")
    ...
    async def async_request_alarmstate(self, command: str):
        ...
        _LOGGER.debug(f"Requesting custom alarm state {command} on entity {self.name}")
        await self._owner.controller.async_add_executor_job(
