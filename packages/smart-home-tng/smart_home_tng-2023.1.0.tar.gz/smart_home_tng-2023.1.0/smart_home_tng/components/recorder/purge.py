...
_DEFAULT_STATES_BATCHES_PER_PURGE: typing.Final = 20  # We expect ~95% de-dupe rate
_DEFAULT_EVENTS_BATCHES_PER_PURGE: typing.Final = 15  # We expect ~92% de-dupe rate


def take(take_num: int, iterable: collections.abc.Iterable) -> list[typing.Any]:
    """Return first n items of the iterable as a list.

    From itertools recipes
    """
    return list(it.islice(iterable, take_num))


def chunked(
    iterable: collections.abc.Iterable, chunked_num: int
) -> collections.abc.Iterable[typing.Any]:
    """Break *iterable* into lists of length *n*.

    From more-itertools
    """
    return iter(functools.partial(take, chunked_num, iter(iterable)), [])


def purge_old_data(
    instance: Recorder,
    purge_before: datetime.datetime,
    repack: bool,
    apply_filter: bool = False,
    events_batch_size: int = _DEFAULT_EVENTS_BATCHES_PER_PURGE,
    states_batch_size: int = _DEFAULT_STATES_BATCHES_PER_PURGE,
) -> bool:
    ...
    """
    _LOGGER.debug(
        "Purging states and events before target "
        + f"{purge_before.isoformat(sep=' ', timespec='seconds')}",
    )
    using_sqlite = instance.dialect_name == Const.SupportedDialect.SQLITE
    ...
    if repack:
        pack.repack_database(instance)
    return True


def _purging_legacy_format(session: sql_orm.Session) -> bool:
    """Check if there are any legacy event_id linked states rows remaining."""
    return bool(session.execute(queries.find_legacy_row()).scalar())
...
def _purge_states_and_attributes_ids(
    ...
    _LOGGER.debug(
        f"After purging states and attributes_ids remaining={has_remaining_state_ids_to_purge}",
    )
...
def _purge_events_and_data_ids(
    ...
    _LOGGER.debug(
        f"After purging event and data_ids remaining={has_remaining_event_ids_to_purge}"
    )
    return has_remaining_event_ids_to_purge


def _select_state_attributes_ids_to_purge(
    session: sql_orm.Session, purge_before: datetime.datetime
) -> tuple[set[int], set[int]]:
    """Return sets of state and attribute ids to purge."""
    state_ids = set()
    attributes_ids = set()
    for state in session.execute(queries.find_states_to_purge(purge_before)).all():
        state_ids.add(state.state_id)
        if state.attributes_id:
            attributes_ids.add(state.attributes_id)
    _LOGGER.debug(
        f"Selected {len(state_ids)} state ids and {len(attributes_ids)} attributes_ids to remove",
    )
    return state_ids, attributes_ids


def _select_event_data_ids_to_purge(
    session: sql_orm.Session, purge_before: datetime
) -> tuple[set[int], set[int]]:
    """Return sets of event and data ids to purge."""
    event_ids = set()
    data_ids = set()
    for event in session.execute(queries.find_events_to_purge(purge_before)).all():
        event_ids.add(event.event_id)
        if event.data_id:
            data_ids.add(event.data_id)
    _LOGGER.debug(
        f"Selected {len(event_ids)} event ids and {len(data_ids)} data_ids to remove"
    )
    return event_ids, data_ids


def _select_unused_attributes_ids(
    session: sql_orm.Session, attributes_ids: set[int], using_sqlite: bool
) -> set[int]:
    ...
    if using_sqlite:
        ...
        seen_ids = {
            ...
            for state in session.execute(
                queries.attributes_ids_exist_in_states_sqlite(attributes_ids)
            ).all()
        }
    else:
        #
        ...
        for attr_ids in it.zip_longest(*groups, fillvalue=None):
            seen_ids |= {
                attrs_id[0]
                for attrs_id in session.execute(
                    queries.attributes_ids_exist_in_states(*attr_ids)
                ).all()
                if attrs_id[0] is not None
            }
    to_remove = attributes_ids - seen_ids
    _LOGGER.debug(
        f"Selected {len(to_remove)} shared attributes to remove",
    )
    return to_remove
...
def _select_unused_event_data_ids(
    session: sql_orm.Session, data_ids: set[int], using_sqlite: bool
) -> set[int]:
    ...
    if using_sqlite:
        seen_ids = {
            state[0]
            for state in session.execute(
                queries.data_ids_exist_in_events_sqlite(data_ids)
            ).all()
        }
    else:
        seen_ids = set()
        groups = [iter(data_ids)] * 100
        for data_ids_group in it.zip_longest(*groups, fillvalue=None):
            seen_ids |= {
                data_id[0]
                for data_id in session.execute(
                    queries.data_ids_exist_in_events(*data_ids_group)
                ).all()
                if data_id[0] is not None
            }
    to_remove = data_ids - seen_ids
    _LOGGER.debug(f"Selected {len(to_remove)} shared event data to remove")
    return to_remove
...
def _select_short_term_statistics_to_purge(
    ...
    statistics = session.execute(
        queries.find_short_term_statistics_to_purge(purge_before)
    ).all()
    _LOGGER.debug(
        f"Selected {len(statistics)} short term statistics to remove", len(statistics)
    )
...
def _select_legacy_event_state_and_attributes_and_data_ids_to_purge(
    ...
    events = session.execute(
        queries.find_legacy_event_state_and_attributes_and_data_ids_to_purge(
            purge_before
        )
    ).all()
    _LOGGER.debug(f"Selected {len(events)} event ids to remove")
    event_ids = set()
    state_ids = set()
    attributes_ids = set()
    data_ids = set()
...
def _purge_state_ids(
    instance: Recorder, session: sql_orm.Session, state_ids: set[int]
) -> None:
    ...
    # for us.
    disconnected_rows = session.execute(queries.disconnect_states_rows(state_ids))
    _LOGGER.debug(f"Updated {disconnected_rows} states to remove old_state_id")

    deleted_rows = session.execute(queries.delete_states_rows(state_ids))
    _LOGGER.debug(f"Deleted {deleted_rows} states")
...
def _purge_batch_attributes_ids(
    instance: Recorder, session: sql_orm.Session, attributes_ids: set[int]
) -> None:
    """Delete old attributes ids in batches of MAX_ROWS_TO_PURGE."""
    for attributes_ids_chunk in chunked(attributes_ids, Const.MAX_ROWS_TO_PURGE):
        deleted_rows = session.execute(
            queries.delete_states_attributes_rows(attributes_ids_chunk)
        )
        _LOGGER.debug(f"Deleted {deleted_rows} attribute states")
...
def _purge_batch_data_ids(
    instance: Recorder, session: sql_orm.Session, data_ids: set[int]
) -> None:
    """Delete old event data ids in batches of MAX_ROWS_TO_PURGE."""
    for data_ids_chunk in chunked(data_ids, Const.MAX_ROWS_TO_PURGE):
        deleted_rows = session.execute(queries.delete_event_data_rows(data_ids_chunk))
        _LOGGER.debug(f"Deleted {deleted_rows} data events")
...
def _purge_statistics_runs(
    session: sql_orm.Session, statistics_runs: list[int]
) -> None:
    """Delete by run_id."""
    deleted_rows = session.execute(queries.delete_statistics_runs_rows(statistics_runs))
    _LOGGER.debug(f"Deleted {deleted_rows} statistic runs")


def _purge_short_term_statistics(
    session: sql_orm.Session, short_term_statistics: list[int]
) -> None:
    """Delete by id."""
    deleted_rows = session.execute(
        queries.delete_statistics_short_term_rows(short_term_statistics)
    )
    _LOGGER.debug(f"Deleted {deleted_rows} short term statistics")


def _purge_event_ids(
    session: sql_orm.Session, event_ids: collections.abc.Iterable[int]
) -> None:
    """Delete by event id."""
    deleted_rows = session.execute(queries.delete_event_rows(event_ids))
    _LOGGER.debug(f"Deleted {deleted_rows} events")


def _purge_old_recorder_runs(
    instance: Recorder, session: sql_orm.Session, purge_before: datetime.datetime
) -> None:
    """Purge all old recorder runs."""
    # Recorder runs is small, no need to batch run it
    deleted_rows = session.execute(
        queries.delete_recorder_runs_rows(
            purge_before, instance.run_history.current.run_id
        )
    )
    _LOGGER.debug(f"Deleted {deleted_rows} recorder_runs")


def _purge_filtered_data(instance: Recorder, session: sql_orm.Session) -> bool:
    ...
    excluded_entity_ids: list[str] = [
        entity_id
        for (entity_id,) in session.query(sql.distinct(model.States.entity_id)).all()
    ...
    excluded_event_types: list[str] = [
        event_type
        for (event_type,) in session.query(sql.distinct(model.Events.event_type)).all()
...
def _purge_filtered_states(
    ...
    state_ids, attributes_ids, event_ids = zip(
        *(
            session.query(
                model.States.state_id, model.States.attributes_id, model.States.event_id
            )
            .filter(model.States.entity_id.in_(excluded_entity_ids))
    ...
    _LOGGER.debug(
        f"Selected {len(state_ids)} state_ids to remove that should be filtered"
    )
...
def _purge_filtered_events(
    instance: Recorder, session: sql_orm.Session, excluded_event_types: list[str]
) -> None:
    """Remove filtered events and linked states."""
    using_sqlite = instance.dialect_name == Const.SupportedDialect.SQLITE
    event_ids, data_ids = zip(
        *(
            session.query(model.Events.event_id, model.Events.data_id)
            .filter(model.Events.event_type.in_(excluded_event_types))
            .limit(Const.MAX_ROWS_TO_PURGE)
            .all()
        )
    )
    _LOGGER.debug(
        f"Selected {len(event_ids)} event_ids to remove that should be filtered"
    )
    states: list[model.States] = (
        session.query(model.States.state_id)
        .filter(model.States.event_id.in_(event_ids))
        .all()
    )
    ...
    if unused_data_ids_set := _select_unused_event_data_ids(
        ...
        _purge_batch_data_ids(instance, session, unused_data_ids_set)
    if core.Const.EVENT_STATE_CHANGED in excluded_event_types:
        session.query(model.StateAttributes).delete(synchronize_session=False)
        instance._state_attributes_ids = {}  # pylint: disable=protected-access


def purge_entity_data(
    instance: Recorder, entity_filter: collections.abc.Callable[[str], bool]
) -> bool:
    ...
    with util.session_scope(session=instance.get_session()) as session:
        selected_entity_ids: list[str] = [
            entity_id
            for (entity_id,) in session.query(
                sql.distinct(model.States.entity_id)
            ).all()
            if entity_filter(entity_id)
        ]
        _LOGGER.debug(f"Purging entity data for {selected_entity_ids}")
