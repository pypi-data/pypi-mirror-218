...
class Condition(core.ConditionPlatform):
    ...
    async def async_validate_condition_config(
        ...
        try:
            config = _cv.DEVICE_CONDITION_SCHEMA(config)
            platform = await self._owner.async_get_device_automation_platform(
                config[core.Const.CONF_DOMAIN], core.DeviceAutomation.Type.CONDITION
            )
            if isinstance(platform, types.ModuleType):
                ...
                return typing.cast(core.ConfigType, platform.CONDITION_SCHEMA(config))
            if isinstance(platform, core.ActionConditionPlatform):
                return await platform.async_validate_condition_config(config)
            raise vol.Invalid("Invalid condition configuration")
    ...
    async def async_condition_from_config(
        ...
        if isinstance(platform, types.ModuleType) and hasattr(
            ...
            # Legacy impl, will be removed
            return platform.async_condition_from_config(self._owner.controller, config)

        if isinstance(platform, core.ActionConditionPlatform):
            return platform.async_condition_from_config(config)
