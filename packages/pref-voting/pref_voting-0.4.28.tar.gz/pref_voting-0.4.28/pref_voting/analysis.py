...
def bootstrap_cia(
    ...
    lowers, uppers = np.percentile(bootstrap_means, 
                                     [(1-confidence)/2*100, 
                                      (1+confidence)/2*100], axis=averaging_axis)
    half_widths = (uppers - lowers) / 2
    
    variance_bootstrap_means = np.var(bootstrap_means, axis=averaging_axis)
    ...
    while np.any(half_widths > precision):
        if verbose:
            ...
            print("Remaining half widths:", np.sum(half_widths > precision))
        ...
        means = np.mean(bootstrap_means, axis=averaging_axis)
        lowers, uppers = np.percentile(bootstrap_means,
                                       [(1-confidence)/2*100, 
                                        (1+confidence)/2*100], axis=averaging_axis)
        half_widths = (uppers - lowers) / 2
