...
class SymbolicEstimator(BaseEstimator, ABC):
    def __init__(self, population_size=10000, generations=20, stopping_criteria=0.95, store_of_fame=50,
        ...
        if constants is not None:
            self.constants_ = list(constants)
            self.constant_range_ = None
        else:
            self.constants_ = None
            self.constant_range_ = constant_range
            if constant_range is not None:
                assert constant_range[0] < constant_range[1]

        if isinstance(func_p, (list, tuple)):
            func_p = list(func_p)
            self.func_p_ = np.array(func_p)
        else:
            self.func_p_ = func_p
    ...
    def refresh_xcs(self):
        ...
        if self.constants_ is None and self.constant_range_ is None:
            temp_cons = np.zeros(self.constant_n)
            self.constant_gen.append(temp_cons)
            return self.xs, None, self.xs.shape[0], self.y

        elif self.constants_ is None and self.constant_range_ is not None:
            if self.temp_c is not None:
                temp_cons = self.temp_c
                index = temp_cons == 0
                sum0 = np.sum(index)
                temp_cons[index] = np.random.random(sum0) * (self.constant_range_[1] - self.constant_range_[0]) + \
                                   self.constant_range_[0]
                ...
                temp_cons = temp_cons * (self.constant_range_[1] - self.constant_range_[0]) + self.constant_range_[0]
        else:
            temp_cons = np.array(self.constants_).astype(np.float32)
    ...
    def run_gp(self):
        ...
        population = generate_random(self.func_num, xs_num=xcs_num, pop_size=self.pop_n,
                                     ...
                                     p=None, func_p=self.func_p_, xs_p=xcs_p)
