...
from pyrography.handlers import (
...
class Dispatcher:
    ...
    def __init__(self, client: "pyrography.Client"):
        self.client = client
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyrography.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyrography.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyrography.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                pyrography.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyrography.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyrography.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyrography.types.ChatJoinRequest._parse(self.client, update, users, chats),
                    ...
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            try:
                                ...
                                raise
                            except pyrography.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
                            finally:
