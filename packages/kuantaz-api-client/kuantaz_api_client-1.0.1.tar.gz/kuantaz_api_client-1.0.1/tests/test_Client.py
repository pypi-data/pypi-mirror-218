import json
import pytest
from kuantaz_api_client import Client, RequestHelper, VerificationResult, StatisticResult, KuantazException

def test_verify_submission_without_tokens():
    api_client = Client('http://test.local', 'testPublicKey', 'testPrivateKey')

    with pytest.raises(KuantazException) as exc:
...
def test_verify_submission_without_validation_tokens():
    ...
    with pytest.raises(KuantazException) as exc:
        result = api_client.verify_submission({'name': 'John Example', '_kuantaz_submitToken': 'submitToken'})
...
def test_verify_submission_form_tokens_empty_response(requests_mock):
    ...
    with pytest.raises(KuantazException) as exc:
        result = api_client.verify_submission({
            'name': 'John Example',
            '_kuantaz_submitToken': 'submitToken',
            '_kuantaz_validationToken': 'validationToken'
        })
...
def test_verify_submission_connection_error(requests_mock):
    ...
    assert 'An error occurred while sending the request to kuantaz.' in str(exc.value)
