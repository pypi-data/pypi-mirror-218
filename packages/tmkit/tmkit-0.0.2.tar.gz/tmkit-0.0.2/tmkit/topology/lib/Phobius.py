...
#
# PHOBIUS (REST) web service Python client using xmltramp2.
#
...
baseUrl = u'https://www.ebi.ac.uk/Tools/services/rest/phobius'
...
def print_usage():
    print("""\
EMBL-EBI PHOBIUS Python Client:
...
[General]
  ...
  --baseUrl             Base URL. Defaults to:
                        https://www.ebi.ac.uk/Tools/services/rest/phobius

Synchronous job:
  The results/errors are returned as soon as the job is finished.
  Usage: python phobius.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python phobius.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python phobius.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python phobius.py --polljob --jobid <jobId> [--outfile string]
