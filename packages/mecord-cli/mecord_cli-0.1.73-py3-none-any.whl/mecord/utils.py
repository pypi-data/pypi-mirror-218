...
def get_mac_address():
    if platform.system() == 'Windows':
        ...
        ok, mac = get_mac_from_nettools()
        if ok:
            return mac
        ok, mac = get_mac_from_system()
        if ok:
            return mac
        return None
    else:
...
def isTest():
    is_test = None
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "env.txt")
    try:
        with open(filename) as fr:
            txt = fr.read()
            is_test = txt.strip() if txt else None
    except:
        pass

    is_test = "" if is_test is None else is_test

    # # 屏蔽写回
    # with open(filename, "a+") as fw:
    #     fw.truncate(0)
    #     fw.write((keyString + "={value}\n").format(value=is_test))

    return len(is_test) > 0
isTest()
def show_help():
    ...
    print('    mecord env [test/product]      set env')
