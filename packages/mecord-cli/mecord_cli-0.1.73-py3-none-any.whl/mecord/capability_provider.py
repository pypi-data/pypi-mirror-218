...
class CapabilityProvider:
    ...
    def version(self):
        ...
        print(f"version is {ver}{test_env}{salt_env}")
    ...
    def env(self):
        if len(sys.argv) <= 2:
            print('please set command! Usage: mecord env [test/product]]')
            return
        value = sys.argv[2]
        ...
        with open(env_file_path, 'w+') as f:
            if value == "test":
                print('your device environment is test')
                f.write('test')
            else:
                print('your device environment is product')
                f.write('')
