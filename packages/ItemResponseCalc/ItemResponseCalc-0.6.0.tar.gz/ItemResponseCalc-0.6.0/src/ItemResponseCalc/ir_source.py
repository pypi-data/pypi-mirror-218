...
ItemResponseSqlite3 --- interface to an SQL database accessed by pd.read_sql_query using sqlite3
...
def item_response_table(items,
    ...
    """Factory creating an access object to a source of item response data.
        ...
        :param source: source for reading input data,
            ...
            OR a SQLAlchemy engine object, if reading from a general SQL database,
...
# def join(*args):
#     """Chain several ItemResponseTable (subclass) instances into one single iterable
#     :param args: sequence of ItemResponseTable objects
#     :return: single Tables instance chaining all args
#     """
#     return Tables(args)
...
# -------------------------------------------------------
# class ItemResponseSqlite3(ItemResponseTable):
#     """Access class for sqlite3 database storage of item response data,
#     using pd.read_sql_query
#     """
#     def __init__(self, con, sql, items, accept_fcn=None, sample_factor=1, **kwargs):
#         """
#         :param con: sqlite3.connection instance
#         :param sql: string with SQL query, for pd.read_sql_query
#         :param items: dict with (column_name, list(ordinal categories))
#         :param respondent: (optional) column header for respondent identification labels
#             used to avoid duplicate respondent records
#         :param accept_fcn: (optional) test function for each row to be valid input
#         :param sample_factor: (optional) scalar integer for down-sampling among records
#             Only one record is returned for every sample_factor records in the file
#         :param kwargs: (optional) additional arguments for pd.read_sql_query, e.g.,
...
#             dtype = dict with column data types
#         """
#         super().__init__(items=items, accept_fcn=accept_fcn, sample_factor=sample_factor)
#         self.con = con
#         self.sql = sql
...
#                                **self.kwargs)
#         except Exception as e:
#             raise ReadError(f'read_sql_query({self.sql}, {self.con}) error: {e}')
