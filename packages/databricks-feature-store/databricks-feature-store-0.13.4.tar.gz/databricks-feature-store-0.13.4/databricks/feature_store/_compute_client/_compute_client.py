...
class ComputeClient:
    ...
    def register_table(
        ...
        *,
        ...
        # Check delta PK constraint for UC table
        if is_uc_table:
            self._check_pk_tk_constraint(
                full_table_name=delta_table,
                primary_keys=primary_keys_as_list,
                timestamp_keys=timestamp_keys_as_list,
            )
    ...
    def write_table(
        ...
        if uc_utils.is_uc_entity(name):
            try:
                ...
                self._set_pk_tk_if_not_exist(
                    full_table_name=name,
                    primary_keys=feature_table.primary_keys,
                    timestamp_keys=feature_table.timestamp_keys,
                )
                # Check if existing PK/TK constraint match with expected one.
                self._check_pk_tk_constraint(
                    full_table_name=name,
                    primary_keys=feature_table.primary_keys,
                    timestamp_keys=feature_table.timestamp_keys,
                ...
                if "PERMISSION_DENIED" in str(e):
                    expected_pk = (
                        feature_table.primary_keys + feature_table.timestamp_keys
                    )
                    _logger.warning(
                        f"Failed to set primary keys constraint {expected_pk} on the table '{name}'. "
                        f"Unity catalog feature tables must have primary keys annotated. "
                        f"Primary key constraint can only be set by the table owner. "
                        f"Please reach out the the table owner to set the primary key constraint by running "
                        f"{SparkClient.set_pk_statement(name, expected_pk)}."
                    )
                else:
                    raise e
    ...
    def get_table(self, name: str, req_context: RequestContext) -> FeatureTable:
        """
        Get FeatureTable from catalog service.
        Warn if catalog schema does not match the delta table schema.
        Set tags.
        Make TK part of PK.
        """
        self._spark_client_helper.check_feature_table_exists(name)
    ...
    def _check_pk_tk_constraint(
        ...
        """
        Check PK/TK constraint on UC table.

        Raise error if existing PK constraint does not match with expected PK constraint.

        # TODO: check TK constraint after SQL syntax is available
        """
        catalog_name, schema_name, table_name = full_table_name.split(".")
        # Expected delta PKs are equivalent to FS PKs + TKs in order
        expected_pk = primary_keys + timestamp_keys
        ...
        if existing_pk:
            # Check if existing PK constraint match with expected PK constraint
            if not expected_pk == existing_pk:
                raise ValueError(
                    f"Existing Primary Key constraint {utils.sanitize_identifiers(existing_pk)} for Delta table {utils.sanitize_multi_level_name(full_table_name)} does not "
                    f"match with expected Primary Key constraint {utils.sanitize_identifiers(expected_pk)}. "
                    f"Use 'ALTER TABLE {utils.sanitize_multi_level_name(full_table_name)} DROP PRIMARY KEY;' to drop existing PK constraint and "
                    f"'ALTER TABLE {utils.sanitize_multi_level_name(full_table_name)} ADD CONSTRAINT {utils.sanitize_identifier(table_name + '_pk')} PRIMARY KEY({', '.join(utils.sanitize_identifiers(expected_pk))})' "
                    f"to add expected Primary Key constraint on the Delta table."
                )
        else:
            # No existing PK constraint
            if expected_pk:
                # But expect a PK constraint
                raise ValueError(
                    f"Expect Primary Key constraint {utils.sanitize_identifiers(expected_pk)} for Delta table {utils.sanitize_multi_level_name(full_table_name)} but found none. "
                    f"Use 'ALTER TABLE {utils.sanitize_multi_level_name(full_table_name)} ADD CONSTRAINT {utils.sanitize_identifier(table_name + '_pk')} PRIMARY KEY({', '.join(utils.sanitize_identifiers(expected_pk))})' "
                    f"to add expected Primary Key constraint on the Delta table."
                )
    ...
    def _set_pk_tk_if_not_exist(
        ...
        # TODO: set TK constraint after SQL syntax is available
        """
        # Expected delta PKs are equivalent to FS PKs + TKs in order
        expected_pk = primary_keys + timestamp_keys
        # Get existing PK of the table from SHOW CREATE TABLE
        ...
        if not existing_pk and expected_pk:
            ...
            # Set PK constraint to all delta primary keys
            self._spark_client.set_cols_pk(
                full_table_name=full_table_name, pk_cols=expected_pk
            )
