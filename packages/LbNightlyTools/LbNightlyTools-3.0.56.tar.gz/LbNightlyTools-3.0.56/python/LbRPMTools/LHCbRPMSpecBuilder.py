...
class LHCbBaseRpmSpec(object):
    ...
    def _createHEPToolsRequires(self):
        ...
        try:
            ...
            for p in version_mimatch:
                tp = packages_info[p]
                v = used_packages[p]
                __log__.warning("Version mismatch for %s, expected %s, got %s",
                                p, v, tp['version'])
        ...
        requires = []
        for n, p in packages_info.items():
    ...
    def _getMatchingRPMFromRepo(self, name, version):
        ...
        with TemporaryDir() as siteroot:
            return Installer(siteroot=siteroot).remoteFindPackage(
    ...
    def _setNextReleaseNumberFromRepo(self):
        ...
        if self._lhcb_release_version <= 0:
            if self._checkRelease:
                ...
                __log__.debug("Looking for releases of %s / %s" %
                ...
                if not packages:
                    ...
                    __log__.warning("Release number set to %s" %
                                    self._lhcb_release_version)
