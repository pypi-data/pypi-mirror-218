...
from LbNightlyTools.Utils import log_call as _log_call, find_path, compatible_lcg_external_files
try:  # py3
...
class cmake_new(object):
    ...
    def configure(self, proj, **kwargs):
        ...
        if "LCG_EXTERNALS_FILE" in kwargs.get("cache_entries", []):
            ...
            ext_file = ext_file.replace("$ENV{BINARY_TAG}",
            ...
            for ext_file in compatible_lcg_external_files(ext_file):
                ...
                if os.path.exists(ext_file):
                    ...
                    kwargs["cache_entries"]["LHCB_PLATFORM"] = os.environ[
                        'BINARY_TAG']
                    kwargs["cache_entries"]["LCG_ARCHITECTURE"] = os.environ[
                        'BINARY_TAG'].split("-", 1)[0]
                    break
            ...
            for dirpath, _dirnames, filenames in os.walk(
                ...
                if "cmake_install.cmake" in filenames:
                    with open(os.path.join(dirpath,
                                           "cmake_install.cmake")) as f:
                        data = f.read()
                    with open(
                            os.path.join(dirpath, "cmake_install.cmake"),
                            "w") as f:
                        f.write(
                            data.replace("file(INSTALL",
                                         "file(INSTALL OPTIONAL"))
        return self._call(
            proj, [
                'cmake', '--install',
