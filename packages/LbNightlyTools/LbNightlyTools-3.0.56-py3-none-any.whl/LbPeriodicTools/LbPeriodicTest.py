...
class PeriodicTest(object):
    ...
    def getBuildMatches(self, buildlist):
        ...
        for buildinfo in buildlist:
            ...
            if not fullmatch(self.slot, buildinfo["slot"]):  # pylint: disable=E1101
                continue
            ...
            for proj in buildinfo["projects"]:
                ...
                for platf in buildinfo["platforms"]:
                    ...
                    if not (fullmatch(self.platform, platf)
                            or fnmatch(platf, self.platform)):
                        continue
                    # Platform matches as well, we can add the
                    # triplet to the list

                    # Check if a label pattern is defined and any label matches
                    labels = buildinfo.get("metadata", {}).get("labels", [])
                    if (not self.label
                            or any(fullmatch(self.label, x) for x in labels)):
                        sched_test = ScheduledTest(
                            buildinfo["build_id"], buildinfo["slot"],
                            proj["name"], platf, self.testgroup,
                            self.testrunner, self.os_label, self.scheduletype,
                            self.scheduletime, self.testenv, self.count)
