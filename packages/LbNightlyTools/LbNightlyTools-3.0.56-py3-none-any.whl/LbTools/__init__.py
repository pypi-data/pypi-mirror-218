def get_toolchain_info(version):
    ...
    url = "https://gitlab.cern.ch/lhcb-core/lcg-toolchains/-/raw/add-lcginfo-dump/LCG_{version}/lcginfo.json"
    import requests
    import json
...
def get_toolchain_packages(version,
                           platform,
                           used_packages,
                           toolchain_variant="LHCB_Core"):
    ...
    if info is None:
        raise Exception("{tv} not found for LCG {v}".format(
            tv=toolchain_variant, v=version))

    # Looking for the packages for the platform
    if platform not in list(info.keys()):
        raise Exception(
    ...
    not_found = []
    version_mismatch = []
    for p, v in list(used_packages.items()):
